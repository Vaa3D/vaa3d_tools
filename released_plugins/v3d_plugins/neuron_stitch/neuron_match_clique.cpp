...
void NeuronLiveMatchDialog::enterEvent(QEvent *e)
{
    ...
    QDialog::enterEvent((QEnterEvent *)e);
}
...
void NeuronLiveMatchDialog::load_img0()
{
    ...
    if(sz_tmp[3]!=1){
        bool ok;
        ch=QInputDialog::getInt(0, "select channel","Which channel to display? (start from 0)",0,0,sz_tmp[3]-1,1,&ok);
        if(!ok)
            return;
    }
...
void neuron_match_clique::output_parameter(QString fname)
{
    ...
    myfile<<"stack_direction(0:x,1:y,2:z)= "<<direction<<"\n";  //stack direction, 0x, 1y, 2z
    myfile<<"candidate_search_plane= "<<midplane<<"\n";    //coordinate of the plane in between
    myfile<<"candidate_search_threshold_distance_to_plane= "<<spanCand<<"\n";  //searching span from the stack plane for the candidate
    myfile<<"candidate_search_threshold_angle_to_plane(cos(a))= "<<angThr_stack<<"\n"; //angular threshold for candidate in stack direction (-1 ~ 1), -1 means nothing will be thresholded
    myfile<<"candidate_search_fragments_threshold= "<<segmentThr<<"\n";  //threshold to filter out small segments when selecting candidates
    myfile<<"candidate_search_section_gap_threshold= "<<gapThr<<"\n";  //threshold to filter out small gaps between segments
    myfile<<"point_match_threshold_distance= "<<pmatchThr<<"\n";   //match threshold for points
    myfile<<"point_match_threshold_angular(cos(a))= "<<angThr_match<<"\n"; //angular threshold for clique match (-1 ~ 1)=cos(theta), when angle is larger than theta (<cos(theta)), the point will not be matched
    myfile<<"triangle_match_threshold_distance= "<<cmatchThr<<"\n";   //match threshold for length of cliques
    myfile<<"stack_direction_rescale= "<<zscale<<"\n";  //resacle stack direction
    myfile<<"segment_length_for_calculate_direction= "<<dir_range<<"\n";   //length of section used to calculate direction of dead end
    myfile<<"spine_length_point_number= "<<spineLengthThr<<"\n"; //number of maximum points for spine
    myfile<<"spine_angle_threshold= "<<spineAngThr<<"\n"; //angular threshold for spine turning
    myfile<<"max_number_of_triangles_to_match= "<<maxClique3Num<<"\n"; //compare less triangles can increase the speed
    myfile<<"match_branches_by_type= "<<(typeConstrain?1:0)<<"\n"; //if true/1, only match the branch with the same type defined in SWC
    file.close();
}
...
void neuron_match_clique::output_matchedMarkers(QString fname, const NeuronTree& nt, QList<int> points)
{
    ...
    QTextStream myfile(&file);
    myfile<<"##x,y,z,radius,shape,name,comment, color_r,color_g,color_b"<<"\n";
    int idx=0;
    //matched candidates first
...
void neuron_match_clique::output_matchedMarkers_orgspace(QString fname_0, QString fname_1)
{
    ...
    for(int i=0; i<pmatch0.size(); i++){
        ...
        myfile<<nt0_org->listNeuron[a].x<<","<<nt0_org->listNeuron[a].y<<","<<nt0_org->listNeuron[a].z<<",0,1, "<<idx<<","
             <<"0 "<<candID0.at(aa)<<" -1 "
             <<canddir0[aa].x<<" "<<canddir0[aa].y<<" "<<canddir0[aa].z<<", 255, 0, 0"<<"\n";
        idx++;
    }
    file.close();
    ...
    myfile1<<"##x,y,z,radius,shape,name,comment, color_r,color_g,color_b"<<"\n";
    idx=0;
    for(int i=0; i<pmatch0.size(); i++){
        int b=pmatch1.at(i);
        int bb=candmatch1.at(i);
        myfile1<<nt1_org->listNeuron[b].x<<","<<nt1_org->listNeuron[b].y<<","<<nt1_org->listNeuron[b].z<<",0,1, "<<idx<<","
             <<"1 "<<candID1.at(bb)<<" -1 "
             <<canddir1[bb].x<<" "<<canddir1[bb].y<<" "<<canddir1[bb].z<<", 0, 255, 0"<<"\n";
        idx++;
    }
    file1.close();
}
...
void neuron_match_clique::output_markers_candspace(QString fname)
{
    ...
    for(int i=0; i<candID0.size(); i++){
        ...
        myfile<<candcoord0[i].x<<","<<candcoord0[i].y<<","<<candcoord0[i].z<<",0,1, "<<idx++<<","
             <<"0 "<<candID0.at(i)<<" -1 "
             <<canddir0[i].x<<" "<<canddir0[i].y<<" "<<canddir0[i].z<<", 128, 0, 128"<<"\n";
    }
    for(int i=0; i<candID1.size(); i++){
        if(candmatch1.contains(i)) continue;
        myfile<<candcoord1[i].x<<","<<candcoord1[i].y<<","<<candcoord1[i].z<<",0,1, "<<idx++<<","
             <<"1 "<<candID1.at(i)<<" -1 "
             <<canddir1[i].x<<" "<<canddir1[i].y<<" "<<canddir1[i].z<<", 0, 128, 128"<<"\n";
    }
    file.close();
}
...
//        return;
    ...
    out<<"SWCFILE="<<fname_neuron0<<"\n";
    out<<"SWCFILE="<<fname_neuron1<<"\n";
    qf_anofile.close();
}
...
void neuron_match_clique::output_affine(QString fname, QString fname_nt0)
{
    ...
    QTextStream out(&qf_anofile);
    out<<"SWCFILE="<<fname_nt0<<"\n";
    out<<"SWCFILE="<<fname_neuron<<"\n";
    qf_anofile.close();
}
