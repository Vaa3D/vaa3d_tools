...
bool proc_app1(V3DPluginCallback2 &callback, PARA_APP1 &p, const QString & versionStr)
{
    ...
    if (!p.p4dImage || !p.p4dImage->valid())
    {
        ...
        if(!p.inmarker_file.isEmpty())  //这里是查看一下准备的数据p中有没有记录相应的marker信息
...
//        cout << *it <<endl;
    ...
    vector<ImageMarker> inmarkers;
    {
        for(V3DLONG i = 0; i < p.landmarks.size(); i++)
        {
            ...
            {
                //add scaling by PHC 121127 如果在交互窗口设置了下采样的比例，那么marker的坐标也需要进行相应的下采样
                x /= dfactor_xy;
                y /= dfactor_xy;
                z /= dfactor_z;
            }
    ...
    if (1)
    {
        ...
        if(!p.outswc_file.isEmpty())
            ...
            outswc_file = QString(p.p4dImage->getFileName()) + rootposstr + "_rayshootingTipAndApp1.swc"; //这里修改下输出的名字 by lkr
