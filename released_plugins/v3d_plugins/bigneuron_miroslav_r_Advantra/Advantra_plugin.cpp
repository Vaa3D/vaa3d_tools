...
//    cout<<"perc = "     <<PARA.perc                 <<endl;
    ...
    unsigned char * scr_1 = new unsigned char[size]; // score
    ...
    if (PARA.saveMidres) {
        QString of = PARA.inimg_file + "_scr_1.tif";
        simple_saveimage_wrapper(callback, of.toStdString().c_str(), scr_1, in_sz, V3D_UINT8);
...
//    unsigned char th = quantile(scr, PARA.perc, 100);
     ...
     // threshold
     unsigned char scr_1_min = scr_1[0];
     unsigned char scr_1_max = scr_1[0];

     for (long i = 0; i < size; ++i) {
         if (scr_1[i]<scr_1_min) scr_1_min=scr_1[i];
         if (scr_1[i]>scr_1_max) scr_1_max=scr_1[i];
     }

    int cntvals = 0;

    for (long i = 0; i < size; ++i) {
        if (scr_1[i]>scr_1_min && scr_1[i]<scr_1_max) {
            cntvals++;
        }
    }

    unsigned char * scr_1_temp = new unsigned char[cntvals];

    cntvals = 0;
    for (long i = 0; i < size; ++i) {
        if (scr_1[i]>scr_1_min && scr_1[i]<scr_1_max) {
            scr_1_temp[cntvals] = scr_1[i];
            cntvals++;
        }
    }

//    cout << "globalth" << endl;
    if (scr_1_min==scr_1_max) return;

//    unsigned char th1 = intermodes_th(data1d_temp, cntvals);

//    unsigned char th1 = otsu_th(data1d_temp, cntvals);

    unsigned char th = (cntvals==0)? scr_1_max : maxentropy_th(scr_1_temp, cntvals);
//    unsigned char th = maxentropy_th(scr_1, size);
//    cout << "\t" << PARA.perc << "th = " << (int)th << endl;

    delete scr_1_temp; scr_1_temp = 0;

    // assign those that are in the foreground with -1 (necessary to disctinct from background)
    for (long i = 0; i < size; ++i) {
        if (scr_1[i]>=th && tagmap[i]==0)
            tagmap[i] = -1;
    }

    delete scr_1; scr_1 = 0;
    ...
    for (long i = 0; i < cnt_fg; ++i) { // go through fg. locs (-1)
        ...
        if (tagmap[z*N*M+y*N+x]==-1) { // can be a candidate
            ...
            if (i2calcs[i][0]>PARA.znccTh) { // check local neighbourhood for locally optimal
                ...
                int gpntr_idx = mround(i2calcs[lmax_i][1]);
