...
bool do_straighten(V3DPluginCallback2 &callback, ParaDialog &paraDlg, QString &qs_filename_img, QString &qs_filename_marker_input, CSParas &paras, QString &qs_filename_strimg_output )
{
     ...
     else
     {
          ...
          V3DLONG l_imgind=paraDlg.comboBox_imgv3d->currentIndex();
          if(l_imgind<0)
          {
          ...
          for(V3DLONG i=0;i<roils_roi.size();i++)
          {
               printf("\t>>roi-%ld: ",i);
               for(V3DLONG j=0;j<roils_roi[i].size();j++)
     ...
     unsigned char *p_img_roi=0;
     {
		...
		ql_boundingrect.push_back(QRect(0,0,sz_img_input[0],sz_img_input[2]));	//xz
          ...
          V3DLONG x_min=max(ql_boundingrect[0].left(),ql_boundingrect[2].left());
		V3DLONG x_max=min(ql_boundingrect[0].right(),ql_boundingrect[2].right());
		V3DLONG y_min=max(ql_boundingrect[0].top(),ql_boundingrect[1].top());
		V3DLONG y_max=min(ql_boundingrect[0].bottom(),ql_boundingrect[1].bottom());
		V3DLONG z_min=max(ql_boundingrect[1].left(),ql_boundingrect[2].top());
		V3DLONG z_max=min(ql_boundingrect[1].right(),ql_boundingrect[2].bottom());
		...
		//crop roi region
		V3DLONG n_pixels=sz_img_roi[0]*sz_img_roi[1]*sz_img_roi[2]*sz_img_roi[3];
		...
		V3DLONG pgsz_x=sz_img_input[0];
		V3DLONG pgsz_xy=sz_img_input[0]*sz_img_input[1];
		V3DLONG pgsz_xyz=sz_img_input[0]*sz_img_input[1]*sz_img_input[2];
		V3DLONG pgsz_x_roi=sz_img_roi[0];
		V3DLONG pgsz_xy_roi=sz_img_roi[0]*sz_img_roi[1];
		V3DLONG pgsz_xyz_roi=sz_img_roi[0]*sz_img_roi[1]*sz_img_roi[2];
		for(V3DLONG x_roi=0;x_roi<sz_img_roi[0];x_roi++)
			for(V3DLONG y_roi=0;y_roi<sz_img_roi[1];y_roi++)
				for(V3DLONG z_roi=0;z_roi<sz_img_roi[2];z_roi++)
				{
					V3DLONG x=x_roi+x_min;
					V3DLONG y=y_roi+y_min;
					V3DLONG z=z_roi+z_min;
					...
					{
						for(V3DLONG c=0;c<sz_img_roi[3];c++)
						{
							V3DLONG ind=pgsz_xyz*c+pgsz_xy*z+pgsz_x*y+x;
							V3DLONG ind_roi=pgsz_xyz_roi*c+pgsz_xy_roi*z_roi+pgsz_x_roi*y_roi+x_roi;
