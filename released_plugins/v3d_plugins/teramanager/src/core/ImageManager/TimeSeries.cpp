...
TimeSeries::TimeSeries(const char *rootDir, string frames_format /* = "" */) throw (IOException) : VirtualVolume(rootDir)
{
    ...
    if(frames_format.empty() || VirtualVolume::isHierarchical(frames_format))
    {
        ...
        if(entries.empty())
            throw IOException(strprintf("in TimeSeries::TimeSeries(): no hierarchical time frames found within \"%s\"", rootDir).c_str());
    ...
    else
    {
        ...
        for(list<string>::iterator it = entries.begin(); it != entries.end(); it++)
        {
            ...
            catch(IOException &ex){warning(strprintf("Cannot import filed time frame at \"%s\": %s", path.c_str(), ex.what()).c_str(),__iim__current__function__);}
            catch(...){warning(strprintf("Cannot import filed time frame at \"%s\"", path.c_str()).c_str(),__iim__current__function__);}
            if(volume)
                frames.push_back(volume);
        }
        ...
        // check condition #2: at least one time frame has been found
        if(frames.empty())
            throw IOException(strprintf("in TimeSeries::TimeSeries(): no filed time frames found within \"%s\"", rootDir).c_str());
    }
    ...
    for(int k=0; k<frames.size()-1; k++)
    {
        ...
        if( frames[k]->getDIM_H() != frames[k+1]->getDIM_H())
            throw IOException(strprintf("in TimeSeries::TimeSeries(): frames have different X size in folder \"%s\"", root_dir).c_str());

        if( frames[k]->getDIM_V() != frames[k+1]->getDIM_V())
            throw IOException(strprintf("in TimeSeries::TimeSeries(): frames have different Y size in folder \"%s\"", root_dir).c_str());

        if( frames[k]->getDIM_D() != frames[k+1]->getDIM_D())
            throw IOException(strprintf("in TimeSeries::TimeSeries(): frames have different Z size in folder \"%s\"", root_dir).c_str());
...
void TimeSeries::setActiveChannels ( iim::uint32 *_active, int _n_active )
{
    ...
    // check precondition #1: valid time series
    if(frames.empty())
        throw IOException("in TimeSeries::setActiveChannels(): empty time series");
...
uint8 * TimeSeries::loadSubvolume_to_UINT8(int V0,int V1, int H0, int H1, int D0, int D1, int *channels /*=0*/, int ret_type /*=iim::DEF_IMG_DEPTH*/) throw (IOException)
{
    ...
    if(V1-V0 <=0 || H1-H0 <= 0 || D1-D0 <= 0)
        throw IOException("in TimeSeries::loadSubvolume_to_UINT8: invalid subvolume intervals");

    // check for valid time frames selection
    if(frames.empty())
        throw IOException("in TimeSeries::loadSubvolume_to_UINT8(): empty time series");
    ...
    for(int k=0; k<frames.size()-1; k++)
    {
        ...
        if(!channels_k || !channels_k1)
            throw IOException("in TimeSeries::loadSubvolume_to_UINT8(): frames have invalid channel selections");
    ...
    /**/iim::debug(iim::LEV3, strprintf("allocating memory for X(%d) x Y(%d) x Z(%d) x C(%d) x T(%d)", H1-H0, V1-V0, D1-D0, frames[0]->getNACtiveChannels(), t1-t0+1).c_str(), __iim__current__function__);

    // try to allocate memory
    try{          subvol_data = new uint8[subvol_size];   }
    ...
    if(iim::ADD_NOISE_TO_TIME_SERIES)
    {
        for(int t=0; t<=t1-t0; t++)
        {
            ...
            for(size_t n=0; n<subvol_frame_size; n++)
                subvol_data[t_stride + n] = static_cast<uint8>((1-w)*subvol_data[t_stride + n] + w*(rand()%256) +0.5f);
        }
    }
