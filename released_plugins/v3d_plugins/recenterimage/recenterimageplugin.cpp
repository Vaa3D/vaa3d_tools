...
void ReCenterImagePlugin::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    if (menu_name == tr("ReCenterImage"))
    {
        ...
        bool ok;
        V3DLONG ndimx = QInputDialog::getInt(parent, tr("Dimension x"),
             tr("Enter dimx:"),
             N, 0, 10000, 1, &ok);

        V3DLONG ndimy = QInputDialog::getInt(parent, tr("Dimension y"),
             tr("Enter dimy:"),
             M, 0, 10000, 1, &ok);

        V3DLONG ndimz = QInputDialog::getInt(parent, tr("Dimension z"),
             tr("Enter dimz:"),
             P, 0, 10000, 1, &ok);
