...

{
    ...
    FILE *fp2=fopen(csv_file.toLatin1(),"w");
    if (!fp2)
        return false;
    if (0) //eswc_flag)
...
//            }
    ...
    out<<"RAWIMG="<<input_image_name<<Qt::endl;
    out<<"SWCFILE="<<input_swc_name<<Qt::endl;
    out<<"MASKIMG="<<fname_output<<Qt::endl;
    out<<"MARKER="<<marker_name<<Qt::endl;
    out<<"CSV="<<csv_file<<Qt::endl;
    out<<"PARA_BG="<<bg_thr<<Qt::endl;
    out<<"PARA_MAXDIS="<<max_dis<<Qt::endl;
    out<<"PARA_CHANNEL="<<sel_channel<<Qt::endl;
    //which segment it is at now
    out<<"SEG_ID="<<seg_id<<Qt::endl; //for by spine view seg_id=-1
    //which marker it is at
    out<<"MAKRER_ID="<<marker_id<<Qt::endl;
