...
bool manual_correct_dialog::load_swc()
{
    eswc_flag=false;
    QString filename;
    filename = QFileDialog::getOpenFileName(0, 0,swc_load_path,"Supported file (*.swc *.eswc)" ";;Neuron structure(*.swc *eswc)");
...
void manual_correct_dialog::write_svm_file(QString filename,vector<int> keep)
{
    ...
    FILE *fp2=fopen(outfile.toLatin1(),"wt");
...
//    label_group_copy=label_group;
    ...
    out<<"RAWIMG="<<img_complete<<Qt::endl;
    out<<"SWCFILE="<<swc_complete<<Qt::endl;
    qf_anofile.close();
