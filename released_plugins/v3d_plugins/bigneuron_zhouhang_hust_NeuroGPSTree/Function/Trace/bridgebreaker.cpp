...
bool BridgeBreaker::Update()
{
	...
	#ifdef _WIN32
	    CTreeCurvePointer tmpTree = std::tr1::dynamic_pointer_cast<const TreeCurve>(m_Tree);
	    CTreeConnectPointer tmpConnect = std::tr1::dynamic_pointer_cast<const TreeConnect>(m_Input);
	    CSVolumePointer tmpOrigImg = std::tr1::dynamic_pointer_cast<const SVolume>(m_OrigImg);
	#else
	...
	if( tmpSoma->size() < 2){
		printf("no break.\n");
#ifdef _WIN32
		std::tr1::shared_ptr<TreeConnect> tmp_Source = std::tr1::dynamic_pointer_cast<TreeConnect>(m_Source);
#else
	...
	std::shared_ptr<TreeConnect> tmp_Source = std::dynamic_pointer_cast<TreeConnect>(m_Source);
#endif
	/*FILE *fp = fopen("D:/break.txt","w");
    for(size_t kk = 0; kk < resultConnect.size();++kk){
        fprintf(fp, "%d %d\n",resultConnect[kk](0,0),resultConnect[kk](0,1));
    }
    fclose(fp);
	system("pause");*/
	...
	if(directionFlag == 0){
		...
		if(minDistInEndPtAndConDist < 5){
			if(0 == conjHeadTailFlag){
				...
				for(int k = 0; k <= idMax; ++k){
					vd3 += currentDendCurve[currentHeadIdx[k] ](4);//.2015-8-13
				}
				vd3 /= idMax + 1;
