...
bool profile_swc_menu(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    float dilate_ratio = QInputDialog::getDouble(parent, "dilate_ratio",
                                 "Enter dialate ratio:",
                                 3.0, 1.0, 100.0);
    int flip = QInputDialog::getInt(parent, "flip in y ?",
                                 "Flip in Y (0/1):",
                                 0, 0, 1);
    int invert = QInputDialog::getInt(parent, "invert intensity ? (for calculating tubularities, signals should be brighter than background",
