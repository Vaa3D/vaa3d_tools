...
bool proc_app1(V3DPluginCallback2 &callback, PARA_APP1 &p, const QString & versionStr)
{
    ...
    if (!p.p4dImage || !p.p4dImage->valid())
    {
        ...
        if(!p.inmarker_file.isEmpty())  //???????????????????p????û???¼?????marker???
...
//        cout << *it <<endl;
    ...
    vector<ImageMarker> inmarkers;
    {
        for(V3DLONG i = 0; i < p.landmarks.size(); i++)
        {
            ...
            {
                //add scaling by PHC 121127 ????????????????????²????????????ômarker????????????????????²???
                x /= dfactor_xy;
                y /= dfactor_xy;
                z /= dfactor_z;
            }
    ...
    trace_para.false_th=p.false_th;
    cout<<"trace_para.false_th="<<trace_para.false_th<<endl;
    ...
    trace_para.b_deformcurve = true; //true; //b_usedshortestpathonly;
    ...
    //NOW START THE RECONSTRUCTION
    {
        ...
        //soma detection if necessary
         ...
         v3d_msg(QString("xis %1,y is %2,z is %3").arg(p0.x).arg(p0.y).arg(p0.z));
           //p0.x = inmarkers[0].x;
           //p0.y = inmarkers[0].y;
           //p0.z = inmarkers[0].z;
            printf("\n*** data sz for actual tracing [ %ld, %ld, %ld]\n",p0.x, p0.y,p0.z);
    ...
    if (1)
    {
        ...
        if(!p.outswc_file.isEmpty())
            ...
            outswc_file = QString(p.p4dImage->getFileName()) + rootposstr + "_rayshootingTipAndApp1.swc"; //??????????????????? by lkr
