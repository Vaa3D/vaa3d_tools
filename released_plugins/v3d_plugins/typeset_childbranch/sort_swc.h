...
QMultiHash<V3DLONG, V3DLONG> getUniqueLUT(QList<NeuronSWC> &neurons, QMultiHash<V3DLONG, NeuronSWC> & LUT_newid_to_node)
{
...
//    QHash<V3DLONG, V3DLONG> LUT = getUniqueLUT(neurons, LUT_newid_to_node);
    QMultiHash<V3DLONG, NeuronSWC> LUT_newid_to_node;
    QMultiHash<V3DLONG, V3DLONG> LUT = getUniqueLUT(neurons, LUT_newid_to_node);

    qDebug()<<"LUT/n";
    auto keys=LUT.keys();
    std::sort(keys.begin(),keys.end());
    for (auto key:keys){
        qDebug()<<key<<":"<<LUT.value(key)<<Qt::endl;
...
//    QList<V3DLONG> idlist = ((QSet<V3DLONG>)LUT.values().toSet()).toList();
    ...
    for(int h=0;h<idlist.size();h++){
        qDebug()<<"the value of islist"<<idlist[h];
    }
...
bool export_list2file(QList<NeuronSWC> & lN, QString fileSaveName, QString fileOpenName)
{
    ...
    myfile<<"# generated by Vaa3D Plugin sort_neuron_swc"<<Qt::endl;
    myfile<<"# source file(s): "<<fileOpenName<<Qt::endl;
