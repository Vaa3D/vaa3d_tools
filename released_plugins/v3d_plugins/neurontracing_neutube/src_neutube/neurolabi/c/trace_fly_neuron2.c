...
#define STOP_TRACING(current_seed)			\
  ...
  for (i = 0; i < pa->size; i++) {
    ...
    */
    ...
    chain->locseg.seg.r1 = width;
    chain->locseg.seg.c = 0.0;

    printf("Search orientation ...\n");
    Local_Neuroseg_Orientation_Search(&(chain->locseg), signal, z_scale, NULL); 
