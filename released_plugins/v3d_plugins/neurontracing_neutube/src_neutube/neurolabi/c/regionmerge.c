...
int Detect_Threshold(int **m, int pn){
  int min_thre = 100;
    ...
    char *stack_name, int size_thre, const char *tm_file)
{
  ...
  if (size_thre == -1) {
    size_thre = Detect_Threshold(neighbors_count, objects_n);
  }
...
{
  static char *Spec[] = {"<input:string> [-r <string>] [-m <string>]", NULL};
  ...
  sprintf(filepath, "/Volumes/myerslab/zhaot/DIADEM_NM/paint/%03d.tif", filenum);
  Write_Stack(filepath, stack);
