      ...
      for (k = 1; k <= m; k++) {
        ...
        for (i = 1; i <= 3; i++) {
          ...
          if (evals[m-i+1] > 0.0005)   /* Guard against zero eigenvalue */
             symmat2[j][i] /=(float) sqrt(evals[m-i+1]);   /* Rescale */
          else
...
float *vector(int n){
   float *v;
   //SP change the memory allocation to n+1
    v = (float *) malloc ((unsigned) (n+1)*sizeof(float));
...
float **matrix(int n,int m)
{
    ...
    //SP explicit allocation of rows and column indices to n+1 and m+1 respectively, because the code starts to reference array index from i = 1, instead of i = 0.
    ...
    for (i = 1; i < n+1; i++)
        {
        mat[i] = (float *) malloc((unsigned) (m+1)*sizeof(float));
