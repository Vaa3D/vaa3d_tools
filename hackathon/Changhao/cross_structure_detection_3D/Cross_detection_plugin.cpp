...
void Cross_detection_Plugin::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    else if (menu_name == tr("detect_crossPoints_3D"))
    {
        int flag = detect_crossPoints_3D(callback,parent);
        if(flag)
        {
            cout<<endl;
            cout<<"*********************************"<<endl;
            cout<<"*detect_crossPoints_3D finished!*"<<endl;
            cout<<"*********************************"<<endl;
        }
        else
            return;
    ...
    else if (menu_name == tr("test"))
    {
        ...
        if(flag)
        {
            cout<<endl;
            cout<<"*********************************"<<endl;
            cout<<"*test show_find_radius!*"<<endl;
            cout<<"*********************************"<<endl;
        }
        else
            return;
...
bool Cross_detection_Plugin::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if(func_name==tr("detect_crossPoints_2_5D"))
    {
        return detect_crossPoints_2_5D(callback,input,output,parent);
    }
...
int detect_crossPoints_2D(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    seek_2D_candidate_points(nx,ny,image_binary,X_loc,Y_loc,count); cout<<"skeletonization candidate size:"<<count<<endl;
    if(count>0)
    {
...
//            cout<<"x is :"<<adjusted_x<<" "<<"y is :"<<adjusted_y<<" "<<"the radius  is :"<<radius<<endl;
    ...
    v3d_msg(QString("the all candidate points have detected. size:%1 ").arg(X_candidate.size()),0);
...
int detect_crossPoints_3D(V3DPluginCallback2 &callback, QWidget *parent)
{
    /* define the color */
    RGBA8 red; red.r=255; red.g=0; red.b=0;
    RGBA8 green; green.r=0; green.g=51; green.b=0;
    ...
    int window_size=pp.window_size;
    int ray_numbers_2d=pp.ray_numbers_2d;
    V3DLONG radiu_block0=pp.based_distance;
    int num_layer=pp.num_layer;
    double soma_radius=20;
    ...
    v3dhandle curwin = pp.curwin;
    unsigned char* data1d=0;
    data1d=pp.data1d;

    V3DLONG min_img_length = MIN(nx, MIN(ny, nz));
    V3DLONG radiu_block=MIN(radiu_block0, min_img_length/2); //the size of the block
    V3DLONG length_block=2*radiu_block+1;
    V3DLONG size_block=length_block*length_block*length_block;
    unsigned char *block;
    try{block=new unsigned char [size_block];}
    catch(...) {v3d_msg("cannot allocate memory for block."); return 0;}
    ...
    /* create the 3D ray matrix */
    vector<vector<float> > ray_x(16*32,vector<float>(100)), ray_y(16*32,vector<float>(100)),ray_z(16*32,vector<float>(100));
    //float x_dis[1024][128], y_dis[1024][128] , z_dis[1024][128];

    unsigned int axis_ang = 8; unsigned int plane_ang = 2*axis_ang;
    for(int len = 0; len < radiu_block; len++)
    {
        int mm = 0;
        for(int n = 1; n <= axis_ang; n++ )
        {
            for(int k = 0; k <= plane_ang-1; k++ )
            {
             ray_x[mm][len] = len * sin(PI * n/axis_ang) * cos(2*PI * k / plane_ang);
             ray_y[mm][len] = len * sin(PI * n/axis_ang) * sin(2*PI * k / plane_ang);
             ray_z[mm][len] = len * cos(PI * n/axis_ang);
             mm++;
            }
        }
    }
    cout<<"create 3D_ray success"<<endl;
    double DBradius = radiu_block*2*sin(PI/16/2);

    LandmarkList sample_point, candidate_point, result_point;
    sample_point.clear(); candidate_point.clear(); result_point.clear();
    LocationSimple s;

    getCalcuMarker(callback.getLandmark(curwin), candidate_point);
    if(!candidate_point.size()){v3d_msg("No candidate points input!");}

    ClusterAnalysis mycluster; mycluster.Init(3);

    for(V3DLONG t = 0; t < candidate_point.size(); t++)
    {
        sample_point.clear();
        V3DLONG candiPz = candidate_point.at(t).z; V3DLONG candiPy = candidate_point.at(t).y; V3DLONG candiPx = candidate_point.at(t).x;
//        candiPz = qBound(radiu_block, candiPz, nz-radiu_block);  candiPy = qBound(radiu_block, candiPy, ny-radiu_block);  candiPx = qBound(radiu_block, candiPx, nx-radiu_block);
        if(candiPz<0&&candiPz>=nz&&candiPy<0&&candiPy>=ny&&candiPx<0&&candiPx>=nx)
        {v3d_msg(QString("This candidate_point is out of img bound! Skip this point.")); continue;}
//        V3DLONG num_block = 0;
//        for(V3DLONG c = candiPz-radiu_block; c <= candiPz+radiu_block; c++)
//        {
//             V3DLONG z_location = c*nx*ny;
//             for(V3DLONG b = candiPy-radiu_block; b <= candiPy+radiu_block; b++)
//             {
//                 V3DLONG y_location=b*nx;
//                 for(V3DLONG a = candiPx-radiu_block; a <= candiPx+radiu_block; a++)
//                 {
//                    if(c>=0&&c<nz&&b>=0&&b<ny&&a>=0&&a<nx)
//                    {
//                        block[num_block] = data1d_copy[z_location+y_location+a];
//                        num_block++;
//                    }
//                    else
//                    {
...
//             }
//        }
//        v3d_msg(QString("block%1 have created.").arg(t));
        ...
        {
            for(int j=1; j < radiu_block; j+=1)
            {
                V3DLONG sample_loc_x = candiPx+ray_x[i][j]; V3DLONG sample_loc_y = candiPy+ray_y[i][j]; V3DLONG sample_loc_z = candiPz+ray_z[i][j];
                double pixel = p4DImage->getValueUINT8(sample_loc_x, sample_loc_y, sample_loc_z, 0); //cout<<"pixel:"<<pixel<<"  ";
                if(pixel>thres_2d)
                {
                    x_loc.push_back(sample_loc_x);
                    y_loc.push_back(sample_loc_y);
                    z_loc.push_back(sample_loc_z);
                    Pixe.push_back(pixel);
                    LocationSimple sample_marker;
                    sample_marker.x = sample_loc_x; sample_marker.y = sample_loc_y; sample_marker.z = sample_loc_z;
                    sample_marker.color = green;
                    sample_point.push_back(sample_marker);
//                    result_point.push_back(sample_marker);
                }
//                else
//                {
//                    LocationSimple sample_marker;
//                    sample_marker.x = sample_loc_x; sample_marker.y = sample_loc_y; sample_marker.z = sample_loc_z;
//                    sample_marker.color = red;
//                    result_point.push_back(sample_marker);
//                }
            }
        }
        v3d_msg(QString("sample curlist size: %1").arg(sample_point.size()),0);


        int flag_cluster = 0;
        if(mycluster.Read_from_coordiante(x_loc, y_loc, z_loc, DBradius, 3))
        {
            flag_cluster = mycluster.DoDBSCANRecursive();
            cout<<"flag_cluster:"<<flag_cluster<<endl;
            if(flag_cluster>3)
            {
                LocationSimple tmpmarker; tmpmarker = candidate_point[t];
                result_point.push_back(tmpmarker);
            }
        }
    }

    Image4DSimple * new4DImage = new Image4DSimple();
//        new4DImage->setData((unsigned char *)block, length_block, length_block, length_block, nc, p4DImage->getDatatype());
    new4DImage->setData((unsigned char *)data1d_copy, nx, ny, nz, nc, p4DImage->getDatatype());
    v3dhandle newwin = callback.newImageWindow();
    callback.setImage(newwin, new4DImage);
    callback.updateImageWindow(newwin);
    displayMarker(callback, newwin, result_point);
...
bool detect_crossPoints_2_5D(V3DPluginCallback2 &callback,const V3DPluginArgList &input,V3DPluginArgList &output,QWidget *parent)
{
    /* define the color */
    RGBA8 red; red.r=255; red.g=0; red.b=0;
    RGBA8 green; green.r=0; green.g=51; green.b=0;
