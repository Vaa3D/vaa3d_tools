...
{
    //callback=&callback1;
    curwin=callback1.currentImageWindow();
    if(!curwin)
    {
...
void Branch_Detection_Model::Gui_input_CrossPoint_3D_Method()
{
    ...
    if(p4DImage->getZDim() > 1)
          dialog->setWindowTitle("3D neuron image cross point detection algorithm");
    else
          dialog->setWindowTitle("2D neuron image cross point detection algorithm");
    QGridLayout * layout = new QGridLayout();
