...
NeuronTree LinkPoints(V3DPluginCallback2 &callback, unsigned char ****p4d, V3DLONG sz[4],
...
CurveTracePara & trace_para, double trace_z_thickness)
{
    ...
    //
    tracedNeuron = trace_one_pt_to_N_points_shortestdist_G(callback, p4d, sz, p0, pp, trace_para, trace_z_thickness, mmUnit);
    //

    if (1) //trace to some selected markers
    {
...
V_NeuronSWC_list trace_one_pt_to_N_points_shortestdist_G(V3DPluginCallback2 &callback, unsigned char ****p4d, V3DLONG sz[4],
                                                             LocationSimple & p0, vector<LocationSimple> & pp,
                                                             CurveTracePara & trace_para, double trace_z_thickness,
                                                             vector< vector<V_NeuronSWC_unit> > & mmUnit)
{
    ...
    if (sz[3]==1)
    {
        if(1)
        {
            s_error = find_shortest_path_graphimg_G(callback, p4d[chano], sz[0], sz[1], sz[2],
                                              trace_z_thickness,
                                              trace_bounding_box.x0, trace_bounding_box.y0, trace_bounding_box.z0,
                                              trace_bounding_box.x1, trace_bounding_box.y1, trace_bounding_box.z1,
                                              p0.x, p0.y, p0.z,
                                              n_end_nodes,
                                              pp, //fix this bug 100624
                                              mmUnit,
                                              sp_para);
