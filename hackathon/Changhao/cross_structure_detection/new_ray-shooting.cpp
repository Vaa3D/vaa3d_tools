...
template<class T> double markerRadius_hanchuan_XY(T* &inimg1d, V3DLONG nx, V3DLONG ny, V3DLONG x,V3DLONG y, double thresh)
{
    ...
    for (ir=1; ir<=max_r; ir=ir+0.5)
    {
        ...
        for (dz= zlower; dz <= zupper; ++dz)
            for (dy= -ir; dy <= +ir; dy=dy+0.5)
                for (dx= -ir; dx <= +ir; dx=dx+0.5)
                {
                    total_num++;
    ...
    for(V3DLONG j=1;j<ny-1;j++)
    {
        for(V3DLONG i=1;i<nx-1;i++)
        {
            if(image_binary[j*nx+i]>0)
            {
                ...
                int num=abs(new_neighbor[1]-new_neighbor[8]);
                cnp=0.5*(sum+num);
                if(cnp==cnp_val)     //  if cnp==3 ,the (i,j) point is the 2D candidate point.
                {
                    x_loc.push_back(i);
                    y_loc.push_back(j);
                    count=count+1;
...
{
    valid_layer.clear();
    double plane_size = nx*ny;
    for(V3DLONG t = 0; t < nz; t++)
    {
        V3DLONG count = 0;
        for(V3DLONG i = 0; i <plane_size; i++)
        {
            V3DLONG offset = t*plane_size+i;
            if(data1d[offset]<thresh)
                count++;
        }
        double bkg_proportion = count/plane_size;cout<<t<<" portion:"<<bkg_proportion<<"  ";
        if(bkg_proportion>0.95)
        {   valid_layer.push_back(0);}
        else
        {   valid_layer.push_back(1);}
    }
