...
void Branch_Detection_Model::Gui_input_Rorate_3D_Method()
{
    if(p4DImage==NULL)
    {
      ...
      if(p4DImage->getZDim() > 1)
          ...
          number_layer_spinbox->setRange(1,(((sz[0]/2<sz[1]/2)?(sz[0]/2):(sz[1]/2))<sz[2]/2)?((sz[0]/2<sz[1]/2)?(sz[0]/2):(sz[1]/2)):(sz[2]/2));
...
void Branch_Detection_Model::Gui_input_CrossPoint_3D_Method()
{
    if(p4DImage==NULL)
    {
       v3d_msg("No image is open.");
      ...
      if(p4DImage->getZDim() > 1)
          ...
          QSpinBox * soma_radius_spinbox = new QSpinBox();
          soma_radius_spinbox->setRange(10,40);
          soma_radius_spinbox->setValue(soma_radius);
          ...
          layout->addWidget(new QLabel("soma_radius layer"),5,0);
          layout->addWidget(soma_radius_spinbox, 5,1,1,5);
          ...
          based_distance=based_distance_spinbox->value();
          num_layer=number_layer_spinbox->value();
          soma_radius=soma_radius_spinbox->value();
          if (dialog)
          {
              delete dialog;
              dialog=0;
