...
public:
	void Execute(const QString &menu_name, V3DPluginCallback &callback, QWidget *parent)
	{
	    ...
	    else if(channelToFilter==-1)	//process each channel seperatelly
	    {
	    	//allocate memory for 1 channel
	    	//import data
	    	//process data
	    	//copy data back
	    	//do it for another channel if required
			;
	    }
		else							//only process given channel
		{
			;
		}
		...
		if (menu_name == QObject::tr("ITK Bilateral Filter (slow) ..."))
		{
