...
do
	...
	do
		...
		echo "./tiffrotate $sbj_scale $rotate $sbj_img && ./v3d_stitch_pair -t $tgt_img -C $channel -s $sbj_img -c $channel -u $result_txt -n 0"
		./tiffrotate $sbj_scale $rotate $sbj_img && ./v3d_stitch_pair -t $tgt_img -C $channel -s $sbj_img -c $channel -u $result_txt -n 0 >> /dev/null 2>&1 &
		taskid=$!
		#echo $taskid
		tasks=("${tasks[@]}" "$taskid")
		#echo "tasks : ${tasks[@]} , task num :  ${#tasks[@]}"
