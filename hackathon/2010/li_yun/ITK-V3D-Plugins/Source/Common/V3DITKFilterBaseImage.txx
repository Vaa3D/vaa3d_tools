...
::TransferOutput( V3D_Image3DBasic & outputImage ) const
{
  ...
  unsigned char *psrc = reinterpret_cast< unsigned char * >( output1d );
  ...
  //Allocate space for the outputImage

  V3DLONG nchanbytes = size[0]*size[1]*size[2]*nunitbytes;

  try {
	outputImage.data1d = new unsigned char [nchanbytes];
  }
  catch (...)
  {
	v3d_msg(QString("Fail to allocate a buffer memory for output of the plugin in TransferOutput( V3D_Image3DBasic & outputImage ) const."));
	return ;
  }

  //now copy data

  for (V3DLONG i=0; i<nchanbytes; i++)
  	outputImage.data1d[i] = psrc[i];
