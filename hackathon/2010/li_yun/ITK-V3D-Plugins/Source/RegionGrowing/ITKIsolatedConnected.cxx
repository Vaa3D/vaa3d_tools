...
void Execute(V3DPluginCallback &callback, QWidget *parent)
{
	...
	{
		seedPosition1[0] = list_landmark_sub[0].x -1; // notice 0-based and 1-based difference
		seedPosition1[1] = list_landmark_sub[0].y -1;
		seedPosition1[2] = list_landmark_sub[0].z -1;		
		
		seedPosition2[0] = list_landmark_sub[1].x -1; // notice 0-based and 1-based difference
		seedPosition2[1] = list_landmark_sub[1].y -1;
		seedPosition2[2] = list_landmark_sub[1].z -1;		
...
virtual void SetupParameters()
{
	...
	dialog.AddDialogElement("UpperThreshold", 80.0, 0.0, 255.0); 
	dialog.AddDialogElement("ReplaceValue", 255.0, 0.0, 255.0); 

	if( dialog.exec() == QDialog::Accepted )
	{
	isolatedConnected->SetLower( dialog.GetValue("LowerThreshold") );
	isolatedConnected->SetUpper( dialog.GetValue("UpperThreshold") );
	isolatedConnected->SetReplaceValue( dialog.GetValue("ReplaceValue") );
	}
}			
...
private:
	...
	typename OutputImageType::IndexType  			seedPosition1; 	
	typename OutputImageType::IndexType  			seedPosition2;
