...
//        bsz = dialog.bsz;
        ...
        if(swcFilePath.toUpper().endsWith(".ASWC") || swcFilePath.toUpper().endsWith(".ESWC"))
        {
            NeuronTree nt;
            if(readASWC_file(swcFilePath, dataDirPath, nt)!=0)
            {
                cout<<"fail in loading input ASWC file"<<endl;
                return;
            }
...
//        DataFlow df(pc, string(dataDirPath.toUtf8().constData()), bsx, bsy, bsz);
...
//            cout<<"test "<<((unsigned short*)p)[0]<<endl;
            ...
            gs.b_autoSWCLineMode = true;
            callback.setGlobalSetting(gs);
...
//    pb_browse_img = new QPushButton("Browse...");
    ...
    // input swc
    label_swc = new QLabel(QObject::tr("Choose a neuron reconstruction (.aswc): "));
...
{
    fn_swc = QFileDialog::getOpenFileName(this, tr("Open a neuron reconstruction (.aswc)"),
                                          QDir::currentPath(),
                                          tr("ASWC (*.*)"));
