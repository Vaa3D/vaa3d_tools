 ...
 *  modified to c++ template by Yang Yu, on 12/12/2017
 */
...
template<class T>
QHash<T, T> ChildParent(QList<NeuronSWC> &neurons, const QList<T> & idlist, const QHash<T,T> & LUT)
{
    QHash<T, T> cp;
    for (T i=0;i<neurons.size(); i++)
    {
...
template<class T>
QHash<T, T> getUniqueLUT(QList<NeuronSWC> &neurons)
{
    QHash<T,T> LUT;
...
template<class T>
void DFS(bool** matrix, T* neworder, T node, T* id, T siz, int* numbered, int *group)
{
    if (!numbered[node]){
        numbered[node] = *group;
        neworder[*id] = node;
...
template<class T>
bool SortSWC(QList<NeuronSWC> & neurons, QList<NeuronSWC> & result, T newrootid, double thres)
{
    ...
    for (T i = 0;i<siz;i++)
	{
        ...
        for (T j=0;j<parentSet.size();j++)
		{
            T v2 = (V3DLONG) (parentSet.at(j));
            if (v2==-1) continue;
			matrix[i][v2] = true;
			matrix[v2][i] = true;
		}
    ...
    T* neworder = new V3DLONG[siz];
	int* numbered = new int[siz];
    for (T i=0;i<siz;i++) numbered[i] = 0;
	...
	while((*group)>1)
	{
		...
		int mingroup = 1;
        T m1,m2;
        for (T ii=0;ii<siz;ii++){
			if (numbered[ii]==1)
                for (T jj=0;jj<siz;jj++)
					if (numbered[jj]!=1)
					{
                        dist2 = computeDist2<T>(neurons.at(idlist.at(ii)),neurons.at(idlist.at(jj)));
						if (dist2<min)
						{
							min = dist2;
	...
	*group = 1;
	...
	while (*id<siz)
	{
        ...
        for (T ii=offset+1;ii<(*id);ii++)
		{
            for (T jj=offset;jj<ii;jj++) //after DFS the id of parent must be less than child's
			{
