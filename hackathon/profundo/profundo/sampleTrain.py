...
class filesListBrainMRLandmark(files):
    ...
    def _listLandmarks_all(self, suffix):
        # extend directory path
        current_dir = self.dir + suffix
        landmark_files = []
        for filename in self.files_list:
    ...
    def sample_circular(self, shuffle=False):
        ...
        while True:
            for idx in indexes:
                ...
                landmarks = [sitk_image.TransformPhysicalPointToContinuousIndex(point) for point in landmarks]
                # 0-2 RV insert points
                # 1 -> RV lateral wall turning point
                # 3 -> LV lateral wall mid-point
                # 4 -> apex
                # 5-> center of the mitral valve
                landmark = np.round(landmarks[4]).astype('int')
