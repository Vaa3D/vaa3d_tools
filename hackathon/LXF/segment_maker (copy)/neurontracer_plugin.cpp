...
//        resultTree_rebase.hashNeuron.clear();
        ...
        next_rebase = next_marker(callback,curr_window_nt);
        v3d_msg("check_1");
        LandmarkList marker_show;
        for(V3DLONG i=0;i<marker_rebase2.size();i++)
        {
            marker_show.push_back(marker_rebase2[i]);
        }
        marker_show.push_back(next_rebase);
        v3d_msg("check_2");
        callback.setLandmarkTeraFly(marker_show);
