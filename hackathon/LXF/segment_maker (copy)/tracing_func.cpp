...
extern LocationSimple next_rebase;
...
//        }
        ...
        double judge =  (next_rebase.x-t.x)*(next_rebase.x-t.x)+(next_rebase.y-t.y)*(next_rebase.y-t.y)+(next_rebase.z-t.z)*(next_rebase.z-t.z);
        if(judge<0.0001) //the same marker
        {
            change = false;
            bool miok;
        ...
        v3d_msg("show_marker");
        //callback.setLandmarkTeraFly(marker_rebase);
        LandmarkList marker_mark;

        for(V3DLONG i=0;i<terafly_landmarks_terafly.size();i++)
        {
            double dis_match_point = (terafly_landmarks_terafly[i].x-t.x)*(terafly_landmarks_terafly[i].x-t.x)+(terafly_landmarks_terafly[i].y-t.y)*(terafly_landmarks_terafly[i].y-t.y)+(terafly_landmarks_terafly[i].z-t.z)*(terafly_landmarks_terafly[i].z-t.z);
            if(dis_match_point>0.001)
            {
                marker_mark.push_back(terafly_landmarks_terafly[i]);
            }
        }


        marker_rebase2 = marker_mark;
