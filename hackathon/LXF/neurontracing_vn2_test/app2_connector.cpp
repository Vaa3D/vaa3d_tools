...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
    ...
    if(1)
    {
        ...
        vector<MyMarker*> tmpoutput2;

        for(V3DLONG i=0;i<outswc.size();i++)
        {
            int ind=0;
            for(V3DLONG j=0;j<outswc.size();j++)
            {
                if(outswc[i]->parent==outswc[j]->parent)
                {
                    ind++;
                }
            }
            if(ind==1)
            {
                //cout<<"LXFLXFLXFLXF"<<endl;
                tmpoutput2.push_back(outswc[i]);
            }

        }
        cout<<"outswc = "<<outswc.size()<<endl;
        cout<<"tmpoutput2 = "<<tmpoutput2.size()<<endl;
              v3d_msg("check2");
              QString outswc_file2 = "test_app2.swc";
        saveSWC_file(outswc_file2.toStdString(), tmpoutput2, infostring);
