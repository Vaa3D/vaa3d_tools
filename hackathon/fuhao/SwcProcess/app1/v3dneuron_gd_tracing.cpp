															 ...
															 vector< vector<V_NeuronSWC_unit> > & mmUnit) 
{
	...
	if (trace_bounding_box == NULL_BoundingBox)
	{
        ...
        {
            //
            trace_bounding_box.x0 = MAX(MIN(p0.x,px.at(0))-20,0);
            trace_bounding_box.y0 = MAX(MIN(p0.y,py.at(0))-20,0);
            trace_bounding_box.z0 = MAX(MIN(p0.z,pz.at(0))-20,0);
            trace_bounding_box.x1 = MIN(MAX(p0.x,px.at(0))+20,sz[0]-1);
            trace_bounding_box.y1 = MIN(MAX(p0.y,py.at(0))+20,sz[0]-1);
            trace_bounding_box.z1 = MIN(MAX(p0.z,pz.at(0))+20,sz[0]-1);
