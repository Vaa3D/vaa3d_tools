...
sed -i '/^$/d' total_result.txt #delete space
tail -n +2 total_result.txt |awk '{sum2+=$2;sum3+=$3;sum4+=$4;sum5+=$5;sum6+=$6;count++}END{print sum2/count,sum3/count,sum4/count,sum5/count,sum6/count}' >> total_result.txt #tail for printf line begining at line 2
