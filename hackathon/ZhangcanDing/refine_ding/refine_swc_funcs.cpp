...
NeuronTree smoothSWCTerafly(V3DPluginCallback2 &callback,QString fname_img, NeuronTree nt, QList<ImageMarker>& break_points,QList<CellAPO> apo_list,int half_length)
{

    std::cout<<"now start to smooth the swc"<<std::endl;
    ...
    for(int i=0; i<break_points.size();i++)
    {
        ...
        {
            if(NTDIS(break_points.at(i),list.at(j))<0.5 && list.at(j).level != 180)
            {
                index = j;
...

        {
            ...
            QString crop_img= QString("%1_crop.tif").arg(QString::number(list.at(index_begin).x));
            simple_saveimage_wrapper(callback,crop_img.toStdString().c_str(), total1dData,mysz,1);
            ...
            CellAPO start_mark;
            start_mark.x=list.at(index_begin).x;
            start_mark.y=list.at(index_begin).y;
            start_mark.z=list.at(index_begin).z;
            start_mark.volsize=50;
            start_mark.color.r=0;
            start_mark.color.g=0;
            start_mark.color.b=255;
            apo_list.push_back(start_mark);

            LocationSimple p0;
            p0.x = list.at(index_begin).x - start_x; //
            p0.y = list.at(index_begin).y - start_y/*+((list.at(index_end).y-list.at(index_begin).y)/4)*/;
            p0.z = list.at(index_begin).z - start_z/*+((list.at(index_end).z-list.at(index_begin).z)/4)*/;
            ImageMarker ind0;
            ind0.x=p0.x;  ind0.y=p0.y; ind0.z=p0.z; ind0.type=0; ind0.shape=1;


            vector<LocationSimple> pp;
            LocationSimple pEnd;
            pEnd.x = list.at(index_end).x - start_x/*-((list.at(index_end).x-list.at(index_begin).x)/4)*/;
            pEnd.y = list.at(index_end).y - start_y/*-((list.at(index_end).y-list.at(index_begin).y)/4)*/;
            pEnd.z = list.at(index_end).z - start_z/*-((list.at(index_end).z-list.at(index_begin).z)/4)*/;
            pp.push_back(pEnd);
            ImageMarker indend;
            indend.x=pEnd.x;  indend.y=pEnd.y; indend.z=pEnd.z; indend.type=0; indend.shape=1;

            QList<ImageMarker> Plist;
            Plist.push_back(ind0);Plist.push_back(indend);
            QString crop_marker= QString("%1_crop.marker").arg(QString::number(list.at(index_begin).x));
            writeMarker_file(crop_marker,Plist); //added by ding



//            unsigned char * total1dData = 0;
//            total1dData = callback.getSubVolumeTeraFly(fname_img.toStdString(),start_x,end_x+1,start_y,end_y+1,start_z+(list.at(index_begin).z - start_z)/2,end_z+1+(list.at(index_begin).z - start_z)/2);
//            V3DLONG mysz1[4];
//            mysz1[0] = end_x-start_x+1;
//            mysz1[1] = end_y-start_y+1;
//            mysz1[2] = end_z-start_z+1;
//            mysz1[3] = 1;
            ...
            //reconstruction of new crop
            QString crop_eswc= QString("%1_crop.eswc").arg(QString::number(list.at(index_begin).x));
            writeESWC_file(crop_eswc,nt_gd);
            //previous reconstruction
            QString pre_eswc=QString("%1_pre.eswc").arg(QString::number(list.at(index_begin).x));
            NeuronTree tmp_nt;
            for(int k=0; k<list.size();k++){
                if(list.at(k).x<=list.at(index_end).x && list.at(k).x >=list.at(index_begin).x && list.at(k).y<=list.at(index_end).y && list.at(k).y >=list.at(index_begin).y && list.at(k).z<=list.at(index_end).z && list.at(k).z >=list.at(index_begin).z )
                {
                 NeuronSWC tmp_nr;
                 tmp_nr.n=list.at(k).n-addon_index;
                 tmp_nr.x= list.at(k).x-start_x;
                 tmp_nr.y= list.at(k).y-start_y;
                 tmp_nr.z= list.at(k).z-start_z;
                 tmp_nr.r= list.at(k).r;
                 tmp_nr.type=7;
                 tmp_nr.pn=(list.at(k).pn == -1)?  list.at(k).pn : list.at(k).pn - addon_index;
                 tmp_nt.listNeuron.append(tmp_nr);
            ...
            writeESWC_file(pre_eswc,tmp_nt);

            //intermediate results output end
...
QList <ImageMarker> break_points_reselect(QList<ImageMarker> & break_points, NeuronTree nt )
{
    ...
    NeuronTree pre_list_nt=neuronlist_2_neurontree(pre_list);
    QString pre_pre_list;
    pre_pre_list="/home/penglab/PBserver/tmp/Ding/17302_001/pre_breaking_points.eswc";
    writeESWC_file(pre_pre_list,pre_list_nt);
    ...
    NeuronTree tip_list_nt=neuronlist_2_neurontree(tmp_list);
    QString tip_list_file;
    tip_list_file="/home/penglab/PBserver/tmp/Ding/17302_001/tip_branch_points.eswc";
    writeESWC_file(tip_list_file,tip_list_nt);



   // for each node in breaking points list, find the nearest parent node and output the mid node
...
//            while(tmp_pn != pre_list.at(i).n)
//            {
//                int index=pre_list.at(j).n;
//                int tmp_pn=getParent(index+cnt,nt);
//                cnt++;
//            }


            for(int count=0;count<50;count++)
                {   int index=pre_list.at(j-count).n;
                    int tmp_pn=getParent(index,nt);
                    if(pre_list.at(i).n ==tmp_pn)
                    {
                        int midcount=count/2;
                        tmp_list.push_back(list.at(j-midcount));
                    } break;
    ...
    NeuronTree break_list_nt=neuronlist_2_neurontree(tmp_list);
    QString break_new_list_file;
    break_new_list_file="/home/penglab/PBserver/tmp/Ding/17302_001/new_breaking_points.eswc";
    writeESWC_file(break_new_list_file,break_list_nt);
