...
bool refine_swc::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    if(func_name==tr("smooth"))
    {
        if(infiles.size()<3)
        {
            fprintf (stderr, "Need input terafly image path, ini_swc and refined swc. \n");
            return false;
        }
        ...
        NeuronTree nt2_refined = readSWC_file(QString(infiles[2]));
        NeuronTree nt2_sorted = SortSWC_pipeline(nt2_refined.listNeuron,VOID, 0);
        NeuronTree nt2_smoothed = smoothSWCTerafly(callback,infiles[0],nt2_sorted,break_points,apo_list,half_length);
        NeuronTree nt2_smoothed_sorted = SortSWC_pipeline(nt2_smoothed.listNeuron,VOID, 0);

        writeAPO_file(QString(outfiles[1]),apo_list);
    ...
    else if (func_name == tr("refine"))
	{
        ...
        //output the first break points to apo and markerfile. by Ding
        QString markerfile;
        ...
        //added by Ding, run the 2nd refine according to the new break points and save to apo and markerfile
