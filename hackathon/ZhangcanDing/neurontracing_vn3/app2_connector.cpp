...
bool proc_app2(V3DPluginCallback2 &callback, PARA_APP2 &p, const QString & versionStr)
{
  //  bool b_menu = true;
    ...
    if (p.bkg_thresh < 0)
    {
        ...
        V3DLONG sz[3];
        sz[0]=p4dImageNew->getXDim();
        sz[1]=p4dImageNew->getYDim();
        sz[2]=p4dImageNew->getZDim();
        sz[3]=p4dImageNew->getCDim();
        adaptiveLocalTheshold(p4dImageNew->getRawDataAtChannel(0),mask, sz);
        QString maskfn="/home/penglab/Ding/adaptiveLocalThesh/1mask.v3draw";

        simple_saveimage_wrapper(callback,maskfn.toStdString().c_str(),mask,sz,1);
        v3d_msg("check",1);
    ...
    else if(inmarkers.size() == 1)
    {
        ...
        if(p.is_gsdt)
        {
            if(phi == 0)
            {
                ...
                {
                    {case V3D_UINT8:
                        unsigned char *fixmask=0;
                        fastmarching_dt(indata1d, phi,fixmask, in_sz[0], in_sz[1], in_sz[2], p.cnn_type, p.bkg_thresh);
                        QString fixedmaskfn="/home/penglab/Ding/adaptiveLocalThesh/fixThreshmask.v3draw";
                        simple_saveimage_wrapper(callback,fixedmaskfn.toStdString().c_str(),fixmask,in_sz,1);
                        if(fixmask){delete  [] fixmask;fixmask=0;};
                        break;}
                    case V3D_UINT16:  //this is no longer needed, as the data type has been converted above
