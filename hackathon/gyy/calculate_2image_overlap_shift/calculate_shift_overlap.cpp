...
bool calculate_2Img_overlap_shift(unsigned short * pdata1, unsigned short * pdata2, V3DLONG in_sz1[4], unsigned short * &p_cut_data1, unsigned short * &p_cut_data2, QString orientation, int & XY_overlap, int & XY_shift);
void cut_planes_edge(unsigned short *pdata, unsigned short *&p_cut_data, long long in_sz1[]);
...
bool calculate_2image_overlap_shift(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    if(!calculate_2Img_overlap_shift(data1d_copy1, data1d_copy2, in_sz1, p_cut_data1, p_cut_data2, orientation1, XY_overlap, XY_shift))
    {
...
bool calculate_2Img_overlap_shift(unsigned short * pdata1, unsigned short * pdata2, V3DLONG in_sz1[4], unsigned short * &p_cut_data1, unsigned short * &p_cut_data2, QString orientation, int & XY_overlap, int & XY_shift)
{
    cut_planes_edge(pdata1, p_cut_data1, in_sz1);
    cut_planes_edge(pdata2, p_cut_data2, in_sz1);
    ...
    if(orientation == "left" || orientation == "right")
    {
        for(int ss1 = 10; ss1 < 10+overlap_range; ss1 ++)
        {
            for(int ss2 = 0; ss2 < shift_range; ss2 ++)
            {
                ...
                for(int row = 0; row < (in_sz1[1]-40)-ss2; row ++)
                {
                    for(int col = 0; col < ss1; col ++)
                    {
                        int diff;
                        if(orientation == "left")
                            diff = p_cut_data1[row*(in_sz1[0]-40)+(in_sz1[1]-40)-ss1+col]-p_cut_data2[(row+ss2)*(in_sz1[0]-40)+col];
                        else
                            diff = p_cut_data2[row*(in_sz1[0]-40)+(in_sz1[1]-40)-ss1+col]-p_cut_data1[(row+ss2)*(in_sz1[0]-40)+col];
    ...
    else if(orientation == "up" || orientation == "down")
    {
        for(int ss1 = 10; ss1 < 10+overlap_range; ss1 ++)
        {
            for(int ss2 = 0; ss2 < shift_range; ss2 ++)
            {
                ...
                {
                    for(int col = 0; col < (in_sz1[0]-40)-ss2; col ++)
                    {
                        int diff = p_cut_data1[((in_sz1[1]-40)-ss1+row)*(in_sz1[0]-40)+ss2+col]-p_cut_data2[row*(in_sz1[0]-40)+col];
                        sum[indexOL] += diff * diff;
                        count[indexOL] += 1;
                    }
                }
    ...
    cout<<"diff_ps = "<<diff_ps<< endl;
    return 1;
}

void cut_planes_edge(unsigned short *pdata, unsigned short * &p_cut_data, V3DLONG in_sz1[4])
{
    cout<<"cut planes edge!"<<endl;
    if(p_cut_data){delete []p_cut_data; p_cut_data = 0;}
    cout<<__LINE__<<endl;
    p_cut_data = new unsigned short[(in_sz1[0]-40) * (in_sz1[1]-40)];
    cout<<__LINE__<<endl;
    for(int i = 0; i < (in_sz1[1]-40); i ++)
        for(int j = 0; j < (in_sz1[0]-40); j ++)
        {
            p_cut_data[i*(in_sz1[0]-40)+j] = pdata[(i+20)*in_sz1[0]+(20+j)];
        }
}
...
//    cut_planes_edge(pdata2, p_cut_data2, in_sz1);
    cout<<"calculate 2Img overlap shift!"<<endl;
    const int overlap_range = 20, shift_range = 10;
    int overlap = 90;
    double sum[overlap_range * shift_range] = {0};
    int difference[overlap_range * shift_range] = {0};
    int count[overlap_range * shift_range] = {0};
    ...
    if(orientation == "left" || orientation == "right")
    {
        for(int ss1 = overlap; ss1 < overlap+overlap_range; ss1 ++)
        {
            for(int ss2 = 0; ss2 < shift_range; ss2 ++)
            {
                int indexOL = shift_range*(ss1-overlap)+ss2;
                for(int row = 0; row < in_sz1[1]-ss2; row ++)
                {
                    for(int col = 0; col < ss1; col ++)
                    {
                        int diff;
                        if(orientation == "left")
                            diff = pdata1[row*(in_sz1[0])+(in_sz1[1])-ss1+col]-pdata2[(row+ss2)*(in_sz1[0])+col];
                        else
                            diff = pdata2[row*(in_sz1[0])+(in_sz1[1])-ss1+col]-pdata1[(row+ss2)*(in_sz1[0])+col];
                        sum[indexOL] += diff * diff;
                        count[indexOL] += 1;
                    }
                }
            ...
            for(int ss2 = 0; ss2 < shift_range; ss2 ++)
            {
                ...
                for(int row = 0; row < in_sz1[1]-ss2; row ++)
                {
                    for(int col = 0; col < ss1; col ++)
                    {
                        int diff;
                        if(orientation == "left")
                            diff = pdata1[(row+ss2)*(in_sz1[0])+(in_sz1[1])-ss1+col]-pdata2[row*(in_sz1[0])+col];
                        else
                            diff = pdata2[(row+ss2)*(in_sz1[0])+(in_sz1[1])-ss1+col]-pdata1[row*(in_sz1[0])+col];
                        sum[indexOL] += diff * diff;
                        count[indexOL] += 1;
                    }
                }
    ...
    else if(orientation == "up" || orientation == "down")
    {
        ...
        for(int ss1 = overlap; ss1 < overlap+overlap_range; ss1 ++)
        {
            for(int ss2 = 0; ss2 < shift_range; ss2 ++)
            {
                ...
                for(int row = 0; row < ss1; row ++)
                {
                    for(int col = 0; col < (in_sz1[0])-ss2; col ++)
                    {
                        int diff = pdata1[((in_sz1[1])-ss1+row)*(in_sz1[0])+ss2+col]-pdata2[row*(in_sz1[0])+col];
                        sum[indexOL] += diff * diff;
                        count[indexOL] += 1;
                    }
                }
