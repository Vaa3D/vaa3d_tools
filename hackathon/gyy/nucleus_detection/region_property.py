...
path_image = r'F:\data\test\022770_048010_009600.tif_cut.tif'
path_markers = r'F:\data\test\022770_048010_009600.tif_cut.tif.marker'
save_path = r'F:\data\test'
regionPath = save_path + '/' + 'Region.txt'
regionTitle = '##Name, Intensity, Area, Major, Minor, Ave\n'
...
for j in output_:
    num += 1
    file_region.write(str(num) + ',' + str(IMG[points[num-1][0]][points[num-1][1]][points[num-1][2]]) + ',' + str(j.area) + ',' + str(round(j.major_axis_length, 2)) + ','
                      + str(round(j.minor_axis_length, 2)) + ',' + str(round((j.major_axis_length + j.minor_axis_length)/2, 2)) + '\n'
                      )
