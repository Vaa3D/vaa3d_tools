...
image_dir_dir = 'F:/data/testdata'
localMax_dir_dir = r'F:\data\comparing\18_2016_touching_soma\results_marker'
manual_dir_dir = 'F:/data/manual_testdata'
save_dir_dir = r'F:\data\comparing\18_2016_touching_soma\results'
image_dir_list = os.listdir(image_dir_dir)
recall_Path_all = save_dir_dir + '/recall_all.txt'
recallTitle = '##Name, Recall, Precision, Error_rate, F1, n_recall, n_wrong, len(points_m)\n'
file_recall_all = open(recall_Path_all, 'w')
file_recall_all.write(recallTitle)

for img_dir_i in image_dir_list:
    image_dir = image_dir_dir + '/' + img_dir_i
    localMax_dir = localMax_dir_dir + '/' + img_dir_i
    manual_dir = manual_dir_dir + '/' + img_dir_i
    img_list = os.listdir(image_dir)
    save_dir = save_dir_dir + '/' + img_dir_i
    isexist = os.path.exists(save_dir)
    if not isexist:
        os.mkdir(save_dir)

    recallPath = save_dir + '/' + img_dir_i + '.txt'
    # recallTitle = '##Name, Recall, Precision, F1, n_recall, n_wrong, len(points_m)\n'
    file_recall = open(recallPath, 'w')
    file_recall.write(recallTitle)
    for name in img_list:
        file_name = name.split('.')[0]
        path_image = image_dir + '/' + name
        ...
        tifffile.imwrite(save_dir + '/' + file_name + '_color.tif', seg_color)
        ...
        tifffile.imwrite(save_dir + '/' + file_name + '_seg_need_0.tif', seg___)
        tifffile.imwrite(save_dir + '/' + file_name + '_seg_touch_0.tif', seg_)
        ...
        tifffile.imwrite(save_dir + '/' + file_name + '_seg_need_seg_color.tif', color_seg_need)
        ...
        tifffile.imwrite(save_dir + '/' + file_name + '_seg_wrong_0.tif', seg__)
        ...
        print('path: ', img_dir_i + '/' + file_name)
        print('Recall = ', Recall)
        ...
        file_recall.write(img_dir_i + '/' + file_name + ',' + str(Recall) + ',' + str(Precision) +',' + str(Error_rate)+ ','+ str(F1)+','
                          + str(n_recall) + ',' + str(n_wrong) + ',' + str(len(points_m)) + '\n'
                          )
        file_recall_all.write(img_dir_i + '/' + file_name + ',' + str(Recall) + ',' + str(Precision) +',' + str(Error_rate)+ ','+ str(F1)+','
                          + str(n_recall) + ',' + str(n_wrong) + ',' + str(len(points_m)) + '\n'
                          )

    file_recall.close()
file_recall_all.close()
acas = 1
