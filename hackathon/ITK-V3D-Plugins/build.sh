...
if [ $build_ITK = "y" -o $build_ITK = "Y" ]; then
	...
	ITK_DIR=`pwd`
	echo $ITK_DIR
	cmake -DBUILD_TESTING=OFF -DBUILD_EXAMPLES=OFF -DITK_USE_REVIEW=ON ../InsightToolkit-4.1.0
	make -j4
	cd ../
else
	if [ ! -d $ITK_BUILD_DIR ]; then
...
echo " set the vaa3d basic_c_fun source dir: "
...
echo 'set the vaa3d binary dir:'
read V3D_BINARY_DIR
echo 'you binary dir is :'$V3D_BINARY_DIR
if [ -d $PLUGINS_BUILD_DIR ]; then
	cd $PLUGINS_BUILD_DIR
	cmake -DV3D_BASIC_C_FUN_SOURCE_DIR=$V3D_BASIC_C_FUN_SOURCE_DIR -DV3D_BINARY_DIR=$V3D_BINARY_DIR -DITK_DIR=$ITK_DIR ../
	make -j4
	cd ../
else
