...
build_itk_library()
{
	...
	if [ $SYSTEM_NAME = 'Darwin' ]; then
		...
		cmake -DBUILD_TESTING=OFF -DBUILD_EXAMPLES=OFF -DITK_USE_REVIEW=ON -    DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE=-fPIC -DCMAKE_C_FLAGS_RELEASE=-fPIC -Wno-dev ../$ITK_SOURCE_DIR
	else
...
cmake -DV3D_BASIC_C_FUN_SOURCE_DIR=$V3D_BASIC_C_FUN_SOURCE_DIR -DV3D_BINARY_DIR=$V3D_BINARY_DIR -DITK_DIR=$ITK_DIR -DCMAKE_BUILD_TYPE=Debug -Wno-dev ../
make -j4
make install
cd ../
