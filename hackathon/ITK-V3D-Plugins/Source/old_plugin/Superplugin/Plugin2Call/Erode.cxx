...
public:
    ...
    void ComputeOneRegion(const V3DPluginArgList & input, V3DPluginArgList & output)
    {
        ...
        if (b_use_binary)
        {
            filter_b->SetInput((ImageType*) p);
            filter_b->SetKernel(  structuringElement );
        }
        else
        {
            filter_g->SetInput((ImageType*) p);
            filter_g->SetKernel(  structuringElement );
        }

        PixelType foreground = 255;
        if (b_use_binary)filter_b->SetErodeValue( foreground );
        try
        {
            if (b_use_binary)
            {
        ...
        if (b_use_binary)
        {
            arg.p=filter_b->GetOutput();
        }
        else
        {
            arg.p=filter_g->GetOutput();
        }
...
bool ErodePlugin::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output,
                         V3DPluginCallback2 & v3d,  QWidget * parent)
{
    ...
    ErodeSpecialized<unsigned char> *runner=new ErodeSpecialized<unsigned char>(&v3d);
...
void ErodePlugin::domenu(const QString & menu_name, V3DPluginCallback2 & callback, QWidget * parent)
{
