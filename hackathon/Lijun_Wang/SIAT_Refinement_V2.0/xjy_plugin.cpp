...
{
    if(menu_name == tr("convert data to 8-bit"))
    {
        QString currentPath;
        QDir dir;
    ...
    else if(menu_name == tr("split swc"))
    {
        Py_SetPythonHome((wchar_t *)(L"./Python36"));//需要配置为本地的python环境
        Py_Initialize();
        if ( !Py_IsInitialized() )
        {
        }
    ...
    else if (menu_name == tr("refine"))
	{
       ...
       PyObject* pFunsplitswc= PyObject_GetAttrString(pModule,"split_swc");  // 这里的hellow就是python文件定义的函数
        ...
        while(getline(infile_s2m,line_s2m))
        {
            ...
            //当只有两三个点时，对swc进行重采样
            ifstream cho;
            cho.open(inswc_file.toLatin1(),ios::in);
            string c;
            int cnt_len = 0;
            while(getline(cho,c))
            {
            ...
            if(cnt_len <= 3&&cnt_len > 0)
            {
                QString qt = currentPath.toLatin1()+"//v3d_qt6.exe";
                QString resample_plugin = currentPath.toLatin1()+"//plugins//neuron_utilities//resample_swc//resample_swc.dll";
                QString resample_para = "resample_swc";
                QString resample_out = savepath.toLatin1()+"//"+QString::fromStdString(s2m[0])+".resample.swc";
                QString cmd = qt + " /x " + resample_plugin + " /f " + resample_para + " /i "+ inswc_file +" /o "+ resample_out +" /p 1";
                //cout<<cmd;
                system(cmd.toLatin1());
                inswc_file = resample_out;
            }
        ...
        QString para = "mean_shift_with_constraints";
...
bool xjy_refine::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    if (func_name == tr("refine"))//split swc, refine and stitch: v3d -x xjy.dll -f refine -i datapath swcpath savepath (tif) (-o outpath)
	{
        /**********convert tiff to 8-bit v3draw*************/
        QString currentPath;
        QDir dir;
        currentPath=dir.currentPath() + "//v3d_external//bin";
        QString rawpath = "";
        string tif = "tif";
        if(infiles.size()>=4&&infiles[3] == tif)//如果输入的是未压缩的tif文件则做一步转换
        {
            cout<<"success";
            QString DATAPATH = infiles[0];
            FILE *filepath;
            ofstream outfile_ms;
            outfile_ms.open(currentPath.toLatin1()+"//dir_tif2raw.txt",ios::app);//创建tif转v3draw的数据目录
            long Handle;
            struct _finddata_t FileInfo;
            string path = DATAPATH.toStdString()+"//*.tif";
            if ((Handle = _findfirst(path.c_str(), &FileInfo)) == -1L)
                printf("没有找到匹配的项目\n");
            else
            {
            ...
            QString temppath = QDir::fromNativeSeparators(DATAPATH);//把windows的"\"转换为"/"
            int index = temppath.lastIndexOf("/");
            QString upperpath = temppath.left(index);//回到传入的数据文件夹的上一层目录创建转换后数据的存放路径
            rawpath = upperpath+"//tif2v3draw";
            QDir *tif2v3draw = new QDir;
            tif2v3draw->mkdir(rawpath);//创建存储转出的v3draw格式文件的文件夹

            ifstream infile_ms;
            infile_ms.open(currentPath.toLatin1()+"//dir_tif2raw.txt",ios::in);
            string line_ms;
            QString qt = currentPath.toLatin1()+"//v3d_qt6.exe";
            QString plugin = currentPath.toLatin1()+"//plugins//data_type//Convert_8_16_32_bits_data//datatypeconvert.dll";
            QString para = "dtc";
            int p = 1;//转成8位
            while(getline(infile_ms,line_ms))
            {
                ...
                splitString(line_ms,a_ms,".");
                QString in_data = DATAPATH + "//"+QString::fromStdString(line_ms);
                QString out = rawpath + "//" + QString::fromStdString(a_ms[0]) +".v3draw";
//                QFileInfo fi(in_marker);
//                if(!fi.isFile())//如果不存在与数据对应的marker文件则跳过这个数据
//                    continue;
                QString cmd = qt + " /x " + plugin + " /f " + para + " /i " + in_data + " /o " + out + " /p " + QString::number(p);
                //cout<<cmd;
                system(cmd.toLatin1());
            }
            QFile fileTemp_tif(currentPath.toLatin1()+"//dir_tif2raw.txt");//删除v3draw数据目录
            fileTemp_tif.remove();
        }

        cout<<"skip";
        /***********split swc***********/
        Py_SetPythonHome((wchar_t *)(L"./v3d_external/bin/Python36"));//需要配置为本地的python环境
        Py_Initialize();
        if ( !Py_IsInitialized() )
        {
        }
       ...
       PyObject* pFunsplitswc= PyObject_GetAttrString(pModule,"split_swc");  // 这里的hellow就是python文件定义的函数
        ...
        if(infiles.size()>=4&&infiles[3] == tif)
        {
            datapath = rawpath;
        }
        else
        {
            datapath = QString::fromStdString(infiles[0]);
        }

        PyTuple_SetItem(pArgs, 0, Py_BuildValue("s", datapath.toStdString().c_str()));
        //QString swcpath = QFileDialog::getOpenFileName(NULL, "select the swc to split","*.swc");//swcpath是完整的需要拆分的swc存放的地址
        QString swcpath = QString::fromStdString(infiles[1]);
        PyTuple_SetItem(pArgs, 1, Py_BuildValue("s", swcpath.toStdString().c_str()));
        //QString savepath = QFileDialog::getExistingDirectory(NULL,"select a folder to save the splited swc",".");//savepath是拆分后的swc存放的地址
        QString savepath = QString::fromStdString(infiles[2]);
        PyTuple_SetItem(pArgs, 2, Py_BuildValue("s", savepath.toStdString().c_str()));
        PyObject* pValue = PyObject_CallObject(pFunsplitswc, pArgs);
        cout<<pValue;
        Py_Finalize();
        ...
        while(getline(infile_m2s,line_m2s))
        {
            ...
            {
                outpath = currentPath.toLatin1()+"//Meanshift_swc";
                QDir *msswc = new QDir;
                msswc->mkdir(QString::fromStdString(outpath));
            }

             outfile.open(outpath +"//"+m2s[0].c_str()+"_meanshift.swc",ios::app);//创建转换后的swc文件
             string line1,line2,line3;
             while(getline(infile1,line1)&&getline(infile2,line2))
             {
                QVector<string> sv1;
    ...
    else if (func_name == tr("refine_ns"))//refine without stitching: v3d -x xjy.dll -f refine_ns -i datapath swcpath (/o outpath)
    {
        /**********convert tiff to 8-bit v3draw*************/
        QString currentPath;
        QDir dir;
        ...
        while(getline(infile_m2s,line_m2s))
        {
            ...
            {
                outpath = currentPath.toLatin1()+"//Meanshift_NOSTITCH_swc";
                QDir *msswc = new QDir;
                msswc->mkdir(QString::fromStdString(outpath));
            }

             outfile.open(outpath +"//"+m2s[0].c_str()+"_meanshift_NOSTITCH.swc",ios::app);//创建转换后的swc文件
             string line1,line2,line3;
             while(getline(infile1,line1)&&getline(infile2,line2))
             {
                QVector<string> sv1;
    ...
    else if (func_name == tr("split_swc"))//spli swc: v3d -x xjy.dll -f split_swc -i datapath swcpath /o savepath
	{
        Py_SetPythonHome((wchar_t *)(L"./v3d_external/bin/Python36"));//需要配置为本地的python环境
        Py_Initialize();
        if ( !Py_IsInitialized() )
       ...
       PyObject* pFunsplitswc= PyObject_GetAttrString(pModule,"split_swc");  // 这里的hellow就是python文件定义的函数
        ...
        PyTuple_SetItem(pArgs, 1, Py_BuildValue("s", swcpath.toStdString().c_str()));
        QString savepath = outfiles[0];
        PyTuple_SetItem(pArgs, 2, Py_BuildValue("s", savepath.toStdString().c_str()));
        PyObject* pValue = PyObject_CallObject(pFunsplitswc, pArgs);
        cout<<pValue;
        Py_Finalize();
	}
    else if (func_name == tr("convert_tif"))//convert tif to 8-bit v3draw: v3d -x xjy.dll -f convert_tif -i datapath /o savepath
	{
        QString currentPath;
        QDir dir;
        currentPath=dir.currentPath() + "//v3d_external//bin";
