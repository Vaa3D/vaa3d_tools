...
struct NeuronFactor
{
	...
	NeuronFactor(int le, V3DLONG la, float a, float b, float c, unordered_map<string, double> fea, vector<double> fea_v, V3DLONG pa, V3DLONG ch, V3DLONG sb) :
		level(le), label(la), x(a), y(b), z(c), feature(fea), feature_vector(fea_v), parent(pa), child(ch), sibling(sb) {}
};

typedef unordered_map<V3DLONG, NeuronFactor> NeuronFactors;

struct NeuronFactorTree
{
	int level;
	...
	NeuronFactorTree(int le, NeuronFactor nft_root, NeuronFactors nfs) :
		level(le), neuron_factor_tree_root(nft_root), neuron_factors(nfs) {}
};
