...
struct NeuronFactor
{
	...
	NeuronFactor(int le, V3DLONG la, float a, float b, float c, 
		V3DLONG pa, V3DLONG ch, V3DLONG ch_sb, V3DLONG sb) : //a featureless vision;
		level(le), label(la), x(a), y(b), z(c), 
		parent(pa), child(ch), child_sibling(ch_sb), sibling(sb) {}

	NeuronFactor(int le, V3DLONG la, float a, float b, float c, unordered_map<string, double> fea, vector<double> fea_v, 
		V3DLONG pa, V3DLONG ch, V3DLONG ch_sb, V3DLONG sb) :
		level(le), label(la), x(a), y(b), z(c), feature(fea), feature_vector(fea_v), 
		parent(pa), child(ch), child_sibling(ch_sb), sibling(sb) {}
};
