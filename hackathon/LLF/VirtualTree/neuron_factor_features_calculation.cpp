...
void calculate_features_at_node(const NeuronTree neuron, NeuronFactor &current, const V3DLONG id_child, const V3DLONG id_ch_sibling, const V3DLONG id_sibling, const V3DLONG id_parent, const NeuronFactors nfs)
{
	...
	*/
	...
	//2019-06-09, Update: remove to 22;
	...
	*/
	...
	// for wrong node relation: node have more than two child:
	int tips_ch;
	if (nfs.count(id_child))
		tips_ch = (nfs.at(id_child).feature.count("fea_tips_num") ? nfs.at(id_child).feature.at("fea_tips_num") : 1);//for leaf node, con't have feature "fea_tips_num";
	else
		tips_ch = 1;
	
	int tips_ch_s;
	if (nfs.count(id_ch_sibling))
		tips_ch_s = (nfs.at(id_ch_sibling).feature.count("fea_tips_num") ? nfs.at(id_ch_sibling).feature.at("fea_tips_num") : 1);
	else
		tips_ch_s = 1;
