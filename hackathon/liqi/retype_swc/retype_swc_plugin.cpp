...
void retype_swc::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("retype"))
    {
        ...
        if (fileOpenName.toUpper().endsWith(".SWC") || fileOpenName.toUpper().endsWith(".ESWC"))
        {
             ...
             type = QInputDialog::getInteger(parent, "Please specify the terafly node type","type:",3,0,256,1,&ok);
             if (!ok)
                 return;
			...
			type = QInputDialog::getInteger(parent, "Please specify the VR node type","type:",3,0,256,1,&ok);
            for(V3DLONG i = 0; i < nt.listNeuron.size(); i++)
            {
                if(nt.listNeuron[i].r != 1)
                    nt.listNeuron[i].type = type;
