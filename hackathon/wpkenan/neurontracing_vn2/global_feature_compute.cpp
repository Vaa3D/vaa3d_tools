...
#define getparent2(n,nt) ((nt).listNeuron.at(n).pn<0)?(1000000000):((nt).hashNeuron.value((nt).listNeuron.at(n).pn))
...
void computeFeature(const NeuronTree & nt, double * features)
{
	...
	//feature # 18: Average parent2-daughter Ratio
...
void computeLinear(const NeuronTree & nt)
{
	...
	for (int i=0;i<list.size();i++)
	{
		...
		if (childs[i].size()==0)
			...
			N_bifs++;
		int parent2 = getparent2(i,nt);
		if (parent2==VOID) continue;
		double l = dist(curr,list.at(parent2));
		Diameter += 2*curr.r;
		Length += l;
