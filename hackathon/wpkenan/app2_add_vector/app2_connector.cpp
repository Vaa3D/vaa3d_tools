  ...
  //  bool b_menu = true;
    ...
    if (!p.p4dImage || !p.p4dImage->valid())
    {
        ...
        for(int i = 0; i < file_inmarkers.size(); i++)
        {
			...
			t.radius = file_inmarkers[i].radius;
...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
    ...
    else if(inmarkers.size() == 1)
    {
        ...
        {
			LocationSimple pt;
			pt.x = inmarkers[0].x+2;
			pt.y = inmarkers[0].y+2;
			pt.z = inmarkers[0].z+2;
			pt.radius = 10;
            ...
            switch(datatype)
            {
                case V3D_UINT8:
					...
					////compute_rgn_stat_new(pt, indata1d, 1, in_sz[0], in_sz[1], in_sz[2], 1, V3D_UINT8);
					//
					unsigned char ***indata3d;
					indata3d = (unsigned char ***)malloc(in_sz[2] * sizeof(unsigned char**));
					for (int i = 0; i < in_sz[2]; i++){
						indata3d[i] = (unsigned char **)malloc(in_sz[1] * sizeof(unsigned char*));
						for (int j = 0; j < in_sz[1]; j++){
							indata3d[i][j] = (unsigned char *)malloc(in_sz[0] * sizeof(unsigned char));
						}
					}
					for (int z = 0; z < in_sz[2]; z++){
						for (int y = 0; y < in_sz[1]; y++){
							for (int x = 0; x < in_sz[0]; x++){
								indata3d[z][y][x] = indata1d[z*in_sz[1] * in_sz[0] + y*in_sz[0] + x];
							}
						}
					}
					...
					//	//cout << i << endl;
					//	if (i % 10000 == 0){
					//		cout << i << ": " << GetTickCount()-endTime1 << endl;
					//		endTime1 = GetTickCount();
					//	}
					//	compute_rgn_stat_new(pt, indata3d, 1, in_sz[0], in_sz[1], in_sz[2], 1, V3D_UINT8,vec1,vec2,vec3);
					//}
					compute_rgn_stat_new(pt, indata3d, 1, in_sz[0], in_sz[1], in_sz[2], 1, V3D_UINT8, vec1, vec2, vec3, sigma1, sigma2, sigma3);
					cout << sigma1 << "," << sigma2 << "," << sigma3 << endl;
					for (int i = 0; i < 3; i++){
						cout << vec3[i] << ",";
					}
					cout << endl;
					...
					cout << endl;
					return 0;
					//compute_rgn_stat_new(pt, indata1d, 1, in_sz[0], in_sz[1], in_sz[2], 1, V3D_UINT8, vec1, vec2, vec3, sigma1, sigma2, sigma3);

					//cout << GetTickCount() - startTime << endl;
					//wpshowDirection(vec1, vec2, vec3, sigma1, sigma2, sigma3, pt);
