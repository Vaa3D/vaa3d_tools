...
double consin(double vec1[], double vec2[], int len){
	double sum = 0;
	double A = 0;
	double B = 0;
	for (int i = 0; i < len; i++){
                                         ...
                                         bool is_break_accept = false)
{
	...
	//add dircetion
	T ***indata3d;
	indata3d = (T ***)malloc(sz2 * sizeof(T**));
	for (int i = 0; i < sz2; i++){
		indata3d[i] = (T **)malloc(sz1 * sizeof(T*));
		for (int j = 0; j < sz1; j++){
			indata3d[i][j] = (T *)malloc(sz0 * sizeof(T));
		}
	}
	for (int z = 0; z < sz2; z++){
		for (int y = 0; y < sz1; y++){
			for (int x = 0; x < sz0; x++){
				indata3d[z][y][x] = inimg1d[z*sz1 * sz0 + y*sz0 + x];
			}
		}
	}

	double **direction;
	direction = (double **)malloc(sz0*sz1*sz2 * sizeof(double*));
	for (int i = 0; i < sz0*sz1*sz2; i++){
		direction[i] = (double *)malloc(3 * sizeof(double));
	}
	...
	#pragma omp parallel for private(pt,vec1,vec2,vec3)
	for (int i = 0; i < sz0 * sz1 * sz2; i++){
		pt.x = i%sz0;
		pt.y = int(i/sz0)%sz1;
		pt.z = int(i/(sz0*sz1));
		...
		compute_rgn_stat_new(pt, indata3d, 1, sz0, sz1, sz2, 1, V3D_UINT8, vec1, vec2, vec3, sigma1, sigma2, sigma3);
		direction[i][0] = vec1[0];
		direction[i][1] = vec1[1];
		direction[i][2] = vec1[2];
		//cout << i << endl;
	}
			...
			for(int jj = -1; jj <= 1; jj++)
			{
				...
				for(int ii = -1; ii <= 1; ii++)
				{
					...
					if(state[index] != ALIVE)
					{
						...
						double vec[3] = { ii, jj, kk };
						double new_dist = phi[min_ind] + (GI(index) + GI(min_ind))*factor*0.5;
						//double consin_dist = consin(direction[index], direction[min_ind],3);
						double consin_dist = double(exp((consin(vec, direction[min_ind], 3) - 1)) * 2000);
						//consin_dist = 0;
						//delete[] vec;
						cout << "new_dist:" << (GI(index) + GI(min_ind))*factor*0.5 << "consin_dist:" << consin_dist << endl;
						new_dist += consin_dist;
						/*new_dist += consin_dist;*/
						long prev_ind = min_ind;
