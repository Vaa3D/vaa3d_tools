...
void swc_to_maskimage(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    for (V3DLONG i = 0; i < filenames.size();i++)
    {
        ...
        ComputemaskImage(neuron, pImMask, sx, sy, sz,10);
	}
...
{
    cout << "in" << __LINE__ << " " << __FUNCTION__ << endl;
    ...
    if(paras.size() != 0 && paras.size() != 4) return false;

    QString qs_input(infiles[0]);
    NeuronTree neuron = readSWC_file(qs_input);
    QString qs_output = outfiles.empty() ? qs_input + "_mask.v3draw" : QString(outfiles[0]);
    NeuronSWC *p_cur=0;
    double x_min,x_max,y_min,y_max,z_min,z_max;
    x_min=x_max=y_min=y_max=z_min=z_max=0;
    V3DLONG sx,sy,sz;
    ...
    // compute coordinate region
    if (paras.empty()) {nx = sx; ny = sy; nz = sz;ComputemaskImage(neuron, pImMask, sx, sy, sz);}
    else {
        nx = atoi(paras[0]);
        ny = atoi(paras[1]);
        nz = atoi(paras[2]);
        ComputemaskImage(neuron, pImMask, sx, sy, sz,atoi(paras[3]));
    }
