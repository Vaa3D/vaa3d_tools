...
int SwcTree::initialize(){
	...
	V3DLONG min_x = 9999999, min_y = 9999999, min_z = 9999999;
	V3DLONG max_x = 0, max_y = 0, max_z = 0;
	...
	for (V3DLONG i = 0; i<num_p; ++i)
	{
		...
		max_x = max(nt.listNeuron[i].x, max_x);
		max_y = max(nt.listNeuron[i].y, max_y);
		max_z = max(nt.listNeuron[i].z, max_z);



		min_x = min(nt.listNeuron[i].x, min_x);
		min_y = min(nt.listNeuron[i].y, min_y);
		min_z = min(nt.listNeuron[i].z, min_z);
	...
	cout << "wpdebug: " << __FUNCTIONW__ << " " << __LINE__ << " " << max_x - min_x << " " << max_y - min_y << " " << max_z - min_z << endl;
