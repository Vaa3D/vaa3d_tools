...
class ResidualAttentionModel_92_32input_update(nn.Module):
    ...
    def __init__(self):
        ...
        # self.mpool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)  # 16*16
        self.residual_block1 = ResidualBlock(4, 16)  # 32*32
        self.attention_module1 = AttentionModule_stage1_cifar(16, 16, size1=(4, 4), size2=(2, 2))  # 32*32
        self.residual_block2 = ResidualBlock(16, 32, 2)  # 16*16
        self.attention_module2 = AttentionModule_stage2_cifar(32, 32, size=(2, 2))  # 16*16
        self.attention_module2_2 = AttentionModule_stage2_cifar(32, 32, size=(2, 2))  # 16*16 # tbq add
        self.residual_block3 = ResidualBlock(32, 64, 2)  # 4*4
        self.attention_module3 = AttentionModule_stage3_cifar(64, 64)  # 8*8
        self.attention_module3_2 = AttentionModule_stage3_cifar(64, 64)  # 8*8 # tbq add
        self.attention_module3_3 = AttentionModule_stage3_cifar(64, 64)  # 8*8 # tbq add
        self.residual_block4 = ResidualBlock(64, 128)  # 8*8
        self.residual_block5 = ResidualBlock(128, 128)  # 8*8
        self.residual_block6 = ResidualBlock(128, 128)  # 8*8
        self.mpool2 = nn.Sequential(
            nn.InstanceNorm3d(128),
            nn.LeakyReLU(inplace=True),
            nn.AvgPool3d(kernel_size=2)
        )
        self.fc = nn.Linear(128*2*2*2,5)
        self.convcsz=nn.Conv3d(128,128,kernel_size=3,stride=8,padding=1)
    def forward(self, x):
        out = self.conv1(x)
        # out = self.mpool1(out)
        # print(out.data)
        ...
        out = self.residual_block6(out)
        out = self.mpool2(out)
        # print(out.shape)              #1,128,16,16,16
        out=self.convcsz(out)
        out = out.view(out.size(0), -1)
        out = self.fc(out)
