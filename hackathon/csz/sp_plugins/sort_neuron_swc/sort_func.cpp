...
bool sort_func(const V3DPluginArgList & input, V3DPluginArgList & output)
{
    ...
    if (hasPara)
    {
        isdatafile=atoi(paralist->at(0));
        isout=atoi(paralist->at(1));
        if (paralist->size()==2)
        {
        ...
        {
            thres = atof(paralist->at(2));
            cout<<"threshold: "<<thres<<endl;
            if (paralist->size() ==4 )
            {
                rootid = atoi(paralist->at(3));
                cout<<"root id: "<<rootid<<endl;
            }
            else if (paralist->size() >4)
            {
    ...
    if(isdatafile){
        ...
        fileOpenName = QString(inlist->at(0));
        ...
        iswc=((vector<NeuronTree*> *)(input.at(0).p))->at(0);
        neuron=iswc->listNeuron;
    }
    ...
    if(isout){
        ...
        otswc=((vector<NeuronTree*> *)(output.at(0).p))->at(0);
    }
    ...
    if(isout){
        ...
        if (!SortSWC(neuron, otswc->listNeuron , rootid, thres))
        {
            cout<<"Error in sorting swc"<<endl;
            return false;
        }
