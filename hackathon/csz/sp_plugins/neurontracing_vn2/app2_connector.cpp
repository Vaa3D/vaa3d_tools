...
//quantile(p4dImageNew->getRawDataAtChannel(0), p4dImageNew->getTotalUnitNumberPerChannel(), qua);
template <class T1, class T2> bool quantile(T1 *data, V3DLONG n, T2 & qua)
{
    if (!data || n<=0)
      return false;
...
//static bool cmp(vector<long> sta1,vector<long> sta2){
//    return sta1[1]<sta2[1];
//}
template <class T1, class T2> bool maxgrad(T1 *data, V3DLONG n, T2 & qua)
{
    if (!data || n<=0)
      return false;
    ...
    unordered_map<T1,long> greysta;
    for(int i=0;i<n;i++){
        greysta[data[i]]++;
    }
//    vector<vector<long>> sta(2,vector<long> (greysta.size()));
//    int i=0;
//    for(auto &[greyvalue,count]:greysta){
//        sta[0][i]=greyvalue;
//        sta[1][i]=count;
//        i++;
//    }
//    std::cout<<"-----------------------csz debug "<<i<<"\n";
//    sort(sta.begin(),sta.end(),cmp);
    vector<long> sta(258,0);
    for(auto &[greyvalue,count]:greysta){
        sta[int(greyvalue)]=count;
    }
    int index=-1,maxd=-1;
    int start;
    for(int i=0;i<256;i++){
...
//            p.bkg_thresh = imgAve +0.5*td ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5; //20170523, PHC
            double qua;
            //quantile(p4dImageNew->getRawDataAtChannel(0), p4dImageNew->getTotalUnitNumberPerChannel(), qua);
            maxgrad(p4dImageNew->getRawDataAtChannel(0), p4dImageNew->getTotalUnitNumberPerChannel(), qua);
            p.bkg_thresh=qua;
            cout<<"csz debug "<<p.bkg_thresh<<endl;
        }
        else
            p.bkg_thresh = 0;
    ...
    if(1)
    {
        ...
        if(p.isout==1)
            ...
            p.pneurontree->deepCopy(swc_convert(outswc));
            //qDebug()<<p.pneurontree;
        }
        ...
        //call sort_swc function
            if(p.isout){
                ...
                V3DPluginArgList input_sort;
                V3DPluginArgList output;
                vector<NeuronTree *> inputswc;
                vector<NeuronTree *> outputswc;
                NeuronTree *otswc=new NeuronTree();
                NeuronTree *otswc2=new NeuronTree();
                inputswc.push_back(p.pneurontree);
                qDebug()<<p.pneurontree->listNeuron.size();
                outputswc.push_back(otswc);
                arg.type="Inputswc";
                arg.p=(void *)(&inputswc);
                input_resample.push_back(arg);
                arg.type="InputParam";
                std::vector<char*> arg_resample_para; arg_resample_para.push_back("0");arg_resample_para.push_back("0");arg_resample_para.push_back("10");
                arg.p = (void *) & arg_resample_para;
                input_resample.push_back(arg);
                arg.type="Outputswc";
                ...
                arg.type="Inputswc";
                arg.p=(void *)(&inputswc);
                input_sort.push_back(arg);
                arg.type="InputParam";
                std::vector<char*> arg_sort_para; arg_sort_para.push_back("0");arg_sort_para.push_back("0");arg_sort_para.push_back("0");
                arg.p = (void *) & arg_sort_para;
                input_sort.push_back(arg);
                outputswc.push_back(otswc2);
                arg.type="Outputswc";
                arg.p =(void *)(&outputswc);
                output.clear();
                output.push_back(arg);
...
bool PARA_APP2::fetch_para_commandline(const V3DPluginArgList &input, V3DPluginArgList &output, V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    int k=0;
    isdatafile=atoi(paras[k]);k++;
    isout=atoi(paras[k]);k++;
