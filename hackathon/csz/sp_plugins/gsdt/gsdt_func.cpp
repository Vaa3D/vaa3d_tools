...
bool gsdt(const V3DPluginArgList & input, V3DPluginArgList & output,V3DPluginCallback2 &callback)
{
    ...
    if(paras.size() >= 3) bkg_thresh = atoi(paras.at(2));
    if(paras.size() >= 4) cnn_type = atoi(paras.at(3));
    if(paras.size() >= 5) channel = atoi(paras.at(4));
    if(paras.size() >= 6) z_thickness = atof(paras.at(5));
    ...
    if(isdatafile==1){
        ...
        Image4DSimple *inimg=((vector<Image4DSimple*> *)(input.at(0).p))->at(0);
        V3DLONG totalbytes = inimg->getTotalBytes();
        inimg1d = new unsigned char [totalbytes];
        //inimg1d=inimg->getRawData();
        memcpy(inimg1d, inimg->getRawData(), totalbytes);
        in_sz[0] = inimg->getXDim();
        in_sz[1] = inimg->getYDim();
        in_sz[2] = inimg->getZDim();
        in_sz[3] = inimg->getCDim();
        datatype=inimg->getUnitBytes();
    }
    if(isout==1){
    ...
    for(V3DLONG i = 0; i < tol_sz; i++)
	{
     ...
     if(isout==1)
        simple_saveimage_wrapper(callback,outimg_file, outimg1d, in_sz, 1);
         ...
         otimg->cleanExistData();
         qDebug()<<otimg->setData((unsigned char *)outimg1d, in_sz[0], in_sz[1], in_sz[2], 1, V3D_UINT8);
         //callback.saveImage(otimg,"D:\\A_DLcsz\\DLtrain\\fixed_data\\result\\1.tiff");
         qDebug()<<otimg;
     }
