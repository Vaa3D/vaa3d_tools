...
//    }
    ...
    if(inparas.size() >= 1) isdatafile = atoi(inparas.at(0));
    if(inparas.size() >= 2) isout = atoi(inparas.at(1));

    Image4DSimple *p4DImage ;
    if(isdatafile==1){
        input_image = ((vector<char*> *)(input.at(0).p))->at(0);
        p4DImage= callback.loadImage(input_image);
    ...
    if(isout==1){
        output_image = ((vector<char*> *)(output.at(0).p))->at(0);
        outimg=new Image4DSimple();
        qDebug()<<"output is file!";
    }else{
        outimg=((vector<Image4DSimple*> *)(output.at(0).p))->at(0);
...
//    char * output_image = ((vector<char*> *)(output.at(0).p))->at(0);
    ...
    int k_sz[3] = {(inparas.size() >= 3) ? atoi(inparas[2]) : 3, (inparas.size() >= 3) ? atoi(inparas[2]) : 3, (inparas.size() >= 4) ? atoi(inparas[3]) : 1};

    spaceSigmaXY = k_sz[0]/3.0;
    spaceSigmaZ = k_sz[2]/3.0;

    colorSigma = (inparas.size() >= 5) ? atoi(inparas[4]) : 35;
...
//    char * output_image = ((vector<char*> *)(output.at(0).p))->at(0);
    ...
    if(inparas.size() >= 3) Wxy = atoi(inparas.at(2));
    if(inparas.size() >= 4) Opt = atoi(inparas.at(3));
...
bool sigma_correction_dofunc(const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback)
{
    ...
    if(inparas.size() >= 1) isdatafile = atoi(inparas.at(0));
    if(inparas.size() >= 2) isout = atoi(inparas.at(1));
    if(inparas.size() >= 3) gain = atoi(inparas.at(2));
    if(inparas.size() >= 4) cutoff = atoi(inparas.at(3));
...
bool enhancement_dofunc(const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback)
{
    ...
    if(inparas.size() >= 3)
    {
        k_sz[0] = atoi(inparas.at(2));
        k_sz[1] = atoi(inparas.at(2));
    }

    if(inparas.size() >= 4) k_sz[2] = atoi(inparas.at(3));
    if(inparas.size() >= 5) colorSigma = atoi(inparas[4]);

    if(inparas.size() >= 6) gain = atoi(inparas.at(5));
    if(inparas.size() >= 7) cutoff = atoi(inparas.at(6));

    if(inparas.size() >= 8) do_bf = atoi(inparas.at(7));
    if(inparas.size() >= 9) do_fft = atoi(inparas.at(8));
