...
bool q_create_kernel_2D(const int rows,const int cols,const int anchor_x,const int anchor_y,const int shape,CKernelMP2D &kernel)
{
	//check parameters
	if(rows<=0 || cols<=0)
	{
	...
	else if (shape == 1)	//disk
	{
		...
		for (int x = 0; x < cols; x++)
			for (int y = 0; y < rows; y++)
				...
				{
					float dis = sqrt(double((x - anchor_x)*(x - anchor_x) + (y - anchor_y)*(y - anchor_y) + (z - anchor_z)*(z - anchor_z)));
					if (dis <= radius)
						kernel.kernel[z][y][x] = 1;
				}
	}
	...
	const CKernelMP3D kernel, unsigned char *&p_img_output)
{
	...
	//perform morphological operation
	long pgsz_y = l_img_width;
	long pgsz_xy = l_img_width * l_img_height;
				...
				for (long shift_z = shift_z_min; shift_z < shift_z_max; shift_z++)
					for (long shift_y = shift_y_min; shift_y < shift_y_max; shift_y++)
						for (long shift_x = shift_x_min; shift_x < shift_x_max; shift_x++)
						{
							...
							long index = pgsz_xy*zz + pgsz_y*yy + xx;

							//if dilation, replaced with the maximal value under kernel
							if (p_img_input[index]>max_value && 
								kernel.kernel[kernel.anchor_y + shift_y][kernel.anchor_x + shift_x][kernel.anchor_z + shift_z] == 1)
								max_value = p_img_input[index];
						}
				long index = pgsz_xy*z + pgsz_y*y + x;
				p_img_output[index] = max_value;
			}
...
bool q_erosion_2D(const unsigned char *p_img_input,const long l_img_width,const long l_img_height,const CKernelMP2D kernel,unsigned char *&p_img_output)
{
	//check parameters
	if(!p_img_input)
	{
				...
				for (long shift_z = shift_z_min; shift_z < shift_z_max; shift_z++)
					for (long shift_y = shift_y_min; shift_y < shift_y_max; shift_y++)
						for (long shift_x = shift_x_min; shift_x < shift_x_max; shift_x++)
						{
							...
							//if erosion, replaced with the minimal value under kernel
							if (p_img_input[index]<min_value &&
								kernel.kernel[kernel.anchor_y + shift_y][kernel.anchor_x + shift_x][kernel.anchor_z + shift_z] == 1)
								min_value = p_img_input[index];
						}
				long index = pgsz_xy*z + pgsz_y*y + x;
...
bool q_closing_2D(const unsigned char *p_img_input,const long l_img_width,const long l_img_height,const CKernelMP2D kernel,unsigned char *&p_img_output)
{
	//check parameters
	if(!p_img_input)
	{
	...
	//dilation
	if(!q_dilation_2D(p_img_input,l_img_width,l_img_height,kernel,p_img_temp))
	{
	...
	//erosion
	if(!q_erosion_2D(p_img_temp,l_img_width,l_img_height,kernel,p_img_output))
	{
	...
	const CKernelMP3D kernel, unsigned char *&p_img_output)
{
	...
	unsigned char *p_img_temp = 0;
	//dilation
	if (!q_dilation_3D(p_img_input, l_img_width, l_img_height, l_img_depth, kernel, p_img_temp))
	{
		printf("ERROR: dilation() return false.\n");
		if (p_img_temp) { delete[] p_img_temp; p_img_temp = 0; }
		return false;
	...
	//erosion
	if (!q_erosion_3D(p_img_temp, l_img_width, l_img_height, l_img_depth, kernel, p_img_output))
	{
		printf("ERROR: q_erosion() return false.\n");
		if (p_img_temp) { delete[] p_img_temp; p_img_temp = 0; }
		return false;
...
bool q_opening_2D(const unsigned char *p_img_input,const long l_img_width,const long l_img_height,const CKernelMP2D kernel,unsigned char *&p_img_output)
{
	//check parameters
	if(!p_img_input)
	{
	...
	unsigned char *p_img_temp=0;
	//erosion
	if(!q_erosion_2D(p_img_input,l_img_width,l_img_height,kernel,p_img_temp))
	{
		printf("ERROR: q_erosion() return false.\n");
		if(p_img_temp) {delete[] p_img_temp; p_img_temp=0;}
		return false;
	...
	//dilation
	if(!q_dilation_2D(p_img_temp,l_img_width,l_img_height,kernel,p_img_output))
	{
		printf("ERROR: dilation() return false.\n");
		if(p_img_temp) {delete[] p_img_temp; p_img_temp=0;}
		return false;
	...
	const CKernelMP3D kernel, unsigned char *&p_img_output)
{
	...
	unsigned char *p_img_temp = 0;
	//erosion
	if (!q_erosion_3D(p_img_input, l_img_width, l_img_height, l_img_depth, kernel, p_img_temp))
	{
		printf("ERROR: q_erosion() return false.\n");
		if (p_img_temp) { delete[] p_img_temp; p_img_temp = 0; }
		return false;
	...
	//dilation
	if (!q_dilation_3D(p_img_temp, l_img_width, l_img_height, l_img_depth, kernel, p_img_output))
	{
		printf("ERROR: dilation() return false.\n");
		if (p_img_temp) { delete[] p_img_temp; p_img_temp = 0; }
		return false;
