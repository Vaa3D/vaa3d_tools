...
foldername_input='D:\ALLEN\1stripremonve_ori';
foldername_output='D:\ALLEN\1stripremove_result';
...
for i=1:length(curdir)
    if(curdir(i).isdir && curdir(i).name(1)~='.')
        ...
        if(length(curdir(i).name)>3 && curdir(i).name(end-2)=='r'&& curdir(i).name(end-1)=='a'&& curdir(i).name(end)=='w')
            ...
            I=loadRaw2Stack(fullfilename);  %%载入图像
            
            %检视图像频谱并确定参数(filter_axis_dir, filter_cutoff, filter_radius)
            imgsize=size(I);
            I_slice=double(I(:,:,round(imgsize(3)/2)));  %%取最中间的切片
            figure;imshow(uint8(I_slice));impixelinfo     %%查看切片时域图
            %显示对比度增强后图像，gamma颜色校正
            c=1;    gamma=0.3;%0.6,3
...
%             figure; imshow(H);
            %构建带状限波滤波器
            filter_axis_dir=0;%滤波器方向同横轴夹角(应按照条纹方向手工选择合适的角度)
            filter_cutoff=10;%截止频率太高将导致低频条纹不能完全消除,太低则会出现虚假纹理!越小离中心越近
            filter_radius=10;%滤波器宽度半径(该值较大时某些高亮区域周围会出现虚假低频纹理!)越小越窄
            %构建水平带状滤波器
            dim_max2=max(imgsize(1:2))*2;%%取长和宽中较大的值乘以2
