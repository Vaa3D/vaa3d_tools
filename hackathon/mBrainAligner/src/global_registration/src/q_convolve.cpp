		...
		const vector<double> &vec1D_kernel)
{
	...
	double *p_img64f_tmp1=new double[l_npixels]();
	double *p_img64f_tmp2=new double[l_npixels]();
	...
	//convolve along X
	for(long i=0;i<l_npixels;i++) p_img64f_tmp1[i]=0.0;	//in some machine, array was not initilized to 0! don't know why
	for(long X=0;X<sz_img[0];X++)
	{
	...
	//convolve along Y
	for(long i=0;i<l_npixels;i++) p_img64f_tmp2[i]=0.0;	//in some machine, array was not initilized to 0! don't know why
	for(long Y=0;Y<sz_img[1];Y++)
	{
