...
bool q_save64f01_image(const double *p_img64f,const long long sz_img[4],const char *filename)
{
	...
	long long sz_img_tmp[4];//V3DLONG=long 
	for (int i = 0; i < 4; i++)	sz_img_tmp[i] = sz_img[i];
	saveImage(filename,p_img4save,sz_img_tmp,1);
#else
...
bool q_extractchannel(const unsigned char *p_img8u,const long sz_img[4],const long l_refchannel,unsigned char *&p_img8u_1c)
{
	...
	if(p_img8u_1c!=0)
	{
		...
		delete []p_img8u_1c;		p_img8u_1c=0;
	}
	...
	//allocate memory
	p_img8u_1c=new unsigned char[l_npixels_1c]();
	if(!p_img8u_1c)
	{
	...
	for(long x=0;x<sz_img_1c[0];x++)
		for(long y=0;y<sz_img_1c[1];y++)
			for(long z=0;z<sz_img_1c[2];z++)
			{
				...
				p_img8u_1c[ind_1c]=p_img8u[ind_ref];
			}
		...
		double *&p_img64f_sub2tar,long l_masscenteroffset[4])
{
	...
	//find mass center of target and subject image
	...
	for(long z=0;z<sz_img[2];z++)
		for(long y=0;y<sz_img[1];y++)
			for(long x=0;x<sz_img[0];x++)
			{
				...
				if(p_img64f_tar[ind]<0 || p_img64f_sub[ind]<0) continue;	//exclude the invalid pixels
	...
	for(long z=0;z<sz_img[2];z++)
		for(long y=0;y<sz_img[1];y++)
			for(long x=0;x<sz_img[0];x++)
			{
				...
				double Ix,Iy,Iz;
				Ix=p_img64f_4d[0][z][y][x_b]-p_img64f_4d[0][z][y][x_s];	//Ix=(E-W)/2
				Iy=p_img64f_4d[0][z][y_b][x]-p_img64f_4d[0][z][y_s][x];	//Iy=(S-N)/2
				Iz=p_img64f_4d[0][z_b][y][x]-p_img64f_4d[0][z_s][y][x];	//Iz=(down-up)/2
