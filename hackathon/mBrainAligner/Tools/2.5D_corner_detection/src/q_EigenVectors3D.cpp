...
//calculates the eigen values and vectors of the 2D hessian images
//        |Ixx Ixy |
//hessian=|Ixy Iyy |
// 
//eigenvalue:eigval2>eigval1

bool l_eigenvectors2D(float *p_Ixx, float *p_Ixy, float *p_Iyy,
	                  long sz_img[4],
	                  float *&p_eigval2, float *&p_eigval1,	     
	                  float *&p_eigvec2x, float *&p_eigvec2y,
                   	  float *&p_eigvec1x, float *&p_eigvec1y)
{
	...
	if (p_Ixx == 0 || p_Ixy == 0  || p_Iyy == 0 )
	{
		printf("ERROR: Invalid input 3D hessian image pointer!\n");
		return false;
	}
	if (p_eigval2 || p_eigval1 ||
		p_eigvec2x || p_eigvec2y ||
		p_eigvec1x || p_eigvec1y )
	{
	...
	p_eigval2 = new(std::nothrow) float[l_npixels](); p_eigval1 = new(std::nothrow) float[l_npixels]();
	p_eigvec2x = new(std::nothrow) float[l_npixels]();	p_eigvec2y = new(std::nothrow) float[l_npixels]();
	p_eigvec1x = new(std::nothrow) float[l_npixels]();	p_eigvec1y = new(std::nothrow) float[l_npixels]();
	if (!p_eigval2 || !p_eigval1 || !p_eigvec2x || !p_eigvec2y ||
		!p_eigvec1x || !p_eigvec1y)
	{
...
#pragma omp parallel for
	...
	{
		SymmetricMatrix Mat(2);
		DiagonalMatrix eigval(2);
		Matrix eigvec(2, 2);
		//Mat.row(1) << (double)p_Ixx[i];
		//Mat.row(2) << (double)p_Ixy[i] << (double)p_Iyy[i];
		Mat(1, 1) = (double)p_Ixx[i]; Mat(1, 2) = (double)p_Ixy[i];
		Mat(2, 1) = (double)p_Ixy[i]; Mat(2, 2) = (double)p_Iyy[i];
		eigenvalues(Mat, eigval, eigvec);

		p_eigval2[i] = (float)eigval(2);
		p_eigval1[i] = (float)eigval(1);

		p_eigvec2x[i] = (float)eigvec(1, 2); p_eigvec2y[i] = (float)eigvec(2, 2); 
		p_eigvec1x[i] = (float)eigvec(1, 1); p_eigvec1y[i] = (float)eigvec(2, 1);
	}
	return true;
}
...
#pragma omp parallel for
	for (long i = 0; i < l_npixels; i++)
	{
		SymmetricMatrix Mat(3);
		DiagonalMatrix eigval(3);
		Matrix eigvec(3, 3);

		//Mat.row(1) << (double)p_Ixx[i];
		//Mat.row(2) << (double)p_Ixy[i] << (double)p_Iyy[i];
		//Mat.row(3) << (double)p_Ixz[i] << (double)p_Iyz[i] << (double)p_Izz[i];
		Mat(1, 1) = (double)p_Ixx[i];
		Mat(2, 1) = (double)p_Ixy[i]; Mat(2, 2) = (double)p_Iyy[i];
		Mat(3, 1) = (double)p_Ixz[i]; Mat(3, 2) = (double)p_Iyz[i]; Mat(3, 3) = (double)p_Izz[i];
		eigenvalues(Mat, eigval, eigvec);

		p_eigval3[i] = (float)eigval(3);
		p_eigval2[i] = (float)eigval(2);
		p_eigval1[i] = (float)eigval(1);

		p_eigvec3x[i] = (float)eigvec(1, 3); p_eigvec3y[i] = (float)eigvec(2, 3); p_eigvec3z[i] = (float)eigvec(3, 3);
		p_eigvec2x[i] = (float)eigvec(1, 2); p_eigvec2y[i] = (float)eigvec(2, 2); p_eigvec2z[i] = (float)eigvec(3, 2);
		p_eigvec1x[i] = (float)eigvec(1, 1); p_eigvec1y[i] = (float)eigvec(2, 1); p_eigvec1z[i] = (float)eigvec(3, 1);
	}
