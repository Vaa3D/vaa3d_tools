...
int tipdetection(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    layout->addWidget(new QLabel("numbers of rays"),1,0);
    layout->addWidget(numbers_2d_spinbox, 1,1,1,5);

    layout->addWidget(new QLabel("magnitude"),2,0);
    layout->addWidget(magnitude_spinbox, 2,1,1,5);

    layout->addWidget(new QLabel("minimum_scale"),3,0);
    layout->addWidget(minimum_scale_spinbox, 3,1,1,5);

    layout->addWidget(new QLabel("number_of_scales"),4,0);
    layout->addWidget(number_of_scales_spinbox, 4,1,1,5);

    layout->addWidget(new QLabel("step_size"),5,0);
    layout->addWidget(step_size_spinbox, 5,1,1,5);

    layout->addWidget(new QLabel("intensity_threshold_scales"),6,0);
    layout->addWidget(intensity_threshold_spinbox, 6,1,1,5);

    layout->addWidget(new QLabel("angle_threshold_scales"),7,0);
             ...
             for(V3DLONG i=radiu_block+1;i<new_ny-radiu_block-1;i++)
             {
                 for(V3DLONG k=radiu_block+1;k<new_nx-radiu_block-1;k++)
                 {
                     ...
                     if(pixe>background_threshold)
                     {
                         ...
                         {
                            xy_flag=rayinten_2D(radiu_block+1,radiu_block+1,number_of_rays,minimum_scale,ray_x,ray_y,length_block,length_block,xy_mip_datald,intensity_threshold,angle_threshold);
                         }

                         if(swith==1) //select xz_plane
                         {
                             xz_flag=rayinten_2D(radiu_block+1,radiu_block+1,number_of_rays,minimum_scale,ray_x,ray_y,length_block,length_block,xz_mip_datald,intensity_threshold,angle_threshold);
                         ...
                         if(swith==2) //select yz_plane
                         {
                             ...
                             yz_flag=rayinten_2D(radiu_block+1,radiu_block+1,number_of_rays,minimum_scale,ray_x,ray_y,length_block,length_block,yz_mip_datald,intensity_threshold,angle_threshold);
