...
int  rayinten_2D(int point_x,int point_y,int m,int n, vector<vector<float>> ray_x,vector<vector<float>> ray_y,V3DLONG nx,  V3DLONG ny, unsigned char *PP, int intensity_threshold,int angle_threshold)
{
    n=n+6;
    int result=0;
    ...
    if(flag1&&flag2&&flag3&&flag4)
    {
       ...
       // cout<<"angle is "<<min_angle<<endl;
        if((0.10*2*PI<min_angle<0.30*2*PI))
        {
            result= 1;
        }
        else {
