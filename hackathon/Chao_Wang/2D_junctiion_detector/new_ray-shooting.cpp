...
void delete_small_area(V3DLONG nx,V3DLONG ny, unsigned char * &input_image)
{
    ...
    for (int j = 1; j < ny - 1; j++)
    {
        ...
        {
            if (input_image[j*nx+i] == 255)   //pixel value is not equal to 0
            {
                //v3d_msg(QString("creat a new stack"));
                stack <V3DLONG> neighborPixels; //new stack
                neighborPixels.push(j*nx+i);     // Pixel coordinates: <i,j> ，Take this pixel as the starting point to find the connected domain
                label=label+1;  // Start a new tag that marks the difference between the connected domains
                while (!neighborPixels.empty())
                {

                    V3DLONG label_coordinate=neighborPixels.top();// Get the top pixel in the stack and mark it with the same label
...
/*******
 * the function get_2D_ValueUINT8 is to get the pixel value of the point of picture.
 * ****/
void get_the_based_distance_rayscan(int point_x,int point_y,int redius,unsigned char * P,V3DLONG sz0,V3DLONG sz1,int &T0,int &T1)
{

     int x_location=point_x;
     int y_location=point_y;
...
//     cout<<"create 2D_ray success"<<endl;
     ...
     for(int i = 0; i <ray_numbers_2d; i++)   //m is the numble of the ray
     {
         ...
         {
             pixe = project_interp_2d((y_location)+ray_y[i][j], (x_location)+ray_x[i][j], P, nx,ny , (x_location), (y_location));
         ...
         if((flag==0)/*||(gap_pixe==0)*/) //Judge no gap ray
         {
...
//     cout<<"x:" <<x_location<<"y:" <<y_location<<endl;
//     cout<<"fx_gap_length_final:" <<gap_length<<"radius:" <<redius<<endl;
     if((gap_length-redius)<=0)
     {
         T0=5;
         T1=4;
     }
...
//     v3d_msg(QString("t0 is %1 . t1 is %2").arg(T0).arg(T1));
...
template<class T> double markerRadius_hanchuan_XY(T* &inimg1d, V3DLONG nx, V3DLONG ny, V3DLONG x,V3DLONG y, double thresh)
{
    ...
    double ir;
    for (ir=1; ir<=max_r; ir=ir+1)
    {
        total_num = background_num = 0;

        int dy, dx;
        //for (dz= zlower; dz <= zupper; ++dz)
            for (dy= -ir; dy <= +ir; dy=dy+1)
                for (dx= -ir; dx <= +ir; dx=dx+1)
                {
                    total_num++;
...
void cross_seek_2D_candidate_points(V3DLONG nx, V3DLONG ny, unsigned char *image_binary, vector<V3DLONG> &x_loc, vector<V3DLONG> &y_loc,vector<V3DLONG> &Tip_flag, int &count)
{
    count=0;
    x_loc.clear();
    y_loc.clear();
...
//    cross_y_loc.clear();
    int new_neighbor[20];
    for(V3DLONG j=2;j<ny-2;j++)
    {
        for(V3DLONG i=2;i<nx-2;i++)
        {
            if(image_binary[j*nx+i]>0)
            {
                if (image_binary[(j-2)*nx+i-2] == 255) new_neighbor[1] = 1;
                else  new_neighbor[1] = 0;
                if (image_binary[(j-2)*nx+i-1] == 255) new_neighbor[2] = 1;
                else  new_neighbor[2] = 0;
                if (image_binary[(j-2)*nx+i] == 255) new_neighbor[3] = 1;
                else  new_neighbor[3] = 0;
                if (image_binary[(j-2)*nx+i+1] == 255) new_neighbor[4] = 1;
                else  new_neighbor[4] = 0;
                if (image_binary[(j-2)*nx+i+2] == 255) new_neighbor[5] = 1;
                else  new_neighbor[5] = 0;
                if (image_binary[(j-1)*nx+i+2] == 255) new_neighbor[6] = 1;
                else  new_neighbor[6] = 0;
                if (image_binary[(j)*nx+i+2] == 255) new_neighbor[7] = 1;
                else  new_neighbor[7] = 0;
                if (image_binary[(j+1)*nx+i+2] == 255) new_neighbor[8] = 1;
                else  new_neighbor[8] = 0;
                if (image_binary[(j+2)*nx+i+2] == 255) new_neighbor[9] = 1;
                else  new_neighbor[9] = 0;
                if (image_binary[(j+2)*nx+i+1] == 255) new_neighbor[10] = 1;
                else  new_neighbor[10] = 0;
                if (image_binary[(j+2)*nx+i] == 255) new_neighbor[11] = 1;
                else  new_neighbor[11] = 0;
                if (image_binary[(j+2)*nx+i-1] == 255) new_neighbor[12] = 1;
                else  new_neighbor[12] = 0;
                if (image_binary[(j+2)*nx+i-2] == 255) new_neighbor[13] = 1;
                else  new_neighbor[13] = 0;
                if (image_binary[(j+1)*nx+i-2] == 255) new_neighbor[14] = 1;
                else  new_neighbor[14] = 0;
                if (image_binary[(j)*nx+i-2] == 255) new_neighbor[15] = 1;
                else  new_neighbor[15] = 0;
                if (image_binary[(j-1)*nx+i-2] == 255) new_neighbor[16] = 1;
                else  new_neighbor[16] = 0;
                ...
                int num=abs(new_neighbor[1]-new_neighbor[16]);
                cnp=0.5*(sum+num);
//                if((cnp==2))
//                {
//                    x_loc.push_back(i);
//                    y_loc.push_back(j);
//                    Tip_flag.push_back(1);
//                    count=count+1;
//                }
//                else if((cnp!=1)/*(cnp==3)||(cnp==4)*/)     //  if cnp==3 or  cnp==4   ,the (i,j) point is the 2D candidate point.
//                {
//                    x_loc.push_back(i);
//                    y_loc.push_back(j);
//                    Tip_flag.push_back(0);
//                    count=count+1;
//                }
...
//                }
                ...
                if((cnp==4))
                {
                    x_loc.push_back(i);
                    y_loc.push_back(j);
                    Tip_flag.push_back(2);
                    count=count+1;
                }
                else if((cnp==1))     //  if cnp==3 or  cnp==4   ,the (i,j) point is the 2D candidate point.
                {
                    x_loc.push_back(i);
                    y_loc.push_back(j);
                    Tip_flag.push_back(1);
...
void new_seek_2D_candidate_points(V3DLONG nx, V3DLONG ny, unsigned char *image_binary, vector<V3DLONG> &x_loc, vector<V3DLONG> &y_loc,vector<V3DLONG> &Tip_flag, int &count)
{
    count=0;
    x_loc.clear();
    y_loc.clear();
    ...
    for(V3DLONG j=1;j<ny-1;j++)
    {
        for(V3DLONG i=1;i<nx-1;i++)
        {
            if(image_binary[j*nx+i]>0)
            {
                ...
                int num=abs(new_neighbor[1]-new_neighbor[8]);
                cnp=0.5*(sum+num);
                if((cnp==3)||(cnp==4))     //  if cnp==3 or  cnp==4   ,the (i,j) point is the 2D candidate point.
                {
                    x_loc.push_back(i);
                    y_loc.push_back(j);
                    count=count+1;
