...
struct PARA_APP1: public PARA_VN
{
    ...
    bool app1_dialog()
    {
        ...
        // fetch parameters from dialog
        {
			...
			QSpinBox * OC_spinbox = new QSpinBox();
            OC_spinbox->setRange(0, 100);
            OC_spinbox->setValue(false_th);
            ...
            layout->addWidget(new QLabel("online confidence threshold \n  (normally do not need to change)"),4,0);
            layout->addWidget(OC_spinbox, 4,1,1,5);
            ...
            false_th = OC_spinbox->value();
			cout<<"false_th="<<false_th<<endl;
