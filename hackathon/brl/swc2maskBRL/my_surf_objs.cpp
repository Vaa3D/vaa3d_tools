...
bool readESWC_file(string swc_file, vector<MyMarkerX*> & swc)
{
    ...
    //		cerr<<"Duplicate Node. This is a graph file. Please read it as a graph."<<endl; return false;
        ...
        ifs>> marker->seg_id;
        ifs>> marker->seg_level;
        ifs>> marker->feature;
        parid_map[marker] = par_id;
       // marker->seg_id=
		swc.push_back(marker);
        cout<<"marker= "<<my_id<<"  marker seg_id= "<<marker->seg_id<<endl;
	}
	ifs.close();
...
vector<MyMarkerX*> getLeaf_markers(vector<MyMarkerX*> & inmarkers)
{
    set<MyMarkerX*> par_markers;
    vector<MyMarkerX*> leaf_markers;
	for(int i = 0; i < inmarkers.size(); i++)
	{
        MyMarkerX * marker = inmarkers[i];
...
vector<MyMarkerX*> getLeaf_markers(vector<MyMarkerX*> & inmarkers, map<MyMarkerX *, int> & childs_num)
{
	...
	{
        MyMarkerX * marker = inmarkers[i];
        MyMarkerX * parent = marker->parent;
