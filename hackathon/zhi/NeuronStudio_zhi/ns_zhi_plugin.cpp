...
void autotrace_ns(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    ns_proc_db_construct(&ppdb);
    ns_pixel_proc_db_register_std( &ppdb );

    ns_image_construct(&image);
    ns_image_set_pixel_proc_db( &image, &ppdb );

    ns_image_create( &image, NS_PIXEL_LUM_U8, 512, 512, 1, 1 );
    ns_image_zero( &image );

    ns_pixel_init( &pixel, NS_PIXEL_LUM_U8 );

    curr = ns_image_begin( &image);
    end = ns_image_end( &image);

    ns_pixel_init_lum_u8(&pixel,10);
    ns_image_iter_set_pixel(curr, &pixel );
    //ns_pixel_set_channel_lum_u32( &pixel, ns_pixel_get_channel_lum_u32( &pixel ) );
