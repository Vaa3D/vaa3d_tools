...
bool prediction_caffe::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    {
        cout<<"Welcome to Caffe 3D segmentation combine plugin"<<endl;
        if(infiles.empty())
        {
            cerr<<"Need input image file"<<endl;
            return false;
        ...
        QString trained_file2 = paras.empty() ? "" : paras[k]; if(trained_file2 == "NULL") trained_file2 = ""; k++;
        if(trained_file2.isEmpty())
        {
            cerr<<"Need a second trained_file"<<endl;
            return false;
        }
        ...
        cout<<"model_file = "<<model_file.toStdString().c_str()<<endl;
        cout<<"trained_file = "<<trained_file.toStdString().c_str()<<endl;
        cout<<"trained_file2 = "<<trained_file2.toStdString().c_str()<<endl;
        ...
        Classifier classifier(model_file.toStdString(), trained_file.toStdString(),"");
        Classifier classifier2(model_file.toStdString(), trained_file2.toStdString(),"");
        ...
        for(V3DLONG iz = 0; iz < P; iz = iz+ita_z)
        {
            for(V3DLONG iy = 0; iy < M; iy = iy+ita_y)
            {
                for(V3DLONG ix = 0; ix < N; ix = ix+ita_x)
                {
                    ...
                    std::vector<float> outputs = classifier.Predict_3D(imgs);
                    std::vector<float> outputs2 = classifier2.Predict_3D(imgs);
                    V3DLONG j=0;
                    for(V3DLONG iiz = zb; iiz <= ze; iiz++)
                    {
                        V3DLONG offsetk = iiz*M*N;
                        for(V3DLONG iiy = yb; iiy <= ye; iiy++)
                        {
                            ...
                            for(V3DLONG iix = xb; iix <= xe; iix++)
                            {
                                int tmp_score = (outputs[j]>0.05 || outputs2[j]>0.005)? 1:-1;
                                im_label[offsetk + offsetj + iix] += tmp_score;
                                j++;
                            }
                        }
