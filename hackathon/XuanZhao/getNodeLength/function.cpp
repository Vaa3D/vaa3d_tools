...
void getUtilityL1Infolder(NeuronTree& nt,int maxR,double axonR, double otherR,double thre){

    cout<<"---------------in getUtilityL1Infolder-----------------"<<endl;
...
void getUtilityContour(NeuronTree& nt){
    qDebug()<<"Interval:0-1,1-20,20-100";
    for(int i=0; i<nt.listNeuron.size(); i++){
        ...
        else if(thisRadius>=1&&thisRadius<20)
        {
            nt.listNeuron[i].r = 4;
            nt.listNeuron[i].type=4;
        }
        else if(thisRadius>=20&&thisRadius<=100)
        {
            nt.listNeuron[i].r = 6;
