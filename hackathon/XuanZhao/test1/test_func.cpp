...
int image_threshold(V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    bool ok;
    int c=-1;
    c=QInputDialog::getInteger(parent,"Channel Number","Set the subject channel number:",
                               0,0,p4DImage->getCDim()-1,1,&ok);
    if(!ok)
        return -1;
    ...
    //setData() will free the original memory automatically
 ...
 *                                  [required]
 */
int image_threshold(const V3DPluginArgList &input, V3DPluginArgList &output,V3DPluginCallback2 &callback)
{
