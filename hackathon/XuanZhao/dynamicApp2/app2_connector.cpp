...
bool PARA_APP2::fetch_para_commandline(const V3DPluginArgList &input, V3DPluginArgList &output, V3DPluginCallback2 &callback, QWidget *parent)
{
  ...
  //  outswc_file = outfiles.empty() ? inimg_file + "_app2.swc" : outfiles[0];
    ...
    f_length = (paras.size() >= k+1) ? atoi(paras[k]) : f_length; k++;//50
...
//    outswc.insert(outswc.begin(),inswc.begin(),inswc.end());
    ...
    cout<<"root marker r: "<<rootMarker->radius<<endl;

    vector<vector<V3DLONG> > children = vector<vector<V3DLONG> >(outswc.size(),vector<V3DLONG>());
    map<MyMarker*,V3DLONG> markerMap;
    for(int i=0; i<outswc.size(); i++){
        markerMap[outswc[i]] = i;
    }
    for(int i=0; i<outswc.size(); i++){
        if(outswc[i]->parent == 0)
            continue;
        V3DLONG parIndex = markerMap[outswc[i]->parent];
        children[parIndex].push_back(i);
    }

    vector<V3DLONG> queue;
    queue.push_back(markerMap[rootMarker]);
    while (!queue.empty()) {
        V3DLONG t = queue.front();
        queue.erase(queue.begin());
        outMarkers.push_back(outswc[t]);
        for(int i=0; i<children[t].size(); i++){
            V3DLONG tc = children[t][i];
            MyMarker* p = outswc[tc];
            while (children[tc].size() == 1) {
                if(dist(*p,*rootMarker)>60 && dist(*p,*(outswc[t]))>20){
                    break;
                }else {
            ...
            if(children[tc].size() == 1){
                leafMarkers.push_back(p);
                outMarkers.push_back(p);
            }else if(children[tc].size() > 1){
                queue.push_back(tc);
            }else if(children[tc].size() == 0){
                outMarkers.push_back(p);
            }
