...
bool getMarkerBetween(vector<MyMarker> &allmarkers, MyMarker m1, MyMarker m2)
{
    ...
    for(double t = 0.0; t <= zx_dist(m1, m2); t += 1.0)
    {
        MyMarker marker;
        int cx = x0 + A*t + 0.5;
        int cy = y0 + B*t + 0.5;
...
bool swcTomask(unsigned char * &outimg1d, vector<MyMarker * > & inswc, long sz0, long sz1, long sz2)
{
    ...
    long sz01 = sz0 * sz1;
    if(outimg1d == 0) outimg1d = new unsigned char[tol_sz]; for(long i = 0; i < tol_sz; i++) outimg1d[i] = 0;
//    map<MyMarker*,int> childs_num = map<MyMarker*,int>();
    vector<MyMarker*> leaf_markers = getLeaf_marker(inswc);
    ...
    for(int i = 0; i < leaf_markers.size(); i++)
    {
        ...
        while(visited_markers.find(p) == visited_markers.end() && p->parent != 0)
        {
            ...
            getMarkerBetween(tmp_markers, *p, *par);
