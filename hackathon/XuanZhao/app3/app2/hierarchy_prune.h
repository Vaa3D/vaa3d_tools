...
template<class T> bool hierarchy_prune2(vector<MyMarker*> &inswc, vector<MyMarker*> & outswc, T * inimg1d, long sz0, long sz1, long sz2, double bkg_thresh = 10.0, double length_thresh = 10.0, bool isSoma = true){
    vector<HierarchySegment*> topo_segs;
    swc2topo_segs(inswc, topo_segs, INTENSITY_DISTANCE_METHOD, inimg1d, sz0, sz1, sz2);
    vector<HierarchySegment*> filter_segs;
    MyMarker* root;
    ...
    for(V3DLONG i=0; i<topo_segs.size(); i++){
        ...
        if(isSoma && dist(*leaf_marker,*root) < 2.0*somaR){
            if(topo_segs[i]->length >= somaR*3){
