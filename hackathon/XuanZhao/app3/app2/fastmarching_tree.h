...
template<class T> bool fastmarching_tree_discardCross(MyMarker root,
                                         T * inimg1d,
                                         vector<MyMarker*> &outtree,
                                         long sz0,
                                         long sz1,
                                         ...
                                         bool is_break_accept = false)
{
    ...
    long * parent = 0;
    char * state = 0;
    vector<vector<long> > children = vector<vector<long> >(tol_sz,vector<long>());
    try
    {
        phi = new float[tol_sz];
        parent = new long[tol_sz];
    ...
    double angleThres = sqrt(3.0)/2.0;
    ...
    while(!heap.empty())
    {
        ...
        long prev_ind_t = prev_ind;
        int count = 0;

        float ppi = 0, ppj = 0, ppk = 0;

        while(parent[prev_ind_t] != prev_ind_t){
            prev_ind_t = parent[prev_ind_t];
            ppi += prev_ind_t % sz0;
            ppj += (prev_ind_t/sz0) % sz1;
            ppk += (prev_ind_t/sz01) % sz2;
            count++;
            if(count >=5 )
                break;
        }

        ppi /= (float)count, ppj /= (float)count, ppk /= (float)count;

        XYZ p0 = XYZ(i-pi,j-pj,k-pk);

        XYZ p1 = XYZ(pi-ppi,pj-ppj,pk-ppk);

        if((children[prev_ind].size()>0 || children[parent[prev_ind]].size()>0)&& inimg1d[min_ind]<bifurcation_thres && dot(normalize(p0),normalize(p1))<angleThres){
            state[min_ind] = FAR;
            continue;
        } // judge cross
        ...
        children[prev_ind].push_back(min_ind); //update children
        ...
        for(int kk = -1; kk <= 1; kk++)
        {
            ...
            for(int jj = -1; jj <= 1; jj++)
            {
                ...
                for(int ii = -1; ii <= 1; ii++)
                {
                    ...
                    XYZ p2 = XYZ(w-i,h-j,d-k);
                    double l = dot(normalize(p1),normalize(p2));
                    if(l<0){
                        continue;
                    }
                    double factor2 = 1.0;
//                    double factor2 = 1.0/exp(l);
                    ...
                    if(state[index] != ALIVE)
                    {
                        double new_dist = phi[min_ind] + (GI(index) + GI(min_ind))*factor*0.5*factor2;//consider direction
                        long prev_ind = min_ind;
