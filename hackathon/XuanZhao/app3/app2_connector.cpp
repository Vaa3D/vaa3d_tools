...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
    ...
    {
        v3d_msg("start to use pruning cross program.\n", 0);
        hierarchy_prune2(inswc, outswc, indata1d, in_sz[0], in_sz[1], in_sz[2], p.bkg_thresh, p.length_thresh, true);
...
//                     saveSWC_file(QString(p.p4dImage->getFileName()).append(QString::number(t)).append("_ini.swc").toStdString(), tmpOutTree2, infostring);
        ...
        for(int i=0; i<pointSize; i++){
            ...
            tmpTree.push_back(outswc[i]);
        }

        outswc.clear();
        for(int i=0; i<pointSize; i++){
            if(tmpTree[i]->type == 7){
                outswc.push_back(tmpTree[i]);
            }
        }
        ...
        {
            if (dfactor_xy>1) tmpTree[i]->x *= dfactor_xy;
            tmpTree[i]->x += (p.xc0);
            if (dfactor_xy>1) tmpTree[i]->x += dfactor_xy/2;

            if (dfactor_xy>1) tmpTree[i]->y *= dfactor_xy;
            tmpTree[i]->y += (p.yc0);
            if (dfactor_xy>1) tmpTree[i]->y += dfactor_xy/2;

            if (dfactor_z>1) tmpTree[i]->z *= dfactor_z;
            tmpTree[i]->z += (p.zc0);
            if (dfactor_z>1)  tmpTree[i]->z += dfactor_z/2;

            tmpTree[i]->radius *= dfactor_xy; //use xy for now
        }
        saveSWC_file(QString(p.p4dImage->getFileName()).append("_false.swc").toStdString(), tmpTree, infostring);
        tmpTree.clear();
...
bool proc_multiApp2(V3DPluginCallback2 &callback, PARA_APP2 &p, const QString & versionStr){
  ...
  //        cout << *it <<endl;
      ...
      for(int i=0;i<inmarkers.size();i++){
          ...
          inmarkers.at(i).x = mass_center[0];
          inmarkers.at(i).y = mass_center[1];
          inmarkers.at(i).z = mass_center[2];
          cout << inmarkers.at(i).x << " " << inmarkers.at(i).y << " " << inmarkers.at(i).z << endl;
          cout << "*********************************************" << endl;
      }
...
//          vector<MyMarker> target; target.insert(target.end(), inmarkers.begin()+1, inmarkers.end());
          if(p.is_gsdt)
          {
              ...
              cout<<endl<<"constructing fastmarching tree ..."<<endl;
              fastmarching_tree_wp(inmarkers, phi, indexOfSoma, outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type,p.bkg_thresh,p.is_break_accept);
          }
          else
          {
              switch(datatype)
              {
                  case V3D_UINT8:
                      fastmarching_tree_wp(inmarkers, indata1d, indexOfSoma, outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type,p.bkg_thresh,p.is_break_accept);
                      break;
                  case V3D_UINT16:
                      fastmarching_tree_wp(inmarkers, (short int*) indata1d, indexOfSoma, outtree, in_sz[0], in_sz[1], in_sz[2], p.cnn_type,p.bkg_thresh,p.is_break_accept);
                      break;
              }
          }
      }
