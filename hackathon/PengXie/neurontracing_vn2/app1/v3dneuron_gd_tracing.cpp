...
// Peng Xie's new functions
bool save_mmUnit(vector< vector<V_NeuronSWC_unit> > & mmUnit, int n_end_nodes, QString outswc){
    V_NeuronSWC_list tn_new;
    int nSegsTrace = mergeback_mmunits_to_neuron_path(n_end_nodes, mmUnit, tn_new);
    NeuronTree nt = convertNeuronTreeFormat(tn_new);
    writeSWC_file(outswc, nt);
    return true;
}
vector< vector<V_NeuronSWC_unit> > swc_to_mmUnit(QString input_swc){
    vector< vector<V_NeuronSWC_unit> > mmUnit;
    NeuronTree nt = readSWC_file(input_swc);
    mmUnit = NeuronTree_to_mmUnit(nt);
    return mmUnit;
}
vector< vector<V_NeuronSWC_unit> > NeuronTree_to_mmUnit(NeuronTree nt){
    vector< vector<V_NeuronSWC_unit> > mmUnit;
    int n_components = 0;
    ...
    if(n_components>1)
    {
        // save mmUnit as a vector of components; To be implemented
    ...
    {
        NeuronSWC node = nt.listNeuron.at(j);
								...
								CurveTracePara & trace_para, double trace_z_thickness) 
{
    ...
    save_mmUnit(mmUnit, 0, "test_2.swc");
    return nt;
...
//                {
//                    qDebug()<<i;
//                    for(int j=0; j<mmUnit[i].size(); j++)
//                    {
//                        V_NeuronSWC_unit tu = mmUnit[i].at(j);
//                        qDebug()<<tu.n<<tu.type<<tu.x<<tu.y<<tu.z<<tu.r<<tu.parent<<tu.nchild;
//                    }
//                }
//            }
//			if (1)
