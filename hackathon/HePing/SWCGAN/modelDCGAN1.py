...
def generator(n_nodes=256, noise_dim=100, batch_size=32):
    ...
    geometry_hidden4 = Conv2D(64, kernel_size=(5, 5), padding='same', activation='relu', name='g_conv_1')(
        geometry_hidden3)
    geometry_hidden5 = UpSampling2D(size=(2, 2), name='g_up_2', data_format='channels_last')(geometry_hidden4)
    geometry_hidden6 = Conv2D(32, kernel_size=(5, 5), padding='same', activation='relu', name='g_conv_2')(
        geometry_hidden5)
    ...
    morphology_hidden5 = Conv2D(64, kernel_size=(5, 5), padding='same', activation='relu', name='m_conv_1')(
        morphology_hidden4)
    morphology_hidden6 = UpSampling2D(size=(2, 2), name='m_up_2')(morphology_hidden5)
    morphology_hidden7 = Conv2D(32, kernel_size=(5, 5), padding='same', activation='relu', name='m_conv_2')(
        morphology_hidden6)
    morphology_hidden8 = UpSampling2D(size=(2, 2), name='m_up_3')(morphology_hidden7)
    morphology_hidden9 = Conv2D(16, kernel_size=(5, 5), padding='same', activation='relu', name='m_conv_3')(
        morphology_hidden8)
    ...
    morphology_output = Lambda(layers.masked_softmax,
        ...
        morphology_reshape)  # input mismatch because softmax output is n_nodes-1,n_nodes
...
def discriminator(n_nodes=256, batch_size=32, alpha=0.3):
    ...
    discriminator_model = Model(inputs=[geometry_input, morphology_input], outputs=[discriminator_hidden6],
                                name='discriminator')
...
def discriminator(n_nodes=256, batch_size=4):
    ...
    lambda_args = {'n_nodes': n_nodes, 'batch_size': batch_size}
    both_inputs = merge.concatenate([geometry_input, morphology_input], axis=2)
    n_features = n_nodes+3
    embedding = Reshape(target_shape=(n_nodes, n_features))(both_inputs)
    dis_hid1 = Dense
