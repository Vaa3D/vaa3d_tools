    ...
    for i in range(hp.num_train_epochs):
        ...
        for j in range(num_batchs-1):
            ...
            if j % hp.print_step == 0:
                ...
                label =  sess.run(MODEL.label_ids, feed_dict = fd)
                predictions = sess.run(MODEL.predictions, feed_dict=fd)
                ...
                for id in range(len(label)):
                    F1_Score += f1_score(y_true=label[id], y_pred=predictions[id], labels=None, average="weighted")
                    Recall_Score = recall_score(y_true=label[id], y_pred=predictions[id], labels=None, average="weighted")
                    Precision = precision_score(y_true=label[id], y_pred=predictions[id], labels=None,
                                                 average="weighted")

                print('Time:%s, Epoch:%s, Batch number:%s/%s, Loss:%s'%(time_now_string(),str(i),str(j),str(num_batchs),str(loss)))
                print('F1_Score:', F1_Score/len(label))
                F1.append(F1_Score)
