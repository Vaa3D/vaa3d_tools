...
integratedDataTypes::segUnit::segUnit(const QList<NeuronSWC>& inputSeg) : to_be_deleted(false), segID(0)
{
	this->nodes = inputSeg;
	NeuronStructExplorer::node2loc_node2childLocMap(this->nodes, this->seg_nodeLocMap, this->seg_childLocMap);
	...
	boost::container::flat_set<int> heads;
	for (auto& node : this->nodes)
		if (node.parent == -1) heads.insert(node.n);
	if (heads.size() == 1) this->head = *heads.begin(); // only 1 root node found
	else if (heads.empty()) this->head = -1;            // no root node found
	else this->head = 0;								// multiple root nodes found
