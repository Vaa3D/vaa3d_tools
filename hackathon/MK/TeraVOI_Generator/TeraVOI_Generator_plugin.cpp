...
QStringList TeraVOI_Generator::menulist() const
{
    return QStringList()
        ...
        <<tr("Generate VOI from big image (Terafly)")
        <<tr("about");
}
...
void TeraVOI_Generator::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("Generate VOI from image series"))
    {
        ...
        {
            v3d_msg("Parameter(s) not specified.");
            return;
        }
        ...
        QProgressBar bar;
        bar.setRange(0, list.size());
        bar.setWindowTitle("VOIs generating progress");
        bar.show();
        ...
        for (int ii=0; ii<list.size(); ii++)
        {
            ...
            bar.setValue(ii);
            //------------------------------------- END of [Acquire node coords, collect and crop images, form 3D cubes, save files to the output folder]
        }
    ...
    else if (menu_name == tr("Generate VOI from big image (Terafly)"))
    {
        //------------------------------------------------------------- Load SWC file and generate the folder for VOIs to be saved
        QString SWCfileName;
        SWCfileName = QFileDialog::getOpenFileName(0, QObject::tr("Choose the neuron structure (.swc) for TeraVOIs to be generated based upon"),
                                                   "", QObject::tr("Supported file (*.swc *.eswc)"
                                                   ";;Neuron structure	(*.swc)"
                                                   ";;Extended neuron structure (*.eswc)"));
        QString SaveFolder = SWCfileName + QString("_VOI");
        QDir().mkdir(SaveFolder);
        NeuronTree nt = readSWC_file(SWCfileName);
        QList<NeuronSWC> list = nt.listNeuron;
        //---------------------------------------------------- END of [Load SWC file and generate the folder for VOIs to be saved]
        ...
        QString m_InputfolderName = QFileDialog::getExistingDirectory(parent, QObject::tr("Choose which terafly image fodler to use"),
                                                                      QDir::currentPath(),
                                                                      QFileDialog::ShowDirsOnly);
        ...
        for (int ii=0; ii<list.size(); ii++)
        {
            ...
            int imgX = in_sz[0];
            int imgY = in_sz[1];
            int imgZ = in_sz[2];  // => Get images demention information
            int channel = in_sz[3];
            //cout << imgX << " " << imgY << " " << imgZ << endl;


            x_coord = int(nt.listNeuron.at(ii).x);
            y_coord = int(nt.listNeuron.at(ii).y);
            z_coord = int(nt.listNeuron.at(ii).z);
            if (x_coord - xRadius <= 0)
            {
                xlb = 0;
                xhb = x_coord + xRadius;
            ...
            NeuronTree prunnedTree = cropSWCfile3D(nt, xlb, xhb, ylb, yhb, zlb, zhb, -1);
            QString outimg_fileSWC = SaveFolder + QString("/x%1_y%2_z%3.swc").arg(x_coord).arg(y_coord).arg(z_coord);
            writeSWC_file(outimg_fileSWC, prunnedTree); // Save cropped neuron tree in separate SWC files.
            ...
            unsigned short* VOIPtr = new unsigned short [VOIsz];
            unsigned char* VOIPtr_cnvrt = new unsigned char [VOIsz];
            for (int zi=zlb; zi<=zhb; zi++) Crop3D(ImgPtr, VOIPtr, xlb, xhb, ylb, yhb, zlb, zhb, zi, imgX, imgY, imgZ);

            unsigned short max_v=0, min_v = 65535;
            for(V3DLONG VOIi=0; VOIi<VOIsz; VOIi++)
            {
                if(max_v<VOIPtr[VOIi]) max_v = VOIPtr[VOIi];
                if(min_v>VOIPtr[VOIi]) min_v = VOIPtr[VOIi];
            }
            max_v -= min_v;
            ...
            {
                for(V3DLONG VOIi=0; VOIi<VOIsz; VOIi++)
                    VOIPtr_cnvrt[VOIi] = (unsigned char) 65535*(double)(VOIPtr[VOIi] - min_v)/max_v;
            }
            else
            {
                for(V3DLONG VOIi=0; VOIi<VOIsz; VOIi++)
                    VOIPtr_cnvrt[VOIi] = (unsigned char) VOIPtr[VOIi];
            }
            cout << VOIPtr_cnvrt[15625] << endl;

            QString outimg_file = SaveFolder + QString("/x%1_y%2_z%3.raw").arg(x_coord).arg(y_coord).arg(z_coord);
            string filename = outimg_file.toStdString();
            const char* filenameptr = filename.c_str();
            simple_saveimage_wrapper(callback, filenameptr, VOIPtr_cnvrt, VOIxyz, 1);
            if(VOIPtr) {delete []VOIPtr; VOIPtr = 0;}
            if(VOIPtr_cnvrt) {delete []VOIPtr_cnvrt; VOIPtr_cnvrt = 0;}

            QString outimg_file_linker = SaveFolder + QString("/x%1_y%2_z%3.ano").arg(x_coord).arg(y_coord).arg(z_coord);
            QFile qf_anofile(outimg_file_linker);
            if(!qf_anofile.open(QIODevice::WriteOnly))
            {
                v3d_msg("Cannot open file for writing.",0);
        ...
        QString FinishMsg = QString("All fiels have been saved in ") + SaveFolder;
        v3d_msg(FinishMsg);
