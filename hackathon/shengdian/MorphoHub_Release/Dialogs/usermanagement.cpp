...
UserManagement::UserManagement(mFileSystem imfs, QWidget *parent)
    :QDialog(parent)
{
    this->mfs=imfs;
    userManagementInit();
...
void UserManagement::userManagementInit()
{
    this->setWindowTitle("MorphoHub-User-Management");
    if(mfs.dbPath.isEmpty())
    {
        QMessageBox::warning(this,tr("Path Error"),tr("please set up dbpath"));
        return;
    }
    this->userconfpath=mfs.getUserConfPath();
    logTextEdit=new QTextEdit(this);
    logTextEdit->setText("Welcome to User management");

    Annotator tmpa;
    userconfTitle=tmpa.mFeatures;
    prioritylist=tmpa.muPriority;
    QFile conffile(this->userconfpath);
    if(this->userconfpath.isEmpty()||!conffile.exists())
    {
...
void UserManagement::createTabView()
{
...
void UserManagement::createActions()
{
...
QTableWidget* UserManagement::createTableUserData()
{
    //get every cloum
...
void UserManagement::saveButton_slot()
{
...
void UserManagement::newButton_slot()
{
...
void UserManagement::deleteButton_slot()
{
...
void UserManagement::reviseTab_slot(int r, int c)
{
    if(c>0)
    {
        saveButton->setEnabled(true);
...
void UserManagement::toLog(const QString &logtext)
{
