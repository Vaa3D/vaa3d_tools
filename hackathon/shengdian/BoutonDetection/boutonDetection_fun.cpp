...
void refinement_dofunc(V3DPluginCallback2 & callback, const V3DPluginArgList & input,V3DPluginArgList & output,bool in_terafly){
   ...
   {
       NeuronTree nt_pre=preprocess_simple(nt), nt_int;
       nt_int=reindexNT(nt_pre);
       nt_out = node_interpolation(nt_int, 2);
...
double kd_get_nearest_dist(ANNkd_tree* kd, float x, float y, float z)
{
    ...
    pt[1] = y;
    pt[2] = z;
    ANNidxArray nIdx = new ANNidx[1];
    ANNdistArray dist = new ANNdist[1];
    kd->annkSearch(pt, 1, nIdx, dist);
...
//    NeuronTree nt_raw; nt_raw.deepCopy(nt);
    ...
    QList<NeuronSWC> outNeuron =  QList<NeuronSWC>();
    //load terafly img
    ...
    // get a kd-tree of dendrites
    NeuronTree dendlist;
    for (int i = 0; i < listNeuron.size(); ++i)
    {
        if (listNeuron[i].type != 2)
            dendlist.listNeuron.append(listNeuron[i]);
    }
    ANNkd_tree * kdt = nt_to_kdt(dendlist, dendlist.listNeuron.size());
    ...
    for(V3DLONG i=0;i<siz;i++)
    {
        ...
        if(scanned.at(i)<=0)
        {
            ...
            for(V3DLONG j=0;j<siz;j++){
                NeuronSWC sj = listNeuron.at(j), spar=sj;
                V3DLONG p = sj.parent;
                if (hashNeuron.contains(p))
                    spar = listNeuron.at(hashNeuron.value(p));
                else
                    continue;
                if(scanned.at(j)==0&&
                ...
                {
                    NeuronSWC sj_shifted=sj, out;
                    sj_shifted.x-=start_x; sj_shifted.y-=start_y; sj_shifted.z-=start_z;
                    spar.x -= start_x; spar.y -= start_y; spar.z -= start_z;
                    if(sj_shifted.z * sz01 + sj_shifted.y* sz0 + sj_shifted.x > pagesz){
                        cout<<"point out of image size, index="<<sj_shifted.n<<endl;
                        return;
                    }
                    int windowradius = refine_radius;
                    if (sj.type == 2)
                    {
                        int min_dis = sqrt(kd_get_nearest_dist(kdt, sj.x, sj.y, sj.z));
                        windowradius = min(windowradius, max(min_dis, 1));
                    }
                    else
                        windowradius /= 2;
                    if(method_code==NeuronTreeRefine)
                        out=calc_mean_shift_center(inimg1d,sj_shifted,spar,in_sz, windowradius);
    ...
    for(V3DLONG i=0;i<outNeuron.size();i++){
        NeuronSWC s = listNeuron[hashNeuron[outNeuron[i].n]];
        s.x=outNeuron[i].x;
        s.y=outNeuron[i].y;
        s.z=outNeuron[i].z;
    }
...
void refinement_Image_fun(V3DPluginCallback2 &callback,string inimg_file, NeuronTree& nt,
                          int method_code,int refine_radius,int nodeRefine_radius){
    ...
    {
        //for all the node
        NeuronSWC s = listNeuron.at(i), spar=s;
        V3DLONG p = s.parent;
        if (hashNeuron.contains(p))
            spar = listNeuron[hashNeuron[p]];
        NeuronSWC out;
        if(method_code==NeuronTreeRefine)
            out=calc_mean_shift_center(inimg1d,s,spar,in_sz, refine_radius);
        else if(method_code==NodeRefine)
   ...
   cout<<"Refine finished"<<endl;
    for(V3DLONG i=0;i<outNeuron.size();i++){
        listNeuron[i].x=outNeuron[i].x;
        listNeuron[i].y=outNeuron[i].y;
        listNeuron[i].z=outNeuron[i].z;
    }
    if(inimg1d) {delete []inimg1d; inimg1d=0;}
    if(inimg1d_raw) {delete []inimg1d_raw; inimg1d_raw=0;}
}
...
NeuronSWC calc_mean_shift_center(unsigned char * & inimg1d,NeuronSWC snode, NeuronSWC spar, V3DLONG sz_image[], int refine_radius)
{
    ...
    V3DLONG y_offset=sz_image[0];
    V3DLONG z_offset=sz_image[0]*sz_image[1];
    double tan[] = {snode.x - spar.x, snode.y - spar.y, snode.z - spar.z};
    double tanlen = sqrt(tan[0]*tan[0] + tan[1]*tan[1] + tan[2]*tan[2]);


    double total_x,total_y,total_z,sum_v,x,y,z, center_dist;
    int windowradius = refine_radius;

    // find the best windowradius, zuohan
    {
        double mean = 0., count = 0.;
        for(V3DLONG X=MAX(x+0.5-windowradius,0); X<=MIN(sz_image[0]-1,x+0.5+windowradius); X++)
            for(V3DLONG Y=MAX(y+0.5-windowradius,0); Y<=MIN(sz_image[1]-1,y+0.5+windowradius); Y++)
                for(V3DLONG Z=MAX(z+0.5-windowradius,0); Z<=MIN(sz_image[2]-1,z+0.5+windowradius); Z++){
                    double dx = X - x;
                    double dy = Y - y;
                    double dz = Z - z;
                    ...
                    if (distance > windowradius)
                        continue;
                    double orient = 1.;
//                    if (tanlen > 1e-5 && distance > 1){
//                        auto&& temp = abs(tan[0] * dx + tan[1] * dy + tan[2] * dz) / (tanlen * distance);
//                        orient -= min(temp * temp, 1.);
//                    }
                    V3DLONG g = inimg1d[Z*z_offset + Y*y_offset + X];
                    double w = g * g * orient;
                    mean += w * distance;
                    count += w;
                }
        mean /= count + 1e-5;
        windowradius = max(1., mean);
    }

    double center[3] = {snode.x, snode.y, snode.z};
    center[0] = x = snode.x;
    center[1] = y = snode.y;
    center[2] = z = snode.z;
    ...
    //find out the channel with the maximum intensity for the marker
    do {
        double sigma = windowradius / 4.;
        double sigma2 = sigma * sigma;
        double prefactor = 1.0 / (std::pow(2 * M_PI * sigma2, 1.5));
        total_x = total_y = total_z = sum_v = 0.;
            ...
            for(V3DLONG Y=MAX(y+0.5-windowradius,0); Y<=MIN(sz_image[1]-1,y+0.5+windowradius); Y++)
                for(V3DLONG Z=MAX(z+0.5-windowradius,0); Z<=MIN(sz_image[2]-1,z+0.5+windowradius); Z++){
                    ...
                    if (distance > windowradius)
                        continue;
                    // zuohan modify
                    double gauss = prefactor * std::exp(-distance / (2 * sigma2));
                    double orient = 1.;
//                    if (tanlen > 1e-5 && distance > 1){
//                        auto&& temp = abs(tan[0] * dx + tan[1] * dy + tan[2] * dz) / (tanlen * distance);
//                        orient -= min(temp * temp, 1.);
//                    }
                    ...
                    V3DLONG pos = Z*z_offset + Y*y_offset + X;
                    double w = orient * gauss * inimg1d[pos] * inimg1d[pos];
                    total_x += w * X;
                    total_y += w * Y;
                    total_z += w * Z;
                    sum_v += w;
        ...
        center_dist=sqrt((center[0]-x)*(center[0]-x) + (center[1]-y)*(center[1]-y) + (center[2]-z)*(center[2]-z));
        x=center[0];
        y=center[1];
        z=center[2];
        windowradius -= sigma;
    ...
    NeuronSWC out_center = snode;
    out_center.x=center[0]; out_center.y=center[1]; out_center.z=center[2];
    out_center.level=inimg1d[V3DLONG(out_center.z)*z_offset+V3DLONG(out_center.y)*y_offset+V3DLONG(out_center.x)];
    return out_center;
}

NeuronSWC calc_mean_shift_center_old(unsigned char * & inimg1d,NeuronSWC snode,V3DLONG sz_image[], double bkg_thre,int windowradius)
{
    //qDebug()<<"methodcode:"<<methodcode;
    V3DLONG y_offset=sz_image[0];
    V3DLONG z_offset=sz_image[0]*sz_image[1];
...
void boutonFilter_dofunc(V3DPluginCallback2 & callback, const V3DPluginArgList & input,V3DPluginArgList & output){
    ...
    */
    ...
    //read para list
    double radius_delta=(inparas.size()>=1)?atof(inparas[0]):2;
    double intensity_delta=(inparas.size()>=2)?atof(inparas[1]):1.2;
...
QList <AxonalBouton> boutonFilter_fun(NeuronTree nt,double radius_delta,double intensity_delta,double AXON_BACKBONE_RADIUS){
     ...
     * for each branch:
            ...
            * merge and filter
    */
    int MIN_PROCESSED_SEG_LEN=20; int MAX_PROCESSED_SEG_LEN=300;
    ...
    vector<double> seg_size,seg_size_feas;
    int max_seg_len=0;
    int max_seg_len_index=0;
    for(int i=0;i<nt_nslist.seg.size();i++)
    {
        V_NeuronSWC curseg=nt_nslist.seg.at(i);
        seg_size.push_back(nt_nslist.seg.at(i).row.size());
        if(curseg.row.size()>max_seg_len)
            max_seg_len_index=i;
//        if(nt_nslist.seg.at(i).row.size()<MIN_PROCESSED_SEG_LEN)
//            MIN_PROCESSED_SEG_LEN=nt_nslist.seg.at(i).row.size();
    }
    double axon_BACKBONE_RADIUS=0;
    for(int io=0;io<nt_nslist.seg.at(max_seg_len_index).row.size();io++){
        axon_BACKBONE_RADIUS+=nt_nslist.seg.at(max_seg_len_index).row.at(io).r;
    }
    axon_BACKBONE_RADIUS=double(axon_BACKBONE_RADIUS/nt_nslist.seg.at(max_seg_len_index).row.size());
    AXON_BACKBONE_RADIUS=MAX(AXON_BACKBONE_RADIUS,axon_BACKBONE_RADIUS);
    cout<<"AXON_BACKBONE_RADIUS ="<<AXON_BACKBONE_RADIUS<<endl;
//    MIN_PROCESSED_SEG_LEN*=1.2;
    seg_size_feas=mean_and_std_seg_fea(seg_size);
//    cout<<"segment min ="<<MIN_PROCESSED_SEG_LEN<<endl;
    MAX_PROCESSED_SEG_LEN=MAX(MAX_PROCESSED_SEG_LEN,seg_size_feas[0]+3*seg_size_feas[1]);
    cout<<"segment max="<<MAX_PROCESSED_SEG_LEN<<endl;
    cout<<"segment size mean ="<<seg_size_feas[0]<<endl;
    cout<<"segment size std="<<seg_size_feas[1]<<endl;
//    MIN_PROCESSED_SEG_LEN=seg_size_feas[0];
    for(int i=0;i<nt_nslist.seg.size();i++)
    {
        V_NeuronSWC curseg=nt_nslist.seg.at(i);
        ...
        double bouton_radius_thre=radius_delta*radius_mean;
        if(curseg.row.size()>=MAX_PROCESSED_SEG_LEN)
            bouton_radius_thre=MIN(radius_mean*(radius_delta+3),8); //filter out axonal long projection branch
        else{
            if(radius_mean>=AXON_BACKBONE_RADIUS){
                bouton_radius_thre=MAX(radius_mean*MIN(radius_delta,2),radius_mean+radius_std); //normal branch size but huge std
                if((radius_mean-radius_std)>=1)
                    bouton_radius_thre=MIN(radius_mean*(radius_delta+3),8); // filter out projection branch with a small std
            }
            if(MIN_PROCESSED_SEG_LEN>=curseg.row.size())
                bouton_radius_thre=MAX(AXON_BACKBONE_RADIUS,radius_mean*radius_delta); //bouton in small branch
        }
        bouton_radius_thre=MIN(MAX(bouton_radius_thre,1),10); //bouton radius >=1

        std::vector<int> outflag_radius=peaks_in_seg(seg_radius,1,0*radius_std);
        ...
        bouton_intensity_thre=MIN(MAX(bouton_intensity_thre,60),250);
...
std::vector<int> peaks_in_seg(std::vector<double> input,int isRadius_fea, float delta)
{
    ...
    */
    ...
    double bro_factor=(isRadius_fea)?0.3:0.3;
...
void boutonswc_pruning_dofunc(V3DPluginCallback2 & callback, const V3DPluginArgList & input,V3DPluginArgList & output){
    ...
    string inswc_file;
    if(infiles.size()>=1) {inswc_file = infiles[0];}
    int ccf_domain_flag=(inparas.size()>=1)?atoi(inparas[0]):1;
    float pruning_vol_r=(inparas.size()>=2)?atof(inparas[1]):500.0;
    float pruning_vol_num=(inparas.size()>=3)?atof(inparas[2]):5.0;
    float pruning_tip_thre=(inparas.size()>=4)?atof(inparas[3]):3.0;

    bool ccf_domain=(ccf_domain_flag>1)?true:false;
    NeuronTree nt = readSWC_file(QString::fromStdString(inswc_file));
    if(!nt.listNeuron.size()) return;
    V3DLONG somaid=get_soma(nt);
    if(somaid<0) {cout<<"no soma node"<<endl;return;}
    string out_nt_filename=(outfiles.size()>=1)?outfiles[0]:(inswc_file + "_pruned.eswc");
    /*pruning very small branches*/
    float min_bouton_dist=10;
    QList<double> topobdists= interbouton_dist(nt,ccf_domain);
    bool isnearbouton=false;
    for(V3DLONG i=0;i<topobdists.size();i++)
        if(topobdists.at(i)<min_bouton_dist)
            isnearbouton=true;
    uint max_pruning_times=3;
    uint pruning_index=0;
    while(isnearbouton&&pruning_index<max_pruning_times){
        nearBouton_pruning(nt,min_bouton_dist,ccf_domain);
        topobdists.clear(); isnearbouton=false;pruning_index++;
        topobdists= interbouton_dist(nt,ccf_domain);
        for(V3DLONG i=0;i<topobdists.size();i++)
            if(topobdists.at(i)<min_bouton_dist)
                isnearbouton=true;
    }
    sparseBouton_pruning(nt,pruning_vol_r,pruning_vol_num,false);
    float pruning_thre=1;
//    NeuronTree nt_internode_pruning=boutonSWC_internode_pruning(nt,pruning_thre,ccf_domain);
    NeuronTree nt_bouton_tip_pruning=tipNode_pruning(nt,pruning_tip_thre,ccf_domain);
        ...
        for(V3DLONG ib=0;ib<siz;ib++){
            ...
            if(ccf_domain)
                b_dist=sqrt((s.fea_val.at(0)-ss.fea_val.at(0))*(s.fea_val.at(0)-ss.fea_val.at(0))+
                                 (s.fea_val.at(1)-ss.fea_val.at(1))*(s.fea_val.at(1)-ss.fea_val.at(1))+
                                 (s.fea_val.at(2)-ss.fea_val.at(2))*(s.fea_val.at(2)-ss.fea_val.at(2)));
            else
                b_dist=dis(s,ss);
            ...
            while(true){
                if(ccf_domain)
                    dist_to_parent_bouton+=sqrt((s.fea_val.at(0)-sp.fea_val.at(0))*(s.fea_val.at(0)-sp.fea_val.at(0))+
                                                (s.fea_val.at(1)-sp.fea_val.at(1))*(s.fea_val.at(1)-sp.fea_val.at(1))+
                                                (s.fea_val.at(2)-sp.fea_val.at(2))*(s.fea_val.at(2)-sp.fea_val.at(2)));
                else
...
NeuronTree boutonSWC_internode_pruning(NeuronTree nt,float pruning_dist,bool ccf_domain){
    ...
    */
    ...
    V3DLONG somaid=get_soma(nt);
    if(somaid<0) {cout<<"no soma node"<<endl;return out;}
    QHash <V3DLONG, V3DLONG>  hashNeuron;hashNeuron.clear();
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        hashNeuron.insert(s.n,i);
...
void bouton_feature_dofunc(V3DPluginCallback2 & callback, const V3DPluginArgList & input,V3DPluginArgList & output){
    ...
    */
    ...
    int ccf_domain_flag=(inparas.size()>=3)?atoi(inparas[2]):1;
    ...
    if(xy_res>0){
        ...
        boutonDesity_label(nt,xy_res,ccf_domain);
        boutonVolDesity_computing(nt,vol_radius,xy_res,ccf_domain);
        //4. distance
        dist_to_soma_simple(nt,ccf_domain);
        //5.
        for(V3DLONG i=0;i<siz;i++){
...
void boutonType_label(NeuronTree& nt,bool ccf_domain){
    ...
    */
    int btype_index=3;
    float max_terminaux_bouton_branch_len=10.0;
...
QList<int> boutonType(NeuronTree nt,bool ccf_domain){
    /*1. tip bouton: boutonType+=1
     * 2. terminaux bouton: boutonType+=1
        * for all the tip branch whose length are less than max_terminaux_bouton_branch_len,
        * if there only exist one bouton at the tip, this is Terminaux bouton.
...
//    int btype_index=3;
    ...
    if(ccf_domain)
        max_terminaux_bouton_branch_len=8.0;
    QList<int> out_list;
    V3DLONG siz=nt.listNeuron.size();
    if(siz==0){return out_list;}
    QHash <V3DLONG, V3DLONG>  hashNeuron;
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        if(s.type==5)
...
QList<int> neighbor_boutons(NeuronTree nt,float vol_r,bool ccf_domain){
    QList<int> outlist;
    V3DLONG siz=nt.listNeuron.size();
    if(siz<=0){return outlist;}
    for(V3DLONG i=0;i<siz;i++)
        outlist.append(0);
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        if(s.parent<0||
                !nt.hashNeuron.contains(s.parent)||
                s.type!=5)
            continue;
        double vol_density=0;
        for(V3DLONG ib=0;ib<siz;ib++){
            NeuronSWC ss = nt.listNeuron.at(ib);
            if(ss.parent<0||
                    !nt.hashNeuron.contains(ss.parent)||
                    ss.type!=5)
                continue;
            double b_dist=0.0;
            if(ccf_domain)
                ...
                b_dist=sqrt((s.x-ss.x)*(s.x-ss.x)+
                                 (s.y-ss.y)*(s.y-ss.y)+
                                 (s.z-ss.z)*(s.z-ss.z));
            if(b_dist<=vol_r)
                vol_density+=1.0;
        }
        outlist[i]=vol_density-1;
    }
    return outlist;
}
QList<double> topo_neighbor_bouton_dist(NeuronTree nt,bool ccf_domain){
    /* min dist2parent-bouton / dist2child-bouton
    */
        ...
        if(ntype.at(i)==0){
            ...
            while(true){
                if(ccf_domain)
                    ...
                    t_density+=sqrt((s.x-sp.x)*(s.x-sp.x)+
                                     (s.y-sp.y)*(s.y-sp.y)+
                                     (s.z-sp.z)*(s.z-sp.z));
                if(ntype.at(pid)>=2||
                        sp.parent<0||
                        !hashNeuron.contains(sp.parent)||
                        sp.type==5)
                {
                    break;
                }
            ...
            if(sp.type==5)
                if(outlist.at(pid)>t_density||
                        outlist.at(pid)==0)
                    outlist[pid]=t_density;
        }
    }
    //step one: dist2child-bouton
    // for each bouton, compute its distance to parent-bouton and record in parent-out-dist
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        if(s.parent<0||!hashNeuron.contains(s.parent))
            continue;
    ...
    //step two: dist2-parent bouton
    // for each bouton, compute its distance to parent-bouton and update self-out-dist
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        if(s.parent<0||!hashNeuron.contains(s.parent))
            continue;
...
QList<double> interbouton_dist(NeuronTree nt,bool ccf_domain){
    /* distance to parent-bouton or parent-bifurcation
    */
    QList<double> outlist;
...
QList<double> seg_contraction(NeuronTree nt,double seg_len,bool ccf_domain){
    double seg_half_len=seg_len/(double)2;
    QList<double> outlist;
    ...
    for(V3DLONG i=0;i<siz;i++){
        ...
        if(s.parent<0||!hashNeuron.contains(s.parent))
            continue;
        QList<V3DLONG> tipseg_nids; tipseg_nids.clear();
        if(ntype.at(i)==0)
            tipseg_nids.append(i);
        V3DLONG pid=hashNeuron.value(s.parent);
        NeuronSWC sp=nt.listNeuron.at(pid);
        double get_seg_len=0.0;
        V3DLONG mid_s=i;
        while(true){
            if(ccf_domain)
                get_seg_len+=sqrt((s.fea_val.at(0)-sp.fea_val.at(0))*(s.fea_val.at(0)-sp.fea_val.at(0))+
                                 (s.fea_val.at(1)-sp.fea_val.at(1))*(s.fea_val.at(1)-sp.fea_val.at(1))+
                                 (s.fea_val.at(2)-sp.fea_val.at(2))*(s.fea_val.at(2)-sp.fea_val.at(2)));
            else
                get_seg_len+=sqrt((s.x-sp.x)*(s.x-sp.x)+
                                 (s.y-sp.y)*(s.y-sp.y)+
                                 (s.z-sp.z)*(s.z-sp.z));
            if(get_seg_len>=seg_half_len&&mid_s==i)
                mid_s=pid;
            if(ntype.at(i)==0&&get_seg_len<seg_half_len)
                tipseg_nids.append(pid);
            if(ntype.at(pid)>2||
                    sp.parent<0||
                    !hashNeuron.contains(sp.parent)||
                    get_seg_len>=seg_len)
        ...
        if(get_seg_len>=seg_len){
            double edist=0.0;
            s = nt.listNeuron.at(i);
            if(ccf_domain)
                edist=sqrt((s.fea_val.at(0)-sp.fea_val.at(0))*(s.fea_val.at(0)-sp.fea_val.at(0))+
                                 (s.fea_val.at(1)-sp.fea_val.at(1))*(s.fea_val.at(1)-sp.fea_val.at(1))+
                                 (s.fea_val.at(2)-sp.fea_val.at(2))*(s.fea_val.at(2)-sp.fea_val.at(2)));
            else
                edist=sqrt((s.x-sp.x)*(s.x-sp.x)+
                                 (s.y-sp.y)*(s.y-sp.y)+
                                 (s.z-sp.z)*(s.z-sp.z));
            double protential_out=edist/get_seg_len;
            if(protential_out>outlist.at(mid_s))
                outlist[mid_s]=protential_out;
            if(ntype.at(i)==0)
                for(int t=0;t<tipseg_nids.size();t++)
                    outlist[tipseg_nids.at(t)]=protential_out;
        }
    }
    return outlist;
}
void boutonDesity_label(NeuronTree& nt,float xy_res,bool ccf_domain){
    /* 0. get bouton type
...
void dist_to_soma_simple(NeuronTree& nt,bool ccf_domain){
    V3DLONG siz=nt.listNeuron.size();    if(siz<=0){return;}
    V3DLONG somaid=get_soma(nt);
    if(somaid<0) {cout<<"no soma node"<<endl;return;}
//    QHash <V3DLONG, V3DLONG>  hashNeuron=nt.hashNeuron;

    NeuronSWC sn=nt.listNeuron.at(somaid);
    for(V3DLONG i=0;i<siz;i++){
        NeuronSWC s = nt.listNeuron.at(i);
        NeuronSWC sraw=s;
        ...
        if(s.parent>=0&&
                nt.hashNeuron.contains(s.parent))
        {
            V3DLONG pid=nt.hashNeuron.value(s.parent);
            NeuronSWC sp=nt.listNeuron.at(pid);
            ...
            while(true){
                if(ccf_domain)
                    ...
                    path_length+=sqrt((s.x-sp.x)*(s.x-sp.x)+
                                     (s.y-sp.y)*(s.y-sp.y)+
                                     (s.z-sp.z)*(s.z-sp.z));

                if(sp.parent<0||!nt.hashNeuron.contains(sp.parent))
                    break;
                //update node
                s=sp;
                pid=nt.hashNeuron.value(s.parent);
                sp=nt.listNeuron.at(pid);
            }
            if(ccf_domain)
                euler_dist=sqrt((sn.fea_val.at(0)-sraw.fea_val.at(0))*(sn.fea_val.at(0)-sraw.fea_val.at(0))+
                                (sn.fea_val.at(1)-sraw.fea_val.at(1))*(sn.fea_val.at(1)-sraw.fea_val.at(1))+
                                (sn.fea_val.at(2)-sraw.fea_val.at(2))*(sn.fea_val.at(2)-sraw.fea_val.at(2)));
            else
                euler_dist=sqrt((sn.x-sraw.x)*(sn.x-sraw.x)+
                                 (sn.y-sraw.y)*(sn.y-sraw.y)+
                                 (sn.z-sraw.z)*(sn.z-sraw.z));
        }
        nt.listNeuron[i].fea_val.append(path_length);
        nt.listNeuron[i].fea_val.append(euler_dist);
    }
}
/*File io*/
...
//        QList<CellAPO> apoboutons=bouton_to_apo(nt);
    ...
    case 2:
    {
         ...
         * neighbor boutons
         * dist2soma{path and Euclidean}
        */

        float radius_scale=(inparas.size()>=2)?atof(inparas[1]):1.0;
        int ccf_domain=(inparas.size()>=3)?atoi(inparas[2]):1;
        float vol_radius=(inparas.size()>=4)?atof(inparas[3]):(100.0);
        double split_seg_len=(inparas.size()>=5)?atof(inparas[4]):(20.0);

        V3DLONG somaid=get_soma(nt);
        if(somaid<0) {cout<<"no soma node"<<endl;return;}
        cout<<"radius scale="<<radius_scale<<endl;
        V3DLONG siz=nt.listNeuron.size();
        NeuronTree out;
        for(V3DLONG i=0;i<siz;i++)
        {
            ...
            s.fea_val.clear();
            for(int f=0;f<3;f++)
                s.fea_val.append(nt.listNeuron[i].fea_val[f]);
            if(radius_scale>0)
                s.fea_val.append(s.r*radius_scale);
            out.listNeuron.append(s);
            out.hashNeuron.insert(s.n,out.listNeuron.size()-1);
        }

        //3. bouton_density feature to fea_val_list
        QList<int> btypelist= boutonType(out,ccf_domain);
        for(V3DLONG i=0;i<out.listNeuron.size();i++)
            out.listNeuron[i].fea_val.append(btypelist.at(i));
        QList<double> bdists= interbouton_dist(out,ccf_domain);
        for(V3DLONG i=0;i<out.listNeuron.size();i++)
            out.listNeuron[i].fea_val.append(bdists.at(i));
        QList<int> nbs= neighbor_boutons(out,vol_radius,ccf_domain);
        for(V3DLONG i=0;i<out.listNeuron.size();i++)
            out.listNeuron[i].fea_val.append(nbs.at(i));
//        cout<<"here"<<endl;
        //4. distance
        dist_to_soma_simple(out,ccf_domain);
        QList<double> topobdists= topo_neighbor_bouton_dist(out,ccf_domain);
        for(V3DLONG i=0;i<out.listNeuron.size();i++)
            out.listNeuron[i].fea_val.append(topobdists.at(i));
        QList<double> seg_con= seg_contraction(out,split_seg_len,ccf_domain);
        for(V3DLONG i=0;i<out.listNeuron.size();i++)
            out.listNeuron[i].fea_val.append(seg_con.at(i));
        QString ccfname=(outfiles.size()>=1)?outfiles[0]:(inswc_file+ "_bouton.eswc");
        writeESWC_file(ccfname,out);
    }
        break;
    case 3:
    {
    ...
    case 6:
    {
        ...
        int to_swc=(inparas.size()>=3)?atof(inparas[2]):0;
        ...
        if(nt_out.listNeuron.size())
        {
            if(to_swc)
                writeSWC_file(outname,nt_out);
            else
    ...
    case 8:
    {
        /*compact bouton swc: ESWC+CCF+bouton_type+CCF_Radius*/
        float radius_scale=(inparas.size()>=2)?atof(inparas[1]):1.0;
        V3DLONG siz=nt.listNeuron.size();
        NeuronTree out;
        for(V3DLONG i=0;i<siz;i++)
        {
            ...
            for(int f=0;f<4;f++)
                s.fea_val.append(nt.listNeuron[i].fea_val[f]);
            if(radius_scale!=1&&radius_scale>0)
                s.fea_val.append(s.r*radius_scale);
            out.listNeuron.append(s);
            out.hashNeuron.insert(s.n,out.listNeuron.size()-1);
        }
    ...
    case 9:
    {
        ...
        */
//        float vol_radius=(inparas.size()>=2)?atof(inparas[1]):(50.0);
        int ccf_domain=(inparas.size()>=2)?atoi(inparas[1]):1;
        V3DLONG somaid=get_soma(nt);
        if(somaid<0) {cout<<"no soma node"<<endl;return;}
        //3. bouton_density feature to fea_val_list
        QList<int> btypelist= boutonType(nt,ccf_domain);
        for(V3DLONG i=0;i<nt.listNeuron.size();i++)
            nt.listNeuron[i].fea_val.append(btypelist.at(i));
        QList<double> bdists= interbouton_dist(nt,ccf_domain);
        for(V3DLONG i=0;i<nt.listNeuron.size();i++)
            nt.listNeuron[i].fea_val.append(bdists.at(i));
        QList<int> nbs= neighbor_boutons(nt,ccf_domain);
        for(V3DLONG i=0;i<nt.listNeuron.size();i++)
            nt.listNeuron[i].fea_val.append(nbs.at(i));
        //4. distance
        dist_to_soma_simple(nt,ccf_domain);
...
//            nt.listNeuron[i].type+=nt.listNeuron[i].fea_val[3];
            nt.listNeuron[i].r=1/*nt.listNeuron[i].fea_val[4]/scale*/;
        }
    }
}
