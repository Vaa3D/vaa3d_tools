...
QStringList combine_file::menulist() const
{
	return QStringList() 
        ...
        <<tr("combine all SWC or ESWC files under a directory with diff")
...
void combine_file::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
    ...
    else if(menu_name == tr("combine all SWC or ESWC files under a directory with diff"))
    {
        generatorcombined4FilesInDir(callback,parent,3);
    }
...
void changeMarkerPositionToNearestSWC(vector<MyMarker*> inputSwc,MyMarker* changeMarker)
{
    double minDistance=0;int nearstNumber;int moveNumber=0;
    if(inputSwc.size()>0)
    {
        //find Nearest Marker number
        for(vector<MyMarker*>::iterator it=inputSwc.begin();it!=inputSwc.end();it++)
        {
            MyMarker* inputMarker=*it;
            if(it==inputSwc.begin())
            {
                minDistance=distanceofMarker(inputMarker,changeMarker);
                nearstNumber=moveNumber;
            }
            else
            {
                if(minDistance>distanceofMarker(inputMarker,changeMarker))
                {
                    minDistance=distanceofMarker(inputMarker,changeMarker);
                    nearstNumber=moveNumber;
                }
        ...
        moveNumber=0;
        //change changeMarker to nearest marker position
        for(vector<MyMarker*>::iterator it=inputSwc.begin();it!=inputSwc.end();it++)
        {
            MyMarker* inputMarker=*it;
            if(nearstNumber==moveNumber)
            {
                changeMarker->x=inputMarker->x;
                changeMarker->y=inputMarker->y;
                changeMarker->z=inputMarker->z;
                break;
            }
            moveNumber++;
        }
...
//version3:
vector<MyMarker*> mergeSWCsorESWCs(vector<MyMarker*> inputSwc1,//inputSwc nodes merge to targetSwc nodes
                                       vector<MyMarker*> inputSwc2)
{
    ...
    int _countnumber=0;int flag[inputSwc.size()],_flag[inputSwc.size()];
    map<MyMarker*,double> marker2SwcDistance;
    if(miok)
    {
        ...
        for(vector<MyMarker*>::iterator inp=inputSwc.begin();inp!=inputSwc.end();inp++)
        {
            ...
            for(vector<MyMarker*>::iterator tari=targetSwc.begin();tari!=targetSwc.end();tari++)
            {
                ...
                if(tari==targetSwc.begin())
                {
                    flag[_countnumber]=distanceofMarker(targetMarkerT,inputMarker);
                }
                else
                {
                    if(flag[_countnumber]>distanceofMarker(targetMarkerT,inputMarker))
                    {
                        flag[_countnumber]=distanceofMarker(targetMarkerT,inputMarker);
                    }
                }
            ...
            for(vector<XYZ>::iterator interpolationp=interpolationPosition.begin();interpolationp!=interpolationPosition.end();interpolationp++)
            {
                ...
                if(distanceofMarker2point(inputMarker,interpolationposition)<flag[_countnumber])
                {
                    flag[_countnumber]=distanceofMarker2point(inputMarker,interpolationposition);
                }
            }
            marker2SwcDistance[inputMarker]=flag[_countnumber];
            _countnumber++;
        }
        _countnumber=0;
        for(vector<MyMarker*>::iterator inp=inputSwc.begin();inp!=inputSwc.end();inp++)
        {
            ...
            MyMarker* inputMarker=*inp;
            if(marker2SwcDistance[inputMarker]<distanceThreshold)
            {
                if(inputMarker->parent!=0&&marker2SwcDistance[inputMarker->parent]>=distanceThreshold)
                {
                    changeMarkerPositionToNearestSWC(targetSwc,inputMarker);
...
void generatorcombined4FilesInDir(V3DPluginCallback2 &callback, QWidget *parent, int method_code)
{
    ...
    {
        QString m_InputfolderName = QFileDialog::getExistingDirectory(parent, QObject::tr("Choose the directory including all swc and eswc files (to diff)"),
                                                                      QDir::currentPath(),
                                                                      QFileDialog::ShowDirsOnly);
        ...
        for(V3DLONG i = 0; i < swcList.size(); i++)
        {
            ...
            {
                MyMarker *inputMarker=*it;
                inputMarker->type=i+2;
            }
            if(i>0&&outswc.size()>0)
            {
                mergeswc=mergeSWCsorESWCs(outswc,inputswc);
            }
            else
            {
                mergeswc=inputswc;
