...
bool WriteSomalistConfToFile(const QString& confpath,QList<SomaConfInfo> &inputsdlist)
{
    ...
    if(scanconffile.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        scanconffile.write(confTitle.toLatin1());
        //inside
        for(int i=0;i<inputsdlist.size();i++)
        {
            ...
            data+="\n";
            scanconffile.write(data.toLatin1());
        }
        scanconffile.close();
        return true;
    }
...
QList<Handler> getAnnotatorlist(const QString& confpath)
{
    QList<Handler> outlist;
    outlist.clear();
    //1.scan conf path.
    QFile scanconffile(confpath);
    if(!scanconffile.exists())
...
bool WriteAnnotatorConfToFile(const QString& confpath,QList<Handler> &inputsdlist)
{
    if (confpath.isEmpty())
        return false;
    QFile scanconffile(confpath);
    ...
    if(scanconffile.open(QIODevice::WriteOnly | QIODevice::Text))
    {
        ...
        for(int i=0;i<inputsdlist.size();i++)
        {
            Handler tempsdi=inputsdlist.at(i);
