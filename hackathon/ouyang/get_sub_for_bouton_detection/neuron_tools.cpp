...
bool vectorMyMarkerX2vectorMyMarker(vector<MyMarkerX*> inputMarkers, vector<MyMarker*> & outmarkers)
{
    ...
    for(int i = 0; i < inputMarkers.size(); i++) ind[inputMarkers[i]] = i;

    for(int i = 0; i < inputMarkers.size(); i++)
    {
        MyMarker * marker = new MyMarker;
        marker->x=inputMarkers.at(i)->x;
        marker->y=inputMarkers.at(i)->y;
        marker->z=inputMarkers.at(i)->z;
        marker->radius=inputMarkers.at(i)->radius;
        marker->type=inputMarkers.at(i)->type;
        marker->parent=0;
...
//    for(int i = 0; i < inputMarkers.size(); i++)
//    {
//        if(inputMarkers[i]->parent != 0)
//        {
//            outmarkers[i]->parent=outmarkers[ind[inputMarkers[i]->parent]];
//        }
//    }
    return true;
}
