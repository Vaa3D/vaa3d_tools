...
bool instantiateR(int argc, char *argv[], vector<int> branchid, vector<double> Angles)
{
    ...
    R.assign(branchid,"branchid");
    R.assign(Angles,"Angles");
...
void QtSholl::setupDisplay(void)  {
    ...
    window->setWindowTitle("Angle Plot");
...
void QtSholl::plot(void) {
    ...
    std::string cmd1_1 = "df=data.frame(x=branchid,y=Angles);";
    std::string cmd1_2 = "p <- ggplot(df, aes(x=x,y=y)) + geom_point(colour=ifelse(Angles>90, 'red','green')) + annotate(geom = 'text', label = 'vaa3d.org', x = Inf, y = Inf, size=6, hjust = 1.2, vjust = 2, colour='lightblue');";
    std::string cmd1_3 = "p <- p + labs(title='Angle Plot', x='Ids of branch nodes[dendrite]', y='Angles of branch') + theme(plot.title = element_text(size=15)) + theme(axis.text = element_text(size=10)) + theme(axis.title = element_text(size=13));";
