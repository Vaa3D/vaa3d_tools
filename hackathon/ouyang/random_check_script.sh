...
then
    ...
    image1=$2/${arr[randomnum1]}.tocheck/${rdimgename1}
    image2=$2/${arr[randomnum2]}.tocheck/${rdimgename2}
    DIFF=$(diff $image1 $image2)
  if [ "$DIFF" != "" ]
    then
      echo ${image1} "and" ${image2} ":are different,testing result passes" 
