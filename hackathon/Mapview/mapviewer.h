...
public:
	...
	QPoint dragThumbnailWinS;///thumbnail start point
	QPoint dragThumbnailWinE;///thumbnail end point
	
	QPoint dragLocalPositionS;///thumbnail start point
	QPoint dragLocalPositionE;///thumbnail end point
...
public:
	...
	void Search_file(V3DPluginCallback &callback, QWidget *parent);
...
class Mutthread_resampraw : public QThread
{
	//QT_THREAD_SUPPORT
	Q_OBJECT
public:
	Mutthread_resampraw(const QString filename, char * imgFile, V3DLONG *original_sz, int target_level);
	void run();
	Y_VIM<float, long, indexed_t<long, float>, LUT<long> > vim;
	QString m_FileName;
	char * imgSrcFile;
