...
void MAPiewerPlugin::domenu(const QString &menu_name, V3DPluginCallback &callback, QWidget *parent)
{
	...
	else if (menu_name == tr("search file"))
	{
		//Search_file(callback,parent)
	}
...
//void Search_file::loadtc_save(V3DPluginCallback &callback, QWidget *parent)
//{
////	QString filename;
////	QStringList filenames;
////	filenames = QFileDialog::getOpenFileNames(0, 0,"","*.*",0,0);
////	if(filenames.isEmpty())
////	{
////		v3d_msg("You don't have any file open in the main window.");
////		return;
////	}
////
////	FILE *pFileLUT
////
////	pFileLUT=fopen("tt1.txt","rb");
////
////	if(pFileLUT==NULL)
////	{
////		printf("reda error");
...
void MAPiewerPlugin::resampling_rawdata(V3DPluginCallback &callback, QWidget *parent)
{
	...
	//qDebug()<<"thread_imgSrcFile"<<imgSrcFile;
     ...
     ///////////////////////////////////////////
     int templevel = 0;
	//for(int k = 1; k <=target_pixel_size; k = k+2)
	for(int k = target_pixel_size; k <=0; k=k-2)
	{
		if(k==0)
			k=1;
		templevel++;
...
void XMapView::setImgData(ImagePlaneDisplayType ptype,ImagePixelType dtype,ImageDisplayColorType Ctype,V3DLONG *sz_compressed,V3DLONG cz0, V3DLONG cz1, V3DLONG cz2,V3DLONG xslicesize,V3DLONG yslicesize, V3DLONG zslicesize, unsigned char *pdata,double * p_vmax, double* p_vmin)
{
	...
	switch (Ptype)
	{
		case imgPlaneX://zy
               ...
               //			dragThumbnailWinS.setX((cz-2)/m_scale);
               //
               //			dragThumbnailWinS.setY((cy-2)/m_scale);
               //
               //			dragThumbnailWinE.setX((cz*3/4)/m_scale);
               //
               //			dragThumbnailWinE.setY((cy*3/4)/m_scale);

			dragThumbnailWinS.setX((cz-2));

			dragThumbnailWinS.setY((cy-2));

			dragThumbnailWinE.setX((cz*3/4));

			dragThumbnailWinE.setY((cy*3/4));
		...
		case imgPlaneY://xz
			...
			dragThumbnailWinS.setX(cx-2);

			dragThumbnailWinS.setY(cz-2);

			dragThumbnailWinE.setX(cx*3/4);

			dragThumbnailWinE.setY(cz*3/4);
			break;
		...
		case imgPlaneZ://xy
			...
			dragThumbnailWinE.setX(cx-cx/4);

			dragThumbnailWinE.setY(cy-cy/4);
...
void XMapView::mouseLeftButtonPressEvent(QMouseEvent *e) //080101
{
     ...
     //	}
	if (m_scale>1) //then interpret the press as a drag operation
	{
		dragStartPosition = e->pos();
