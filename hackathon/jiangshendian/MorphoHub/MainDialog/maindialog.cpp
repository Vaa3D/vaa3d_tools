        ...
        else
        {
            if(scanconffile.open(QIODevice::WriteOnly | QIODevice::Text))
            {
                ...
                for(int i=0;i<ApforthisDialog.protocolLevel.size();i++)
                {
                    ...
                    qDebug()<<levelrules;
                    levelrules+="\n";
                    scanconffile.write(levelrules.toLatin1());
                }
                scanconffile.close();
            }
        }
...
void MainDialog::generateNextNeuron()
{
    ...
    if(curNeuron.alreadyInit())
    {
        ...
        //convert checkerlist
        QStringList checkerslist=nextNeuron.checkers.split("&",Qt::SplitBehavior());
        QString newcheckers;
        ...
        //convert updatetime
        QStringList timelist=nextNeuron.updateTime.split("-",Qt::SplitBehavior());
