...
void ImagePreprocessforTracing(const char* filename, char *outFilnename = NULL, const float BIN_THRESHOLD = 0.8, const int CC_2D_THRESHOLD = 9, const int CC_3D_THRESHOLD = 30)
{
    std::ifstream inInfo(g_sImageInfoPath.data());
	int width, height, zSize;
	inInfo>>width>>height>>zSize;
...
void do_PreprocessforTracing()
{
    ...
    ImagePreprocessforTracing(g_sInputRawFilePath.data(),NULL,g_rThreshold);		//output tmp_binarized  tmp_ForTracing.bin
...
void do_Soma_detection()
{
    ...
    //std::ifstream inImg("test_data_83x102x95.raw", std::ios::binary );		//input  ****raw name****
    ...
    //std::ifstream inBinary("tmp_binarized", std::ios::binary );		//input tmp_binarized
...
{
    if(g_rThreshold>1 || g_rThreshold<0){
        //v3d_msg(QString("threshold out of range"),true);
        return 0;
    }
	...
	do_ImageRepair();
	std::cout<<"do_Tracing"<<std::endl;
	do_Tracing();
	std::cout<<"do_PolygonalApproximation"<<std::endl;
