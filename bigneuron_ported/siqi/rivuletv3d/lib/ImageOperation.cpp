...
void ImageOperation::computeHessian(double sigma, double alpha_one, double alpha_two)
{  
    typedef itk::HessianRecursiveGaussianImageFilter<ProbImageType> HessianFilterType;
    HessianFilterType::Pointer hessianFilter = HessianFilterType::New();
    typedef itk::CastImageFilter<ImageType, ProbImageType> CastFilterType;
    CastFilterType::Pointer castFilter = CastFilterType::New();
    castFilter->SetInput(I);
    ...
    // std::cout<<hessianFilter->GetNumberOfInputs()<<"!!!"<<std::endl;
    // std::cout<<"outputname: "<<hessianFilter->GetNumberOfOutputs()<<"!!!"<<std::endl;
    // std::cout<<"outputname: "<<hessianFilter->GetOutput()<<std::endl;

    const itk::Image< itk::SymmetricSecondRankTensor< double, 3 >, 3 >::Pointer hessianimage = hessianFilter->GetOutput();
    const SymmetricSecondRankTensor< double, 3 >::RealValueType v = 3.0;
    itk::Image< itk::SymmetricSecondRankTensor< double, 3 >, 3 > *p = hessianimage.GetPointer();
    // ((itk::Image< itk::SymmetricSecondRankTensor< double, 3 >, 3 >) *p) * v;
    typedef itk::Image< itk::SymmetricSecondRankTensor< double, 3 >, 3 > SRTType;

//      ImageType::IndexType pixelIndex;
//    pixelIndex[0] = 280; // x position
//    pixelIndex[1] = 410; // y position
//    pixelIndex[2] = 30; // z position
//    ImageType::PixelType value = I->GetPixel( pixelIndex );
//    std::cout<<"I can get pixel out : "<<value<<std::endl;
//    ImageType::PixelType value = ImageType::PixelType::New();
...
//        it.NextSlice();
//    }
//    std::cout<<"It looks like the ImageSliceIteratorWithIndex is working now"<<std::endl;

    typedef itk::Hessian3DToVesselnessMeasureImageFilter< double >
...
//    ImageType::RegionType region = this->finalim->GetLargestPossibleRegion();
//    ImageType::SizeType size = region.GetSize();
//    std::cout << size << std::endl;
//    std::cout<<"size work or not ?"<<std::endl;
//    std::ofstream myfile;
    // myfile.open ("vesselnessimage.txt");
    // for(int zi = 0; zi < size[2]; zi = zi + 1)
    // {
    //   for(int yi = 0; yi < size[1]; yi = yi + 1)
    //   {
    //     for( int xi = 0; xi < size[0]; xi = xi + 1 )
    //     {
    //         pixelIndex[0] = xi; // x position
    //         pixelIndex[1] = yi; // y position
    //         pixelIndex[2] = zi; // z position
    //         value = finalim->GetPixel( pixelIndex );
    //         myfile<<xi<<" "<<yi<<" "<<zi<<" "<<value<<"\n";
    //     }
    //   }
    // }
    // myfile.close();
    ...
    // typedef itk::MultiplyImageFilter<HessianFilterType, HessianFilterType, HessianFilterType> MultiplyFilterType;
    // MultiplyFilterType::Pointer multiplyFilter = MultiplyFilterType::New();
    // multiplyFilter->SetInput(HessianFilter->GetOutput());
    // multiplyFilter->SetConstant(3);
    // multiplyFilter->Update();
    // std::cout<<"multiply or not ?"<<std::endl;

    // std::cout<<"It looks like the vesselness is working now"<<std::endl;
    // int indexint = 300;
    // value = vesselnessFilter->GetIndexedOutputs();
    // std::cout<<"vesselnessFilter value :"<<value<<std::endl;


   // SRTType hessianim;
   // hessianim = hessianimage->DataObject();
   // std::cout<<hessianvalue<<std::<<endl;
   //  hessianimage = hessianimage - hessianvalue;
   //  double hessianvalue;
   //  hessianvalue = hessianimage[1];
   //  std::cout<<"hessian value : "<<hessianvalue<<std::endl;
    
   //  ImageType hessianmatrix;
   //  hessianmatrix = hessianFilter->GetOutput();

   //  std::cout<<"Ready to use hessian!"<<std::endl;
   //  typedef itk::ImageSliceIteratorWithIndex< GradientImageType > SliceIteratorType;
   //  std::cout<<"Ready to use hessian!"<<std::endl;
   //  SliceIteratorType inputIt( hessianFilter, hessianFilter->GetRequestedRegion() );
   ...
   //  ImageType, ImageType,
   //  ImageType> EigenVectorAnalysisFilterType;
