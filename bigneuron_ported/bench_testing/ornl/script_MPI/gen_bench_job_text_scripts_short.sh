...
#APP2
  ...
  if [ $METHOD == "2" -o $METHOD == "-1" ]; then
    echo "./start_vaa3d.sh -x vn2 -f app2 -i $inimgfileTracing -p NULL 0 10 1 1 0 0 5 0 0 0;mv  $inimgfileTracing*_app2.swc $finalfileFolder" >> $outputScript;
  fi;
...
#EnsembleNeuronTracerV2n
  ...
  if [ $METHOD == "26" -o $METHOD == "-1" ]; then
    echo "./start_vaa3d.sh -x EnsembleNeuronTracerV2n -f tracing_func -i $inimgfileTracing;mv  $inimgfileTracing*_EnsembleNeuronTracerV2n.swc $finalfileFolder" >> $outputScript;
  fi;

#EnsembleNeuronTracerV2s

  if [ $METHOD == "27" -o $METHOD == "-1" ]; then
    echo "./start_vaa3d.sh -x EnsembleNeuronTracerV2s -f tracing_func -i $inimgfileTracing;mv  $inimgfileTracing*_EnsembleNeuronTracerV2s.swc $finalfileFolder" >> $outputScript;
  fi;

#3DTraceSWC

  if [ $METHOD == "28" -o $METHOD == "-1" ]; then
    echo "./start_vaa3d.sh -x aVaaTrace3D -f func1 -i $inimgfileTracing -p 20 2 2.5;mv  $inimgfileTracing*_pyzh.swc $finalfileFolder" >> $outputScript;
  fi;
}
