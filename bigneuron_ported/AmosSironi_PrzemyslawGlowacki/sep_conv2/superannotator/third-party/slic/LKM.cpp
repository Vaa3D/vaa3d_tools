        ...
        const FloatType cubeness)
{
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			...
			int z1 = max((FloatType)0.0,			kseedsz[n]-offset);
			int z2 = min((FloatType)m_depth,	kseedsz[n]+offset);
	...
	const int&					STEP)
{
	...
	int adjlabel(0);//adjacent label
        int* xvec = new int[SUPSZ*4*2];//a large safe size
        int* yvec = new int[SUPSZ*4*2];//a large safe size
        int* zvec = new int[SUPSZ*4*2];//a large safe size
	//------------------
	// memory allocation
	//------------------
	sidType** nlabels = new sidType*[depth];
