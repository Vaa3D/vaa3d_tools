...
#define mVerbosity (prhs[2])
  ...
  if ( mxGetNumberOfDimensions(mImg) != 2 )
      mexErrMsgTxt("imgData must be 2-dimensional");
  ...
  // create color image (yes, waste of memory and time)
  unsigned int *colorBuff = new unsigned int[sz];
  for (unsigned i=0; i < sz; i++)
  {
      unsigned int val = ubuff[i];
      colorBuff[i] = (val<<16) | (val<<8) | val;
  }
  ...
  lkm->DoSuperpixelSegmentation(colorBuff, width, height, klabels, numlabels, STEP, M);

  mexPrintf("Done %d regions, generating output!\n", numlabels);
  mexEvalString("drawnow");

  plhs[0] = mxCreateNumericArray( 2, mImgDims, mxUINT32_CLASS, mxREAL );

  unsigned int *outData = (unsigned int *) mxGetData(plhs[0]);


  memcpy( outData, klabels, sz*sizeof(unsigned int) );
  ...
  if (nlhs > 2) // then the 'inverse' list was requested
  {
      ...
      for (int i=0; i < numlabels; i++) // go for each label and find the corresponding pixel idxs
      {
          ...
          {
              if (klabels[idx] == i)
                  pixIdxs.push_back(idx + 1);    // +1 is for matlab compatibility
          }
