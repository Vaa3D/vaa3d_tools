...
namespace SQB
{
    ...
    public:
        ...
        typedef double (*FuncPtrType)( const ArrayType &prevScores, const ArrayType &newScores, double alpha );


        // WIll COPY THE DATA!! Not optimal!
        ArrayType mPrevScores, mNewScores, mY;
        ...
        LineSearch( const ParamArrayType &prevScores, const ParamArrayType &newScores, const ParamArrayType &Y, LossType loss, std::vector<unsigned> *subSampIdxs = 0 )
            : mLossType(loss)
        {
            mLossScale = 0.0;

            // then we don't have to do it on all the data
            if ( subSampIdxs != 0 )
            {
                const unsigned N = subSampIdxs->size();

                mPrevScores.resize(N);
                mNewScores.resize(N);
                mY.resize(N);

                for (unsigned i=0; i < N; i++)
                {
                    const unsigned idx = (*subSampIdxs)[i];
                    mPrevScores.coeffRef(i) = prevScores.coeff( idx );
                    mNewScores.coeffRef(i) = newScores.coeff( idx );
                    mY.coeffRef(i) = Y.coeff( idx );
                }
            }
            else
            {
                mPrevScores = prevScores;
        ...
        double run()
        {
			...
			if (std::isnan(toRet) || std::isinf(toRet))
				toRet = 0;
