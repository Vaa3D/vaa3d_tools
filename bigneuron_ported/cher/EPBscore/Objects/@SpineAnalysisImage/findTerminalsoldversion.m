...
if m1 < (1-th)*y(maxInd(1)) & m2 < (1-th)*y(maxInd(1)); termInd=[termInd, maxInd(1)]; end

m1=y(max(minInd(find(minInd < maxInd(length(maxInd))))));
m2=y(min(minInd(find(minInd > maxInd(length(maxInd))))));
if m1 < (1-th)*y(maxInd(length(maxInd))) & m2 < (1-th)*y(maxInd(length(maxInd))); termInd=[termInd, maxInd(length(maxInd))]; end
...
for i=2:(length(maxInd)-1)
    ...
    switch true
       ...
       % case m1 < (1-th)*y(maxInd(i)) & m2 < (1-th)*y(maxInd(i))
        case m1 < (1-th)*y(maxInd(i)) & m2 < (1-th)*y(maxInd(i))
        ...
        case maxInd(i+1)-maxInd(i) < conv*n
            ...
            if (m1 < (1-th)*y(maxInd(i)) & m3 < (1-th/convCorr)*y(maxInd(i+1))) & (m3 < (1-th)*y(maxInd(i+1))) % & peakDiff < th
