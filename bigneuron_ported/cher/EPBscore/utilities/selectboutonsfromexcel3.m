...
for i=1:size(data,1)
   for j=2:12
        ...
        % going up
        data{i,j+startingratiocol*3-4}= data{i,j+startingratiocol-2}*(data{i,j}>=data{i,j-1});        
        % going down
        data{i,j+startingratiocol*4-5}= data{i,j+startingratiocol-2}*(data{i,j}<data{i,j-1});
        % raw ratio
