...
void write_data(std::vector< std::vector<struct RawPoints > > &dend, QString &path){
    ...
    QString dataFile = path.remove(path.size()-4,4)  + name + QLatin1String(".swc");
    err = fopen_s(&fout,dataFile.toStdString().c_str(), "w");
