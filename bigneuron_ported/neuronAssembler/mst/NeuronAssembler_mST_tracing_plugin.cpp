...
using namespace std;
#include "NeuronAssembler_mST_tracing_plugin.h"
...
Q_EXPORT_PLUGIN2(NeuronAssembler_mST_tracing, NeuronAssembler_mST_tracing);
...
QStringList NeuronAssembler_mST_tracing::menulist() const
{
	return QStringList() 
		<<tr("trace_tc")
		<<tr("trace_raw")
...
QStringList NeuronAssembler_mST_tracing::funclist() const
{
	return QStringList()
		<<tr("trace_tc")
		<<tr("trace_raw")
...
void NeuronAssembler_mST_tracing::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
...
bool NeuronAssembler_mST_tracing::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
//        v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
...
//bool neuronassembler::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    //    P.bkg_thresh = (paras.size() >= k+1) ? atof(paras[k]) : 10; k++;
		...
		P.channel = (paras.size() >= k+1) ? atoi(paras[k]):1; k++;
		P.win = (paras.size() >= k+1) ? atoi(paras[k]):5; k++;
        assembler_tc(callback,parent,P,bmenu);
	}
...
//        printf("vaa3d -x plugin_name -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <tracing_method> <channel> <bkg_thresh> <block size>\n");
		printf("vaa3d -x NeuronAssembler_mST_tracing -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <block size> <channel> <win>\n");
		...
		printf("win			Required by the tracing algorithm. Default value is 5\n");

		printf("outswc_file		Will be named automatically based on the input image file name, so you don't have to specify it.\n\n");


		printf("vaa3d -x NeuronAssembler_mST_tracing -f trace_raw -i <inimg_file> -p <inmarker_file> <block size> <tracing_entire_image> <channel> <win>\n");
...
bool assembler_tc(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	...
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_mST_tracing.swc";
	QString tmpfolder = QFileInfo(tcfile).path()+("/tmp_NeuronAssembler_mST_tracing");
	head->tilename = QFileInfo(tcfile).path().append("/tmp_NeuronAssembler_mST_tracing/").append(QString(region_name));
   // QString tmpfolder = QFileInfo(tcfile).path()+("/tmp");
    ...
    while(walker != NULL)
    {
      ...
      //  QString swcfilename =  walker->tilename + QString("_MOST.swc");
		QString swcfilename =  walker->tilename + QString("_MST_Tracing.swc");
...
//        char channel = '0' + P.channel;
		...
		string S_win = boost::lexical_cast<string>(P.win);
		char* C_win = new char[S_win.length() + 1];
		strcpy(C_win,S_win.c_str());
		arg_para.push_back(C_win);

		full_plugin_name = "MST_tracing";  func_name =  "trace_mst";
  ...
  //      full_plugin_name = "mostVesselTracer"; func_name = "MOST_trace";
        ...
        for(V3DLONG i = 0; i < nt_pruned.listNeuron.size(); i++)
        {
            ...
            if(curr.x < 0.05* P.block_size && left == 0 && walker->direction !=2)
            {
				...
				newNode->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_mST_tracing/").append(QString(region_name));
...
bool assembler_raw(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_mST_tracing.swc";
	QString tmpfolder = QFileInfo(fileOpenName).path()+("/tmp_NeuronAssembler_mST_tracing");
	head->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_mST_tracing/").append(QString(region_name));
