   ...
   //     P.channel = dialog.channel;
	...
	else
	{
		v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
			"Developed by Zhi Zhou, 2015-11-04"));
	}
}
...
//bool neuronassembler::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    //    P.bkg_thresh = (paras.size() >= k+1) ? atof(paras[k]) : 10; k++;
		...
		P.gsdt = (paras.size() >= k+1) ? atoi(paras[k]):0; k++;
		P.gap = (paras.size() >= k+1) ? atoi(paras[k]):0; k++;
		P.length = (paras.size() >= k+1) ? atoi(paras[k]):5; k++;
		P.region = (paras.size() >= k+1) ? atoi(paras[k]):1000; k++;
        assembler_tc(callback,parent,P,bmenu);
	}
...
//        printf("vaa3d -x plugin_name -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <tracing_method> <channel> <bkg_thresh> <block size>\n");
        printf("vaa3d -x NeuronAssembler_region_app2 -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <block size> <ch> <th> <b_256> <gsdt> <gap> <length> <region>\n");
		...
		printf("gsdt			Required by the tracing algorithm. Default value is 0\n");
		printf("gap			Required by the tracing algorithm. Default value is 0\n");
		printf("length			Required by the tracing algorithm. Default value is 5\n");
		printf("region			Required by the tracing algorithm. Default value is 1000\n");

		printf("outswc_file		Will be named automatically based on the input image file name, so you don't have to specify it.\n\n");


        printf("vaa3d -x NeuronAssembler_region_app2 -f trace_raw -i <inimg_file> -p <inmarker_file> <block size> <tracing_entire_image> <ch> <th> <b_256> <gsdt> <gap> <length> <region>\n");
...
bool assembler_tc(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
    ...
    QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_region_app2.swc";
    QString tmpfolder = QFileInfo(tcfile).path()+("/tmp_NeuronAssembler_region_app2");
    head->tilename = QFileInfo(tcfile).path().append("/tmp_NeuronAssembler_region_app2/").append(QString(region_name));
   // QString tmpfolder = QFileInfo(tcfile).path()+("/tmp");
    ...
    while(walker != NULL)
    {
      ...
      //  QString swcfilename =  walker->tilename + QString("_MOST.swc");
		QString swcfilename =  walker->tilename + QString("_region_APP2.swc");
...
//        char channel = '0' + P.channel;
		...
		string S_gsdt = boost::lexical_cast<string>(P.gsdt);
		char* C_gsdt = new char[S_gsdt.length() + 1];
		strcpy(C_gsdt,S_gsdt.c_str());
		arg_para.push_back(C_gsdt);

		string S_gap = boost::lexical_cast<string>(P.gap);
		char* C_gap = new char[S_gap.length() + 1];
		strcpy(C_gap,S_gap.c_str());
		arg_para.push_back(C_gap);

		string S_length = boost::lexical_cast<string>(P.length);
		char* C_length = new char[S_length.length() + 1];
		strcpy(C_length,S_length.c_str());
		arg_para.push_back(C_length);
		...
		full_plugin_name = "Region_Neuron2";  func_name =  "trace_app2";
  ...
  //      full_plugin_name = "mostVesselTracer"; func_name = "MOST_trace";
        ...
        for(V3DLONG i = 0; i < nt_pruned.listNeuron.size(); i++)
        {
            ...
            if(curr.x < 0.05* P.block_size && left == 0 && walker->direction !=2)
            {
                ...
                newNode->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_region_app2/").append(QString(region_name));
...
bool assembler_raw(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
    QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_region_app2.swc";
    QString tmpfolder = QFileInfo(fileOpenName).path()+("/tmp_NeuronAssembler_region_app2");
    head->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_region_app2/").append(QString(region_name));
