...
using namespace std;
#include "NeuronAssembler_tReMap_plugin.h"
...
Q_EXPORT_PLUGIN2(NeuronAssembler_tReMap, NeuronAssembler_tReMap);
...
QStringList NeuronAssembler_tReMap::menulist() const
{
	return QStringList() 
		<<tr("trace_tc")
		<<tr("trace_raw")
...
QStringList NeuronAssembler_tReMap::funclist() const
{
	return QStringList()
		<<tr("trace_tc")
		<<tr("trace_raw")
...
void NeuronAssembler_tReMap::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
//void neuronassembler::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
   ...
   //     P.bkg_thresh = dialog.bkg_thresh;
		...
		P.is_gap = dialog.is_gap;
		P.length_th = dialog.length_th;
...
bool NeuronAssembler_tReMap::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
//        v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
...
//bool neuronassembler::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    //    P.bkg_thresh = (paras.size() >= k+1) ? atof(paras[k]) : 10; k++;
		...
		P.mip = (paras.size() >= k+1) ? atoi(paras[k]):0; k++;
		P.ch = (paras.size() >= k+1) ? atoi(paras[k]):1; k++;
		P.th = (paras.size() >= k+1) ? atof(paras[k]):10; k++;
		P.b_256 = (paras.size() >= k+1) ? atoi(paras[k]):0; k++;
		P.is_gsdt = (paras.size() >= k+1) ? atoi(paras[k]):1; k++;
		P.is_gap = (paras.size() >= k+1) ? atoi(paras[k]):0; k++;
		P.length_th = (paras.size() >= k+1) ? atoi(paras[k]):5; k++;
        assembler_tc(callback,parent,P,bmenu);
	}
...
//        printf("vaa3d -x plugin_name -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <tracing_method> <channel> <bkg_thresh> <block size>\n");
		printf("vaa3d -x NeuronAssembler_tReMap -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <block size> <mip> <ch> <th> <b_256> <is_gsdt> <is_gap> <length_th>\n");
...
bool assembler_tc(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	...
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_tReMap.swc";
	QString tmpfolder = QFileInfo(tcfile).path()+("/tmp_NeuronAssembler_tReMap");
	head->tilename = QFileInfo(tcfile).path().append("/tmp_NeuronAssembler_tReMap/").append(QString(region_name));
...
//        char channel = '0' + P.channel;
		string S_mip = boost::lexical_cast<string>(P.mip);
		char* C_mip = new char[S_mip.length() + 1];
		strcpy(C_mip,S_mip.c_str());
		arg_para.push_back(C_mip);
		...
		string S_is_gsdt = boost::lexical_cast<string>(P.is_gsdt);
		char* C_is_gsdt = new char[S_is_gsdt.length() + 1];
		strcpy(C_is_gsdt,S_is_gsdt.c_str());
		arg_para.push_back(C_is_gsdt);

		string S_is_gap = boost::lexical_cast<string>(P.is_gap);
		char* C_is_gap = new char[S_is_gap.length() + 1];
		strcpy(C_is_gap,S_is_gap.c_str());
		arg_para.push_back(C_is_gap);

		string S_length_th = boost::lexical_cast<string>(P.length_th);
		char* C_length_th = new char[S_length_th.length() + 1];
		strcpy(C_length_th,S_length_th.c_str());
		arg_para.push_back(C_length_th);

		full_plugin_name = "TReMap";  func_name =  "trace_mip";
  ...
  //      full_plugin_name = "mostVesselTracer"; func_name = "MOST_trace";
        ...
        for(V3DLONG i = 0; i < nt_pruned.listNeuron.size(); i++)
        {
            ...
            if(curr.x < 0.05* P.block_size && left == 0 && walker->direction !=2)
            {
				...
				newNode->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_tReMap/").append(QString(region_name));
...
bool assembler_raw(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_tReMap.swc";
	QString tmpfolder = QFileInfo(fileOpenName).path()+("/tmp_NeuronAssembler_tReMap");
	head->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_tReMap/").append(QString(region_name));
