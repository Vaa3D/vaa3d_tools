...
using namespace std;
#include "NeuronAssembler_Snake_plugin.h"
...
Q_EXPORT_PLUGIN2(NeuronAssembler_Snake, NeuronAssembler_Snake);
...
QStringList NeuronAssembler_Snake::menulist() const
{
	return QStringList() 
		<<tr("trace_tc")
		<<tr("trace_raw")
...
QStringList NeuronAssembler_Snake::funclist() const
{
	return QStringList()
		<<tr("trace_tc")
		<<tr("trace_raw")
...
void NeuronAssembler_Snake::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
...
bool NeuronAssembler_Snake::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
//        v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
...
//        printf("vaa3d -x plugin_name -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <tracing_method> <channel> <bkg_thresh> <block size>\n");
		printf("vaa3d -x NeuronAssembler_Snake -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <block size> <ch>\n");
		...
		printf("vaa3d -x NeuronAssembler_Snake -f trace_raw -i <inimg_file> -p <inmarker_file> <block size> <tracing_entire_image> <ch>\n");
...
bool assembler_tc(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	...
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_Snake.swc";
	QString tmpfolder = QFileInfo(tcfile).path()+("/tmp_NeuronAssembler_Snake");
	head->tilename = QFileInfo(tcfile).path().append("/tmp_NeuronAssembler_Snake/").append(QString(region_name));
   // QString tmpfolder = QFileInfo(tcfile).path()+("/tmp");
    ...
    while(walker != NULL)
    {
      ...
      //  QString swcfilename =  walker->tilename + QString("_MOST.swc");
		QString swcfilename =  walker->tilename + QString("_snake.swc");
...
//        char channel = '0' + P.channel;
		...
		full_plugin_name = "snake";  func_name =  "snake_trace";
  ...
  //      full_plugin_name = "mostVesselTracer"; func_name = "MOST_trace";
        ...
        for(V3DLONG i = 0; i < nt_pruned.listNeuron.size(); i++)
        {
            ...
            if(curr.x < 0.05* P.block_size && left == 0 && walker->direction !=2)
            {
				...
				newNode->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_Snake/").append(QString(region_name));
...
bool assembler_raw(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_Snake.swc";
	QString tmpfolder = QFileInfo(fileOpenName).path()+("/tmp_NeuronAssembler_Snake");
	head->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_Snake/").append(QString(region_name));
