...
QStringList NeuronAssembler_region_neuronchaser::menulist() const
{
	return QStringList() 
		<<tr("trace_tc")
		<<tr("trace_raw")
...
QStringList NeuronAssembler_region_neuronchaser::funclist() const
{
	return QStringList()
		<<tr("trace_tc")
		<<tr("trace_raw")
...
void NeuronAssembler_region_neuronchaser::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
//void neuronassembler::domenu(const QString &menu_name, V3DPluginCallback2 &callback, QWidget *parent)
{
   ...
   //     P.bkg_thresh = dialog.bkg_thresh;
		...
		P.scal = dialog.scal;
		P.perc = dialog.perc;
		P.znccTh = dialog.znccTh;
		P.Ndir = dialog.Ndir;
		P.angSig = dialog.angSig;
		P.Ni = dialog.Ni;
		P.Ns = dialog.Ns;
		P.zDist = dialog.zDist;
		P.saveMidres = dialog.saveMidres;
   //     P.channel = dialog.channel;
	...
	else
	{
		v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
			"Developed by Zhi Zhou , 2015-01-01"));
	}
}

bool NeuronAssembler_region_neuronchaser::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
//        v3d_msg(tr("This is a test plugin, you can use it as a demo.. "
...
//bool neuronassembler::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
    ...
    //    P.bkg_thresh = (paras.size() >= k+1) ? atof(paras[k]) : 10; k++;
		...
		P.bkg_thresh = (paras.size() >= k+1) ? atoi(paras[k]):40; k++;
...
//        printf("vaa3d -x plugin_name -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <tracing_method> <channel> <bkg_thresh> <block size>\n");
		printf("vaa3d -x NeuronAssembler_region_neuronchaser -f trace_tc -i <inimg_file> -p <inmarker_file> <tc_file> <block size> <bkg_thresh> <region_number> <channel> <scal> <perc> <znccTh> <Ndir> <angSig> <Ni> <Ns> <zDist> <saveMidres>\n");
		...
		printf("bkg_thresh		Required by the tracing algorithm. Default value is 40\n");
		printf("region_number		Required by the tracing algorithm. Default value is 10000\n");
		...
		printf("perc			Required by the tracing algorithm. Default value is 90\n");
		...
		printf("angSig			Required by the tracing algorithm. Default value is 30\n");
		...
		printf("saveMidres		Required by the tracing algorithm. Default value is 0\n");

		printf("outswc_file		Will be named automatically based on the input image file name, so you don't have to specify it.\n\n");


		printf("vaa3d -x NeuronAssembler_region_neuronchaser -f trace_raw -i <inimg_file> -p <inmarker_file> <block size> <tracing_entire_image> <bkg_thresh> <region_number> <channel> <scal> <perc> <znccTh> <Ndir> <angSig> <Ni> <Ns> <zDist> <saveMidres>\n");
...
bool assembler_tc(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	...
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_region_neuronchaser.swc";
	QString tmpfolder = QFileInfo(tcfile).path()+("/tmp_NeuronAssembler_region_neuronchaser");
	head->tilename = QFileInfo(tcfile).path().append("/tmp_NeuronAssembler_region_neuronchaser/").append(QString(region_name));
   // QString tmpfolder = QFileInfo(tcfile).path()+("/tmp");
    ...
    while(walker != NULL)
    {
      ...
      //  QString swcfilename =  walker->tilename + QString("_MOST.swc");
		QString swcfilename =  walker->tilename + QString("_region_neuronchaser.swc");
...
//        char channel = '0' + P.channel;
		string S_bkg_thresh = boost::lexical_cast<string>(P.bkg_thresh);
		char* C_bkg_thresh = new char[S_bkg_thresh.length() + 1];
		strcpy(C_bkg_thresh,S_bkg_thresh.c_str());
		arg_para.push_back(C_bkg_thresh);

		string S_region_number = boost::lexical_cast<string>(P.region_number);
		char* C_region_number = new char[S_region_number.length() + 1];
		strcpy(C_region_number,S_region_number.c_str());
		arg_para.push_back(C_region_number);
		...
		string S_scal = boost::lexical_cast<string>(P.scal);
		char* C_scal = new char[S_scal.length() + 1];
		...
		strcpy(C_znccTh,S_znccTh.c_str());
		arg_para.push_back(C_znccTh);

		string S_Ndir = boost::lexical_cast<string>(P.Ndir);
		char* C_Ndir = new char[S_Ndir.length() + 1];
		strcpy(C_Ndir,S_Ndir.c_str());
		arg_para.push_back(C_Ndir);

		string S_angSig = boost::lexical_cast<string>(P.angSig);
		char* C_angSig = new char[S_angSig.length() + 1];
		strcpy(C_angSig,S_angSig.c_str());
		arg_para.push_back(C_angSig);

		string S_Ni = boost::lexical_cast<string>(P.Ni);
		char* C_Ni = new char[S_Ni.length() + 1];
		strcpy(C_Ni,S_Ni.c_str());
		arg_para.push_back(C_Ni);

		string S_Ns = boost::lexical_cast<string>(P.Ns);
		char* C_Ns = new char[S_Ns.length() + 1];
		strcpy(C_Ns,S_Ns.c_str());
		arg_para.push_back(C_Ns);

		string S_zDist = boost::lexical_cast<string>(P.zDist);
		char* C_zDist = new char[S_zDist.length() + 1];
		strcpy(C_zDist,S_zDist.c_str());
		arg_para.push_back(C_zDist);

		string S_saveMidres = boost::lexical_cast<string>(P.saveMidres);
		char* C_saveMidres = new char[S_saveMidres.length() + 1];
		strcpy(C_saveMidres,S_saveMidres.c_str());
		arg_para.push_back(C_saveMidres);

		full_plugin_name = "Region_neuronchaser";  func_name =  "trace_neuronchaser";
  ...
  //      full_plugin_name = "mostVesselTracer"; func_name = "MOST_trace";
        ...
        for(V3DLONG i = 0; i < nt_pruned.listNeuron.size(); i++)
        {
            ...
            if(curr.x < 0.05* P.block_size && left == 0 && walker->direction !=2)
            {
				...
				newNode->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_region_neuronchaser/").append(QString(region_name));
...
bool assembler_raw(V3DPluginCallback2 &callback, QWidget *parent,NA_PARA &P,bool bmenu)
{
   ...
   // QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_MOST.swc";
	QString finalswcfilename = fileOpenName + rootposstr + "_NeuronAssembler_region_neuronchaser.swc";
	QString tmpfolder = QFileInfo(fileOpenName).path()+("/tmp_NeuronAssembler_region_neuronchaser");
	head->tilename = QFileInfo(fileOpenName).path().append("/tmp_NeuronAssembler_region_neuronchaser/").append(QString(region_name));
