function score_features = predict_ftrs_3D_v2(X,ftrs,sample_idx,weak_learners)

% tmp_image = X(:,:,1);

score_features = zeros(size(ftrs,1),1);
...
%     new_score = LDARegStumpPredict(wl.reg_tree,single(ftrs));
    
    [~,new_score] = predict_RF_vector_v2(single(ftrs),wl);
